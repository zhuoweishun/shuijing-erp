    ...
    if (data.length === 0) {
      return (
        <div className="empty-state-mobile">
          <div className="text-mobile-body text-gray-500">{emptyText}</div>
        </div>
      )
    }
      ...
      <div className="space-mobile-sm">
        {data.map((record, index) => {
          ...
          const secondaryColumns = columns.filter(col => 
            !col.hideOnMobile && col.priority === 'medium'
          )
          
          // 获取详细信息列
          const detailColumns = columns.filter(col => 
            !col.hideOnMobile && col.priority === 'low'
          )
...
function MobilePagination({ current, total, pageSize, onChange }: MobilePaginationProps) {
  const { isMobile } = useDeviceDetection()
