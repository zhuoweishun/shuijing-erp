      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          <p className="text-sm text-gray-600">
            总消耗: {data.consumed.toLocaleString()} 颗/件
          </p>
          <p className="text-sm text-gray-600">
            消耗次数: {data.count} 次
          </p>
          <p className="text-sm text-gray-600">
            平均消耗: {data.avgConsumption} 颗/件
          </p>
          <p className="text-sm text-gray-600">
            品相: {data.quality}
          </p>
      ...
      <div className="mb-4">
        ...
        {data && (
          <div className="mt-3 flex flex-wrap gap-4 text-sm text-gray-600">
            <span>总消耗量: {data.total_consumption.toLocaleString()} 颗/件</span>
      ...
      {loading ? (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {data?.top_consumed_products?.map((item, index) => (
                    <tr key={item.purchase_id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                        {item.total_consumed.toLocaleString()} 颗/件
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                        {item.consumption_count} 次
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                        {Math.round(item.avg_consumption * 100) / 100} 颗/件
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
        ...
        <div className="flex items-center justify-center h-64 text-gray-500">
          <div className="text-center">
            <p className="text-sm mb-1">暂无消耗数据</p>
            <p className="text-xs">当前时间范围内没有库存消耗记录</p>
          </div>
        </div>
      )}
    </div>
