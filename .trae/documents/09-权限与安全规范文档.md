...
| 雇员 | 库存管理  | 查询（仅基础信息）                  | 隐藏 price\_per\_bead、supplier\_name、unit\_price、total\_price | 《API 接口规范文档》5.1 |
| 老板 | 库存弹窗  | 查看完整信息（含价格、批次、CG编号）       | 可见所有字段，包括价格信息和供应商详情                         | 《UI设计规范文档》5.6   |
| 雇员 | 库存弹窗  | 查看基础信息（隐藏价格）               | 隐藏 unit\_price、total\_price、supplier\_name      | 《UI设计规范文档》5.6   |
...
| inventory\_view | price\_per\_bead、supplier\_name、unit\_price、total\_price | 返回 null         | 《API 接口规范文档》5.1 |
| 库存弹窗数据        | unit\_price、total\_price、supplier\_name           | 前端隐藏显示        | 《UI设计规范文档》5.6   |
...
const usePermission = () => {
  ...
  const filterSensitiveData = <T>(data: T): T => {
    ...
    const sensitiveFields = [
      ...
      'unit_price', 'supplier_name' // 库存弹窗敏感字段
    ];
