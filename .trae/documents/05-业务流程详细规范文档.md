...
**核心修复内容：**
1. **字段名称统一**：purchase_name替代product_name，确保前后端字段完全一致
2. **类型名称修复**：FINISHED_MATERIAL替代FINISHED，修复成品库存查询问题
3. **表单验证增强**：按采购类型进行差异化验证，提升数据质量
4. **图片处理优化**：支持多文件上传、格式验证、大小限制和错误处理
5. **供应商管理**：支持实时搜索、拼音排序和创建新供应商
6. **自动计算逻辑**：根据类型自动计算相关字段，避免手动计算错误
7. **数据同步机制**：采购数据自动同步到materials表，支持库存管理
8. **错误处理完善**：用户友好的错误提示和重试机制
