...
class TableFilterFieldReplacer:
    def __init__(self):
        self.project_root = Path("d:/shuijing ERP")
        self.backup_dir = self.project_root / "backups" / f"table_filter_fields_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        self.field_mappings = {
            ...
            'tempFilterPosition': 'temp_filter_position',
            'productTypeFilter': 'product_type_filter',
            'lowStockFilter': 'low_stock_filter',
            ...
            # 表格相关字段
            'tableMobile': 'table_mobile',
            'tableContainerMobile': 'table_container_mobile'
        }
    ...
    def replace_in_content(self, content, file_path):
        ...
        for old_field, new_field in self.field_mappings.items():
            ...
            replacements = [
                ...
                # JSX属性: <Component fieldName={value} />
                (rf'\b{old_field}=', f'{new_field}='),
    ...
    def generate_report(self):
        ...
        # 保存报告
        report_file = self.project_root / f"table_filter_field_conversion_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
if __name__ == "__main__":
    replacer = TableFilterFieldReplacer()
    replacer.run()