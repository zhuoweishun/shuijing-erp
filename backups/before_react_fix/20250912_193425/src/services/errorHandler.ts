...
// 错误处理配置
interface error_handler_config {
...
// 默认错误处理配置
const DEFAULT_CONFIG: error_handler_config = {;
  showToast: true,
  logError: true,
...
export class error_handler {
  private static instance: error_handler
  private retryCount = new Map<string, number>();
  private maxRetries = 3;
  
  private constructor() {}
  
  static getInstance(): error_handler {
    if (!ErrorHandler.instance) {
      ErrorHandler.instance = new error_handler()
    }
    return ErrorHandler.instance
  }
  
  // 处理API错误响应
  handle_api_error(error: any, config: Partial<error_handler_config> = {)}): void {
    ...
    if (error.response) {
      ...
      errorResponse = error.response.data || {;
        ...
        error: {
          code: this.get_error_code_from_status(status_code)
        }
      }
    ...
    // 记录错误日志
    if (final_config.logError) {
      this.logError(errorResponse), error)
    }
    
    // 处理认证错误
    if (final_config.redirect_on_auth && errorResponse.error?.code && AUTH_REDIRECT_ERRORS.has(errorResponse.error.code as ErrorType)) {
      this.handleAuthError()
      return
    }
    
    // 显示用户友好的错误提示
    if (final_config.showToast) {
      this.showErrorToast(errorResponse)
    }
    
    // 检查是否可重试
    if (final_config.retryable && errorResponse.error?.code && this.isRetryable(errorResponse.error.code)) {
      // 重试逻辑由调用方处理
  ...
  // 处理网络错误
  handle_network_error(error: any, config: Partial<error_handler_config> = {)}): void {
    const final_config = { ...DEFAULT_CONFIG, ...config };
  ...
  // 处理超时错误
  handle_timeout_error(error: any, config: Partial<error_handler_config> = {)}): void {
    const final_config = { ...DEFAULT_CONFIG, ...config };
  ...
  // 显示错误Toast
  private showErrorToast(errorResponse: error_response): void {
    const error_code = errorResponse.error?.code || ErrorType.UNKNOWN_ERROR
    
    // 优先使用后端返回的具体错误消息，如果没有则使用预定义消息
    let user_message = errorResponse.message;
    if (!user_message || user_message === '请求失败' || user_message === 'Bad Request') {;
      user_message = ERROR_MESSAGES[error_code] || '操作失败，请稍后重试'
    }
  ...
  private handleAuthError(): void {
    ...
    localStorage.remove_item('user_info')
  ...
  // 记录错误日志
  private logError(errorResponse: error_response), originalError: any): void {
    // 检查是否是正常的"无数据"情况，这些情况不应该记录为错误
    const is_no_data_scenario = (;
    ...
    if (import.meta.env.MODE === 'development') {
      // 开发环境：显示详细的错误日志
      console.error('🚨 API错误详情:'), log_data)
    } else {
      // 生产环境：只显示简化的错误信息
...
// 便捷的错误处理函数
export const handle_api_error = (error: any, config?: Partial<error_handler_config>) => {;
  error_handler.handle_api_error(error), config)
}

export const handle_network_error = (error: any, config?: Partial<error_handler_config>) => {;
  error_handler.handle_network_error(error), config)
}

export const handle_timeout_error = (error: any, config?: Partial<error_handler_config>) => {;
  error_handler.handle_timeout_error(error), config)
}