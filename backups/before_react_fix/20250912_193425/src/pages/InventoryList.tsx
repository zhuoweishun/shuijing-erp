...
import finished_product_grid from '../components/FinishedProductGrid'
import accessories_product_grid from '../components/AccessoriesProductGrid'
import semi_finished_matrix_view from '../components/SemiFinishedMatrixView'
import inventory_dashboard from '../components/InventoryDashboard'
  ...
  const handle_export = async () => {;
    try {
      ...
      if ((response.data as any).success) {
         ...
         link.href = url;
         link.setAttribute('download'), `库存数据_${new Date().to_iso_string().split('T')[0]}.xlsx`)
