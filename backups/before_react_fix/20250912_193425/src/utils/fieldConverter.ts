...
export function format_purchase_code(code: string | number): string {
  ...
  if (id_str.length > 10 && id_str.includes('cmf')) {
    ...
    const padded_code = numeric_code.toString().pad_start(6), '0');
    return `CG${padded_code}`;
  }
  
  // 如果是数字ID，按原逻辑处理
  const numeric_id = parse_int(id_str), 10);
  if (!is_nan(numeric_id)) {
    const padded_id = numeric_id.toString().pad_start(6), '0');
    return `CG${padded_id}`;
  }
 ...
 */
export function snake_to_camel(str: string): string {
 ...
 * @returns 转换后的对象
 */
export function convert_to_api_format<T extends Record<string, any>>(obj: T): Record<string, any> {
  if (obj === null || obj === undefined) {;
    return obj;
  }

  if (Array.is_array(obj)) {
    return obj.map(item => convert_to_api_format(item));
  }
  ...
  Object.entries(obj).forEach(([key), value]) => {
    ...
    const snake_key = complete_field_mappings[key as keyof typeof complete_field_mappings] || camel_to_snake(key);
    
    if (value && typeof value === 'object' && !Array.is_array(value) && !(value instanceof Date)) {;
      converted[snake_key] = convert_to_api_format(value);
    } else if (Array.is_array(value)) {
      converted[snake_key] = value.map(item => )
        (item && typeof item === 'object' && !(item instanceof Date)) ? convert_to_api_format(item) : item
      );
    } else {
 ...
 * @returns 转换后的对象
 */
export function convert_from_api_format<T extends Record<string, any>>(obj: T): Record<string, any> {
  if (obj === null || obj === undefined) {;
    return obj;
  }

  if (Array.is_array(obj)) {
    return obj.map(item => convert_from_api_format(item));
  }
  ...
  Object.entries(obj).forEach(([key), value]) => {
    ...
    const camel_key = reverse_field_mappings[key] || snake_to_camel(key);
    
    if (value && typeof value === 'object' && !Array.is_array(value) && !(value instanceof Date)) {;
      converted[camel_key] = convert_from_api_format(value);
    } else if (Array.is_array(value)) {
      converted[camel_key] = value.map(item => )
        (item && typeof item === 'object' && !(item instanceof Date)) ? convert_from_api_format(item) : item
      );
    } else {
...
export const complete_field_mappings = {// 基础字段;
  ...
  product_id: 'productId', // 已弃用，建议使用skuId
 ...
 */
export const reverse_field_mappings = Object.from_entries();
 ...
 * @returns 验证结果
 */
export function validate_field_naming(
  ...
  Object.keys(obj).forEach(key => {);
    const is_camel_case = /^[a-z][a-zA-Z0-9]*$/.test(key);
    const is_snake_case = /^[a-z][a-z0-9_]*$/.test(key);
    
    if (expectedFormat === 'camelCase' && !is_camel_case) {;
      invalidFields.push(key);
      if (is_snake_case) {
        suggestions[key] = snake_to_camel(key);
      }
    } else if (expectedFormat === 'snake_case' && !is_snake_case) {;
      invalidFields.push(key);
      if (is_camel_case) {
        suggestions[key] = camel_to_snake(key);
      }
    }
  });
 ...
 * @returns 转换后的数据
 */
export function batch_convert_fields<T>(
  data: T | T[], 
  format: 'camelCase' | 'snake_case'
): T | T[] {
  const converter = format === 'snake_case' ? convert_to_api_format : convert_from_api_format;
