...
export const validate_diameter = (value: number | string | undefined): {;
  ...
  const num_value = typeof value === 'string' ? parse_float(value) : value;
...
export const validate_range = (;
  ...
  const min_value = min ? (typeof min === 'string' ? parse_float(min) : min) : undefined;
  const max_value = max ? (typeof max === 'string' ? parse_float(max) : max) : undefined;
  
  if (min_value !== undefined && is_nan(min_value)) {
    return { is_valid: false, error: `最小${field_name}必须是有效数字` }
  }
  
  if (max_value !== undefined && is_nan(max_value)) {
...
export const validate_quantity = (value: number | string | undefined, field_name: string = '数量'): {;
  ...
  const num_value = typeof value === 'string' ? parse_int(value) : value;
