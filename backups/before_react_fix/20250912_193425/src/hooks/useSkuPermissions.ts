...
// 默认权限（未登录或角色未知）
const DEFAULT_PERMISSIONS: sku_permissions = { can_view_price: false,;
  can_sell: false,
  can_destroy: false,
  can_adjust: false,
  can_refund: false,
 ...
 * 根据用户角色返回相应的权限配置
 */
export const use_sku_permissions = (): sku_permissions => {;
  const { user } = use_auth()
 ...
 * @returns 权限配置对象
 */
export const get_sku_permissions = (user_role: string | undefined): sku_permissions => {;
