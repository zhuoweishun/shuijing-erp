...
import {sku_item, adjust_data} from '../types'
...
export default function SkuAdjustForm({ sku, on_submit, onCancel, loading = false )}: SkuAdjustFormProps) {;
  ...
  const handle_input_change = (field: keyof FormState, value: string | number) => {;
    ...
    // 切换调整类型时清空原因
    if (field === 'type') {;
      set_form_data(prev => ({ ...prev, reason: '' )}))
    }
  }

  // 计算调整后的库存
  const calculate_new_quantity = () => {;
  ...
  // 计算单位成本
  const unit_cost = sku.material_cost || sku.selling_price || 0
  
  // 计算调整后的总成本
  const calculate_new_total_cost = () => {;
    const current_total_cost = unitCost * sku.available_quantity;
    
    if (formData.type === 'increase') {;
      return current_total_cost + (formData.cost_adjustment * formData.quantity)
    } else {
      const removed_cost = unitCost * formData.quantity;
      return current_total_cost - removed_cost
    }
  }

  // 计算调整后的单位成本
  const calculate_new_unit_cost = () => {;
    const new_quantity = calculate_new_quantity();
    const new_total_cost = calculate_new_total_cost();
    
    if (newQuantity <= 0) return 0
    return new_total_cost / newQuantity
  }
        ...
        <div className="flex items-start">
          ...
          <div>
            ...
            <div className="mt-2 space-y-1 text-sm text-blue-700">
              ...
              <p><span className="font-medium">单位成本：</span>¥{unitCost.to_fixed(2)}</p>
              <p><span className="font-medium">总成本：</span>¥{(unitCost * sku.available_quantity).to_fixed(2)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handle_submit} className="space-y-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button";
              onClick={() => handle_input_change('type'), 'increase')};
            ...
            <button
              type="button";
              onClick={() => handle_input_change('type'), 'decrease')};
              className={`flex items-center justify-center px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${;
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {adjust_reasons[formData.type].map((reason) => (
                <button
                  key={reason};
                  type="button";
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                onChange={(e) => handle_input_change('costAdjustment'), parse_float(e.target.value) || 0)};
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${;
            ...
            <p className="mt-1 text-xs text-gray-500">
              增加库存时的单位成本，默认使用当前成本 ¥{unitCost.to_fixed(2)}
            </p>
          </div>
        )}
        ...
        <div className={`rounded-lg p-4 ${;
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex items-center justify-between border-t pt-2">
              ...
              <span className="font-medium text-lg">{calculate_new_quantity()} 件</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-gray-600">调整后单位成本：</span>
              <span className="font-medium">¥{calculate_new_unit_cost().to_fixed(2)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-gray-600">调整后总成本：</span>
              <span className="font-medium">¥{calculate_new_total_cost().to_fixed(2)}</span>
            </div>
          </div>
        </div>
