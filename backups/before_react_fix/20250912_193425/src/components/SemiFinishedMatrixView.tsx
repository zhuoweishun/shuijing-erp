...
export default function SemiFinishedMatrixView({ data, loading, on_cell_click )}: Props) {
  ...
  const update_filter_position = () => {;
    if (filter_button_ref.current) {
      const rect = filter_button_ref.current.getBoundingClientRect();
      setFilterPosition({
        top: rect.bottom + 8,
        left: rect.left
      )})
  ...
  const matrix_data = useMemo(() => {
    ...
    if (viewMode === 'size') {
      ...
      semi_finished_data.forEach(productTypeData => {;
        product_typeData.specifications.forEach(specData => {);
      ...
      Object.values(matrix).forEach(productRow => {);
        Object.values(product_row).forEach(cell => {;
      ...
      product_names.forEach(product_name => {);
        if (!matrix[product_name]) {
          matrix[product_name] = {}
        }
        
        complete_qualities.forEach(quality => {);
      ...
      Object.values(matrix).forEach(productRow => {);
        Object.values(product_row).forEach(cell => {;
          ...
          // 计算每个尺寸的平均价格
          Object.entries(sizePriceData).forEach(([size_key), data]) => {
  ...
  // 获取所有产品名称用于筛选
  const all_product_names = useMemo(() => {;
  ...
  // 筛选后的产品名称
  const filtered_product_names = useMemo(() => {;
                            ...
                            <div className="p-4">
                              ...
                              <div className="flex justify-between items-center mb-3">
                                <span className="text-sm text-gray-600">
                                  已选择 {tempSelectedProducts.length} / {all_product_names.filter(name => name.toLowerCase().includes(tempSearchKeyword.toLowerCase())).length} 个产品
                                </span>
