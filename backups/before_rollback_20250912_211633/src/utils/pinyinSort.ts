 ...
 * @returns 拼音首字母
 */
export function getPinyinFirstLetter(str: string): string {
...
export function sortByPinyin<T>(items: T[], getNameFn: (item: T) => string): T[] {
  return items.sort((a, b) => {
    ...
    // 获取拼音首字母
    const letterA = getPinyinFirstLetter(nameA)
    const letterB = getPinyinFirstLetter(nameB)
 ...
 * @returns 排序后的原材料数组
 */
export function sortMaterialsByTypeAndPinyin(materials: any[]): any[] {
  return materials.sort((a, b) => {
    // 先按产品类型排序
    const typeOrder = ['LOOSE_BEADS', 'BRACELET', 'ACCESSORIES', 'FINISHED']
    const typeA = typeOrder.indexOf(a.product_type)
    const typeB = typeOrder.indexOf(b.product_type)
    ...
    // 同类型内按拼音排序
    const letterA = getPinyinFirstLetter(a.product_name)
    const letterB = getPinyinFirstLetter(b.product_name)
    
    if (letterA !== letterB) {
      return letterA.localeCompare(letterB)
    }
    
    return a.product_name.localeCompare(b.product_name, 'zh-CN', { numeric: true })
  })
}