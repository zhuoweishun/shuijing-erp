...
export interface financial_record {
  id: string, record_type: 'INCOME' | 'EXPENSE' | 'REFUND' | 'LOSS'
  amount: number, description: string, reference_type: 'PURCHASE' | 'SALE' | 'REFUND' | 'MANUAL'
  reference_id?: string;
category?: string, transaction_date: string;
notes?: string, created_at: string, updated_at: string, user_id: string
}
// 财务统计类型;
...
export interface monthly_financial_summary {
  year: number, month: number, year_month: string, total_income: number, total_expense: number, total_refund: number, total_loss: number, net_profit: number
}
// 财务记录创建请求类型;
...
export interface TransactionRecord {
  ...
  amount: number, description: string, details: string, reference_id: string, reference_type: 'PURCHASE' | 'PRODUCTION' | 'DESTROY' | 'SALE'
...
export interface InventoryStatusData {
  stale_period_months: number, stale_threshold_date: string
  
  // 原材料库存, material_inventory: {
    total_cost: number, stale_cost: number, stale_count: number, total_count: number, stale_ratio: number
  }
  // SKU库存, sku_inventory: {
    total_cost: number, stale_cost: number, stale_count: number, total_count: number, stale_ratio: number
  }
