...
interface CityStats { name: string, count: number },
interface CustomerManagementState {
  customers: Customer[]
  analytics: CustomerAnalytics | null, loading: boolean, error: string | null, search_term: string, selected_type: string, time_period: 'week' | 'month' | 'half_year' | 'year' | 'all'
  sort_by: string, sort_order: 'asc' | 'desc'
  pagination: {
    page: number, limit: number, total: number, total_pages: number
  }
  // 筛选相关状态, filters: {
    customer_code_search: string, name_search: string, phone_search: string, city_filter: string[]
    customer_type: string[]
    total_orders_min: string, total_orders_max: string, total_all_orders_min: string, total_all_orders_max: string, total_purchases_min: string, total_purchases_max: string, first_purchase_start: string, first_purchase_end: string, last_purchase_start: string, last_purchase_end: string
  },
  column_filters: {
    visible: boolean, column: string, type: 'search' | 'multiSelect' | 'sortAndRange' | 'sort'
  },
  filter_panel_position: { x: number, y: number },
  city_search_term: string, city_stats: CityStats[]
  show_create_modal: boolean, show_detail_modal: boolean, show_reverse_sale_modal: boolean, show_refund_modal: boolean, selected_customer: Customer | null
},
export default function CustomerManagement() {
  const { is_authenticated } = use_auth()
  const [state, set_state] = useState<CustomerManagementState>({
    customers: [],
    analytics: null,
    loading: false, // 初始设置为false，避免闪烁, error: null,
    search_term: '',
    selected_type: '',
    time_period: 'all',
    sort_by: 'created_at',
...
return (
       <div className="p-4">
         ...
         {column_config.type === 'search' && render_search_filter(column}, {column_config.type === 'multiSelect' && render_multi_select_filter(column}, {column_config.type === 'range' && render_range_filter(column}, {column_config.type === 'sortAndRange' && render_sort_and_range_filter(column}, {column_config.type === 'daterange' && render_date_range_filter(column)
...
case 'phone': return state.filters.phone_search, default: return ''
       }
     },
     ...
     return (
      ...
      {/* 筛选功能 */}
         <div>
           ...
           {column === 'city' && render_city_filter(}, {column === 'customer_type' && render_customer_type_filter()
         </div>
       </div>
     )
   }
...
type="text"
           ...
           className="btn-primary"
                   ...
                   className="btn-primary"
                   {cityData.name}, {cityData.count > 0 && (
...
type="checkbox");
      ...
      {/* 为退货行为列添加占位空间，保持对齐 */}, {categoryName === '退货行为' && types.length < 4 && (;
...
type="number"
             placeholder="最小值"
             value={min));
             onChange={(e) => setrange_values(e.target.value, max},
             className="btn-primary"
             type="number"
             placeholder="最大值"
             value={max)
             onChange={(e) => setrange_values(min, e.target.value)},
             className="btn-primary"
   }
   // 渲染排序和范围筛选;
...
setstate(prev => ({ 
        ...prev, 
        loading: prev.customers.length === 0, // 只有在没有客户数据时才显示loading, error: null 
      ))))
      console.log('📊 当前状态:', {
...
console.log('✅ 数据获取成功:', {
        ...
        setstate(prev => ({
          ...
          city_stats: data.city_stats || [], // 更新城市统计数据, loading: false // 确保数据加载完成后关闭loading
        )))
      } else {
...
const is_no_data_error = error.message?.includes('客户不存在') || error.message?.includes('404')
      ...
      setstate(prev => ({
        ...prev,
        loading: false,
        customers: [], // 设置空数组, pagination: { page: 1, limit: 20, total: 0, total_pages: 0 ), // 重置分页, city_stats: [], // 重置城市统计数据)
...
value={state.pagination.limit))
                ...
                className="btn-primary"
          {/* 手机端分页按钮 - 只有在多页时才显示 */}, {state.pagination.total_pages > 1 && (
...
value={state.pagination.limit)
                onChange={(e) => handle_limit_change(Number(e.target.value)},
                className="btn-primary"
          {/* 分页按钮 - 只有在多页时才显示 */}, {state.pagination.total_pages > 1 && (
            <div>
...
if (amount === undefined || amount === null || amount === '') {
    ...
    // 转换为数字;
const num_amount = typeof amount === 'string' ? parseFloat(amount : Number(amount)
    
    // 检查是否为有效数字;
if (isNaN(num_amount) || !is_finite(num_amount)) {
      return '¥0.00'
    },
...
onClick={() => handle_refund(customer)
            ...
            {/* 分页 */}, {render_pagination(}
          </>
        )}</div>
      
      {/* 反向销售录入模态框 */}, {state.selected_customer && (
        <ReverseSaleModal;
customer={state.selected_customer},
      ...
      {/* 客户详情模态框 */}, {state.selected_customer && (
          <CustomerDetailModal;
customer={state.selected_customer},
...
allCustomers
      ...
      {/* 客户退货模态框 */}, {state.selected_customer && (
        <CustomerRefundModal;
customer={state.selected_customer},
...
is_open={state.show_create_modal},
      ...
      {/* 筛选面板 */}, {state.column_filters.visible && (
        <Portal>
          <div;
