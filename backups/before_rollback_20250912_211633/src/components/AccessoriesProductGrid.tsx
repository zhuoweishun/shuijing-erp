...
interface AccessoryProduct {
  purchase_id: string
  purchase_code?: string, product_name: string
  ...
  total_price?: number
  supplier_name?: string, purchase_date: string, remaining_quantity: number, is_low_stock: boolean
  key?: string
}
...
export default function AccessoriesProductGrid({
  ...
  const fetchAccessoryProducts = async (pageNum: number = 1, append: boolean = false) => {
    try {
      ...
      const params = {
        ...
        product_types: ['ACCESSORIES'], // 只查询配件类型, quality: (selectedQuality, as 'AA' | 'A' | 'AB' | 'B' | 'C') || undefined,
  ...
  const extractAccessoryProducts = (hierarchyData: any[]): AccessoryProduct[] => {
    ...
    // 对配饰产品按产品名称进行A-Z排序
    return sortByPinyin(products, (product: AccessoryProduct) => product.product_name)
  }
