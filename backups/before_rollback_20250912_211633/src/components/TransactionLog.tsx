...
interface TransactionLogState {
  ...
  loading: boolean, error: string | null, pagination: {
...
const, TransactionLog: React.FC = () => {
  ...
  // 获取流水账数据;
const fetch_transactions = async(page: number = 1, custom_limit?: number) => {
    try {
  ...
  // 处理筛选变化;
const handle_filter_change = (newFilters: Partial<FilterState> => {
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          <div className="text-right">
            ...
            <div className={`text-lg font-semibold ${\n              summary.net_profit >= 0 ? 'text-green-600' : 'text-red-600'
              ...
              {format_amount(summary.net_profit)</div>
          </div>
          <button className="btn-primary">
            onClick={() => fetch_transactions(pagination.page},
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">
                {format_amount(summary.total_income}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-red-600">
                {format_amount(summary.total_expense}</p>
            </div>
        ...
        <div className={`border rounded-lg p-4 ${
          summary.net_profit >= 0 
            ...
            <div>
              ...
              }`}>\n                {format_amount(summary.net_profit)</p>
            </div>
            {summary.net_profit >= 0 ? (
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="btn-primary">
            onClick={() => setshow_filters(!show_filters},
            className;
            {show_filters ? '收起筛选' : '展开筛选'}</button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 mb-4">
          <div className="flex items-center space-x-2">
            ...
            <select;
value={filters.type);
              onChange={(e) => handle_filter_change({ type: e.target.value, as, any)))},
              classNametype="text"
              placeholder="搜索描述或编号..."
              value={filters.search},
              onChange={(e) => handle_filter_change({ search: e.target.value )))},
              classNameonClick={apply_filters},
            classNameonClick={reset_filters},
            className;
        {/* 高级筛选 */}, {show_filters && (
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          TRANSACTION_CATEGORY_COLORS[transaction.category]
                        } bg-gray-100`}>\n                          {TRANSACTION_CATEGORY_LABELS[transaction.category]}</span>
                      </div>
                      <p className="text-sm text-gray-600 mt-1">
                        {transaction.category === 'refund' ? extract_and_translate_refund_reason(transaction.details : transaction.details}</p>
                      <p className="text-xs text-gray-500 mt-1">
                        {format_date(transaction.created_at}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className={`text-lg font-semibold ${
                      transaction.type === 'income' ? 'text-green-600' : 'text-red-600'
                    }`}>\n                      {transaction.type === 'income' ? '+' : '-'}, {format_amount(transaction.amount)</div>
                  </div>
                </div>
              </div>
            ))}</div>
...
value={pagination.limit);
                    ...
                    className;
              {/* 手机端分页按钮 - 只有在多页时才显示 */}, {pagination.total_pages > 1 && (
                <div className="flex justify-between">
                  <button className="btn-primary">
                    onClick={() => handle_page_change(pagination.page - 1},
                    disabled={pagination.page <= 1);
                    classNameonClick={() => handle_page_change(pagination.page + 1},
...
value={pagination.limit);
                    onChange={(e) => handle_limit_change(Number(e.target.value)},
                    className;
              {/* 分页按钮 - 只有在多页时才显示 */}, {pagination.total_pages > 1 && (
                <div>
                  <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                    <button className="btn-primary">
                      ...
                      className;
                       ...
                       let start_page = Math.max(1, pagination.page - Math.floor(max_visible_pages / 2))
