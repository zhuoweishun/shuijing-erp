...
interface FinancialReportData {
  period: string, total_income: number, total_expense: number, net_profit: number, incomeByCategory: { category: string; amount: number }[]
...
export default function FinancialReports({ className = '' }: FinancialReportsProps) {
  ...
  const load_report_data = async () => {
    try {
      setloading(true)
      
      // 获取财务统计数据;
const response = await financialApi.get_statistics({
        start_date: dateRange.start_date,
        end_date: dateRange.end_date,
        // groupBy: reportType === 'monthly' ? 'day' : 'month' // 暂时注释掉不支持的参数
      })
...
const csv_content = [
      ...
      [''],
      ['收入分类'],
      ...reportData.incomeByCategory.map(item => [item.category, format_currency(item.amount)]), 
      [''],
      ['支出分类'],
      ...reportData.expenseByCategory.map(item => [item.category, format_currency(item.amount)])
    ]
    
    const csv_string = csv_content.map(row => row.join(',')).join('\n')
    const blob = new Blob([csv_string], { type: 'text/csvcharset=utf-8;' })
      ...
      <div className="flex flex-col, sm:flex-row, sm:items-center, sm:justify-between mb-6">
        ...
        <div className="flex flex-col, sm:flex-row gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <input;
typevalue={dateRange.start_date};
              onChange={(e) => setDateRange(prev => ({ ...prev, start_date: e.target.value }))};
              classNamevalue={dateRange.end_date};
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-700">
                    {format_currency(reportData.total_income}</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-700">
                    {format_currency(reportData.total_expense}</p>
                </div>
                ...
                {reportData.incomeByCategory.length > 0 ? (
                  reportData.incomeByCategory.map((item, index) => (
                    <div key={index} className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                      <span className="text-sm font-medium text-gray-700">{item.category || '未分类'}</span>
                      <span className="text-sm font-bold text-green-600">
                        {format_currency(item.amount}</span>
                    </div>
                  ))
                 : (
                    ...
                    {reportData.monthlyTrend.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-green-600">
                          {format_currency(item.income}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-red-600">
                          {format_currency(item.expense}</td>
                        <td className={`px-6 py-4 whitespace-nowrap text-sm font-medium ${
                          item.profit >= 0 ? 'text-blue-600' : 'text-orange-600'
                        }`}>
                          {format_currency(item.profit}</td>
                      </tr>
                    ))}</tbody>
                </table>
              </div>
