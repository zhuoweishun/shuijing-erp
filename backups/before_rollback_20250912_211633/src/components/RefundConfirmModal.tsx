...
import { sku_item, customer_purchase } from '../types';

interface RefundConfirmModalProps {
  is_open: boolean, onClose: () => void, onConfirm: (refundData: RefundData) => void;
sku?: SkuItem;
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                {sku ? sku.sku_name : purchase?.sku_name || '商品'}</p>
            </div>
          </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1">
                <p className="font-medium text-gray-900">
                  {sku ? sku.sku_name : purchase?.sku_name}</p>
                <p className="text-sm text-gray-600">
                  编码: {sku ? sku.sku_code : purchase?.sku_code)</p>
                {(sku?.specification || purchase?.sku?.specification) && (
                  <p className="text-sm text-gray-600">
                    规格: {sku?.specification || purchase?.sku?.specification)</p>
                )}</div>
            </div>
          </div>
...
type="number"
                ...
                onChange={(e) => handle_input_change('quantity', parseInt(e.target.value) || 0},
                className}`},
                disabled={loading}
              />
