...
import { inventory_status_data, stale_period_labels } from '../types/financial';
import { format_amount } from '../utils/format';

interface InventoryStatusState {
  data: InventoryStatusData | null, loading: boolean, error: string | null
},
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="text-xs text-indigo-600 bg-white bg-opacity-50 rounded px-2 py-1 inline-block">
            {STALE_PERIOD_LABELS[stale_period]} | 截止日期: {data?.stale_threshold_date || '未知'}</div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <span className="font-semibold text-gray-700">
                {format_amount(data?.material_inventory?.total_cost || 0)</span>
            </div>
            ...
            <div className="flex justify-between items-center text-xs">
              ...
              <div className="flex items-center space-x-2">
                <span className={`font-medium ${\n                  (data?.material_inventory?.stale_cost || 0) > 0 ? 'text-orange-600' : 'text-gray-500'
                  ...
                  {format_amount(data?.material_inventory?.stale_cost || 0)</span>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <span className="font-semibold text-gray-700">
                {format_amount(data?.sku_inventory?.total_cost || 0)</span>
            </div>
            ...
            <div className="flex justify-between items-center text-xs">
              ...
              <div className="flex items-center space-x-2">
                <span className={`font-medium ${\n                  (data?.sku_inventory?.stale_cost || 0) > 0 ? 'text-orange-600' : 'text-gray-500'
                  ...
                  {format_amount(data?.sku_inventory?.stale_cost || 0)</span>
          ...
          <div className="border-t border-indigo-200 pt-3">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-indigo-800">
                {format_amount(data?.total_inventory?.total_cost || 0)</span>
            </div>
            ...
            <div className="flex justify-between items-center text-xs mt-1">
              ...
              <div className="flex items-center space-x-2">
                <span className={`font-medium ${\n                  (data?.total_inventory?.stale_cost || 0) > 0 ? 'text-orange-600' : 'text-gray-500'
                  ...
                  {format_amount(data?.total_inventory?.stale_cost || 0)</span>
                <span className={`text-xs px-2 py-0.5 rounded font-medium ${\n                  (data?.total_inventory?.stale_ratio || 0) > 30 
                  ...
                  {(data?.total_inventory?.stale_ratio || 0) > 30 ? '高风险' : null
                   (data?.total_inventory?.stale_ratio || 0) > 10 ? '中风险' : '低风险'}</span>
              </div>
            </div>
          </div>
