...
if (!formData.name.trim()) {
      newErrors.name = '客户姓名不能为空'} else if (formData.name.trim().length < 2) {
      newErrors.name = '客户姓名至少2个字符'} else if (formData.name.trim().length > 20) {
      newErrors.name = '客户姓名不能超过20个字符'}
    // 手机号验证;
if (!formData.phone.trim()) {
      newErrors.phone = '手机号不能为空'} else {
...
if (!formData.address.trim()) {
      newErrors.address = '地址不能为空'} else if (formData.address.trim().length > 100) {
...
if (result.error?.code === 'PHONE_ALREADY_EXISTS') {
          setErrors({ phone: '该手机号已被使用' )) else {
          toast.error(result.message || '创建客户失败'}
      }
    ) catch (error) {
      console.error('创建客户失败:', error )
      toast.error('网络错误，请稍后重试') finally {
