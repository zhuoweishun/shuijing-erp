...
interface ProductTypeTabProps {
  selectedType: ProductType, onTypeChange: (type: ProductType, unit_type: UnitType) => void
  className?: string
}

export default function ProductTypeTab({ selectedType, onTypeChange, className = '' }: ProductTypeTabProps) {
  return (
      ...
      <div className="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-gray-600">
            {tabOptions.find(option => option.id === selectedType)?.description}
          </span>
        </div>
      </div>
    </div>
