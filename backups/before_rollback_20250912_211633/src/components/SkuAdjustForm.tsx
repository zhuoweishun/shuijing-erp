...
interface SkuAdjustFormProps {
  sku: SkuItem, onSubmit: (data: AdjustData) => void, onCancel: () => void
  loading?: boolean
}

interface FormState {
  type: 'increase' | 'decrease'
  quantity: number, reason: string, cost_adjustment: number
}
...
export default function SkuAdjustForm({ sku, onSubmit, onCancel, loading = false }: SkuAdjustFormProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.quantity || formData.quantity <= 0) {
      ...
      newErrors.quantity = `减少数量不能超过可售数量(${sku.available_quantity}件)`
    }
        ...
        <div className="flex items-start">
          ...
          <div>
            ...
            <div className="mt-2 space-y-1 text-sm text-blue-700">
              ...
              <p><span className="font-medium">可售数量：</span>{sku.available_quantity} 件</p>
