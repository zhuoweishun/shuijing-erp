...
interface SemiFinishedMatrixData {
  product_type: string, total_quantity: number, total_variants: number, has_low_stock: boolean, specifications: SpecificationData[]
}

interface SpecificationData {
  specification_value: number, specification_unit: string, total_quantity: number, total_variants: number, has_low_stock: boolean, qualities: QualityData[]
}

interface QualityData {
  quality: string, remaining_quantity: number, is_low_stock: boolean, price_per_unit: number | null, price_per_gram: number | null, batch_count: number, batches: BatchData[]
}

interface BatchData {
  purchase_id: number
  purchase_code?: string, product_name: string, product_type: string, purchase_date: string, supplier_name: string, original_quantity: number, used_quantity: number, remaining_quantity: number, price_per_unit: number | null, price_per_bead: number | null, price_per_gram: number | null
  photos?: string[]
...
export default function SemiFinishedMatrixView({ data, loading, onCellClick }: Props) {
  const { user } = useAuth()
  ...
  const matrixData = useMemo(() => {
    ...
    if (viewMode === 'size') {
      // 按尺寸展示的矩阵
      const matrix: { [productName: string]: { [size: string]: MatrixCell } } = {},
        ...
        productTypeData.specifications.forEach(specData => {
          ...
          specData.qualities.forEach(qualityData => {
            qualityData.batches.forEach(batch => {
              ...
              if (!matrix[productName]) {
                matrix[productName] = {}
              }
              
              if (!matrix[productName][size]) {
                matrix[productName][size] = {
              ...
              const cell = matrix[productName][size]
              cell.totalQuantity += batch.remaining_quantity
              cell.batches.push(batch)
              
              // 计算品相分布
              const quality = formatQuality(qualityData.quality)
              cell.qualityDistribution[quality] = (cell.qualityDistribution[quality] || 0) + batch.remaining_quantity
              
              // 检查低库存状态
              if (qualityData.is_low_stock) {
                cell.isLowStock = true
              }
            })
          })
        })
          ...
          cell.batches.forEach(batch => {
            ...
            if (price > 0) {
              ...
              if (!qualityPriceData[batchQuality]) {
                qualityPriceData[batchQuality] = { totalPrice: 0, totalQuantity: 0 }
              }
              
              qualityPriceData[batchQuality].totalPrice += price * batch.remaining_quantity
              qualityPriceData[batchQuality].totalQuantity += batch.remaining_quantity
            }
          })
          
          // 计算每个品相的平均价格
          Object.entries(qualityPriceData).forEach(([quality, data]) => {
            cell.qualityPrices[quality] = data.totalQuantity > 0 ? data.totalPrice / data.totalQuantity : 0
          })
          
          // 计算总体平均价格（加权平均）
          const totalPrice = Object.values(qualityPriceData).reduce((sum, data) => sum + data.totalPrice, 0)
          const totalQuantity = Object.values(qualityPriceData).reduce((sum, data) => sum + data.totalQuantity, 0)
          cell.avgPrice = totalQuantity > 0 ? totalPrice / totalQuantity : 0
        })
      })
      ...
      // 按品相展示的矩阵
      const matrix: { [productName: string]: { [quality: string]: MatrixCell } } = {}
      // 定义完整的品相选项，确保所有品相都显示
        ...
        productTypeData.specifications.forEach(specData => {
          specData.qualities.forEach(qualityData => {
            ...
            qualityData.batches.forEach(batch => {
              ...
              if (!matrix[productName][quality]) {
                matrix[productName][quality] = {
                  productType: productTypeData.product_type,
                  productName: productName,
              ...
              const cell = matrix[productName][quality]
              cell.totalQuantity += batch.remaining_quantity
              cell.batches.push(batch)
              
              // 在品相视图中，qualityDistribution显示尺寸分布
              const sizeKey = `${specData.specification_value}mm`
              cell.qualityDistribution[sizeKey] = (cell.qualityDistribution[sizeKey] || 0) + batch.remaining_quantity
      ...
      productNames.forEach(productName => {
        ...
        completeQualities.forEach(quality => {
          if (!matrix[productName][quality]) {
            // 创建空的品相单元格
            matrix[productName][quality] = {
              productType: 'LOOSE_BEADS', // 默认类型，实际会被数据覆盖
              productName: productName,
              size: 0,
              quality: quality,
              totalQuantity: 0,
      ...
      Object.values(matrix).forEach(productRow => {
        Object.values(productRow).forEach(cell => {
            ...
            if (price > 0) {
              ...
              if (batchSpec) {
                ...
                if (!sizePriceData[sizeKey]) {
                  sizePriceData[sizeKey] = { totalPrice: 0, totalQuantity: 0 }
                },
                sizePriceData[sizeKey].totalPrice += price * batch.remaining_quantity
                sizePriceData[sizeKey].totalQuantity += batch.remaining_quantity
              }
            }
          })
          ...
          Object.entries(sizePriceData).forEach(([sizeKey, data]) => {
            cell.qualityPrices[sizeKey] = data.totalQuantity > 0 ? data.totalPrice / data.totalQuantity : 0
          })
      ...
      return {
        matrix,
        allSizes: [],
        allQualities: completeQualities, // 使用预定义的完整品相顺序, productNames: Array.from(productNames).sort((a, b) => a.localeCompare(b, 'zh-CN'))
      }
    }
  }, [data, viewMode])
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                onClick={() => setViewMode(viewMode === 'size' ? 'quality' : 'size')},
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">
              {Object.entries(matrixData.matrix)
                .filter(([productName]) => filteredProductNames.includes(productName))
                .reduce((total, [, productRow]) => 
                  total + Object.values(productRow).reduce((sum, cell) => sum + cell.totalQuantity, 0), 0
                )}
            </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-red-600">
              {Object.entries(matrixData.matrix)
                .filter(([productName]) => filteredProductNames.includes(productName))
                .reduce((total, [, productRow]) => 
                  total + Object.values(productRow).filter(cell => cell.isLowStock).length, 0
                )}
            </div>
              ...
              <tr>
                            ...
                            <div className="p-4">
                              ...
                              <div className="max-h-60 overflow-y-auto mb-3">
                                {allProductNames
                                  ...
                                  .map(productName => (
                                  <label key={productName} className="flex items-center space-x-2 py-1 hover:bg-gray-50 rounded px-2 cursor-pointer">
                                    <input
                                      type="checkbox"
                                      checked={tempSelectedProducts.includes(productName)},
                                      onChange={() => handleProductToggle(productName)},
                                      className="rounded border-gray-300 text-crystal-600 focus:ring-crystal-500"
                                    />
                                    <span className="text-sm text-gray-700 truncate" title={productName}>
                                      {productName}
                                    </span>
                                  </label>
                                ))}
                ...
                {viewMode === 'size' ? (
                  matrixData.allSizes.map(size => (
                    <th key={size} className="px-3 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider min-w-32">
                      {size}mm
                    </th>
                  ))
                ) : (
                  matrixData.allQualities.map(quality => (
                    <th key={quality} className="px-3 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider min-w-32">
                      {quality}级
                    </th>
                  ))
                )}
              ...
              {filteredProductNames.map(productName => (
                <tr key={productName} className="hover:bg-gray-50">
                  <td className="px-4 py-4 text-sm font-medium text-gray-900 sticky left-0 bg-white z-10 border-r border-gray-200">
                    <div className="max-w-32 truncate" title={productName}>
                      {productName}
                    </div>
                  </td>
                  {viewMode === 'size' ? (
                    matrixData.allSizes.map(size => {
                      const cell = matrixData.matrix[productName]?.[size]
                                ...
                                {Object.entries(cell.qualityDistribution)
                                  ...
                                  .map(([quality, quantity]) => (
                                    <div key={quality} className="flex justify-center items-center space-x-1">
                                      ...
                                      <span className="text-xs">{quality}: {quantity}{cell.productType === 'LOOSE_BEADS' ? '颗' : cell.productType === 'BRACELET' ? '颗' : cell.productType === 'ACCESSORIES' ? '片' : '件'}</span>
                                    </div>
                                  ))
                                }
                              </div>
                    ...
                    matrixData.allQualities.map(quality => {
                      const cell = matrixData.matrix[productName]?.[quality]
                      
                      if (!cell) {
                        return (
                          <td key={quality} className="px-3 py-4 text-center">
                            <div className="text-gray-400 text-xs">-</div>
                          </td>
                        )
                      }
                            ...
                            {user?.role === 'BOSS' && Object.keys(cell.qualityDistribution).length > 0 && (
                              <div className="text-xs text-gray-600 mt-1 space-y-0.5">
                                {Object.entries(cell.qualityDistribution)
                                  ...
                                  .sort(([a], [b]) => {
                                    // 按尺寸排序
                                    const sizeA = parseFloat(a.replace('mm', ''));
                                    const sizeB = parseFloat(b.replace('mm', ''));
                                    return sizeA - sizeB;
                                  })
                                  .map(([sizeKey, quantity]) => (
                                    <div key={sizeKey} className="flex justify-center items-center">
                                      <span className="text-xs">{sizeKey}: {quantity}{cell.productType === 'LOOSE_BEADS' ? '颗' : cell.productType === 'BRACELET' ? '颗' : cell.productType === 'ACCESSORIES' ? '片' : '件'}</span>
                                    </div>
                                  ))
                                }
                              </div>
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900">
                  {selectedCell.productName} - {viewMode === 'size' ? `${selectedCell.size}mm` : `${selectedCell.quality}级`}
                </h3>
                <button
                  onClick={() => setSelectedCell(null)},
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <div className="text-sm text-gray-500">总库存</div>
                    <div className="text-xl font-bold text-gray-900">{selectedCell.totalQuantity} {
                  ...
                  {user?.role === 'BOSS' && (
                        <div>
                          ...
                          {selectedCell.avgPrice > 0 ? (
                            <div className="text-xl font-bold text-gray-900">¥{selectedCell.avgPrice.toFixed(2)}</div>
                          ) : (
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {Object.entries(selectedCell.qualityDistribution).map(([key, quantity]) => (
                      <div key={key} className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          {viewMode === 'size' ? (
                            <>
                              <div className={`w-3 h-3 rounded-full ${getQualityColor(key)}`}></div>
                              <span className="text-sm text-gray-700">{key}级</span>
                            </>
                          ) : (
                            <>
