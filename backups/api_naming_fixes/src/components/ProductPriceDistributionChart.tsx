...
export default function ProductPriceDistributionChart() {
  ...
  const [loading, setLoading] = use_state(true)
  const [data, setData] = use_state<PriceDistributionData | null>(null)
  const [material_type, setMaterial_type] = use_state<string>('LOOSE_BEADS')
  const [priceType, setPriceType] = use_state<string>('unit_price')

  // 获取价格分布数据
  const fetch_price_distribution = async () => {set_loading(true);
    try {
      const response = await inventoryApi.get_price_distribution({;
      ...
      if (response.success && response.data) {
        ...
        set_data(response.data as PriceDistributionData)
      } else {
  ...
  const format_price = (price: number | string | null | undefined) => {;
    ...
    // 检查是否为有效数字
    if (is_nan(num_price) || !is_finite(num_price)) return '***'
    
    return `¥${num_price.to_fixed(2)}`
  }
    ...
    return(
      <div className={isMobile ? 'p-mobile' : 'p-6'}>
        <div className={`flex items-center ${isMobile ? 'flex-col space-y-3' : 'justify-between'} mb-6`}>
          ...
          <button
            onClick={fetch_price_distribution};
  ...
  // 准备图表数据
  const chart_data = price_type === 'unit_price' 
    ? // 单价区间分布数据
    ...
    : // 总价分布数据
      data.top_price_products?.map((item), index) => {
        const raw_value = item.total_price;
        const num_value = typeof raw_value === 'string' ? parse_float(raw_value) : raw_value;
        const value = (num_value && !is_nan(num_value) && is_finite(num_value)) ? num_value : 0;
            ...
            <Pie
              ...
              label={isMobile ? false : ({ name, value, percent }) => ;
                priceType === 'unit_price' 
                  ? `${name}: ${value}个 (${((percent || 0) * 100).to_fixed(1)}%)`
                  : `${name}: ${format_price(value)} (${((percent || 0) * 100).to_fixed(1)}%)`
              }
