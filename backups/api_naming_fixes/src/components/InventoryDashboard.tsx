...
import material_distribution_pie_chart from './product_distribution_pie_chart'
import inventory_consumption_chart from './inventory_consumption_chart'
import product_price_distribution_chart from './product_price_distribution_chart'
...
export default function InventoryDashboard() {
  ...
  const [statistics, setStatistics] = use_state<InventoryStatistics | null>(null)
  const [refreshing, setRefreshing] = use_state(false)

  // 获取统计数据
  const fetch_statistics = async () => {try {;
      set_loading(true)
      ...
      // 安全地访问错误信息
      const error_message = error?.response?.data?.message || error?.message || '获取统计数据失败';
