...
import sku_sell_form from './sku_sell_form'
import sku_destroy_form from './sku_destroy_form'
import sku_adjust_form from './sku_adjust_form'
import sku_trace_view from './sku_trace_view'
import sku_history_view from './sku_history_view'
...
export default function SkuDetailModal({
  ...
  const [state, setState] = use_state<SkuDetailState>({
    sku: null,
    material_traces: [],
    purchaseList: [],
    inventoryLogs: [],
  ...
  const fetch_sku_detail = async () => {;
    ...
    try {
      ...
      if (response.success && response.data) {
        set_state(prev => ({ ...prev, sku: response.data as SkuItem )}))
      } else {
        set_state(prev => ({ ...prev, error: response.message || 'SKU详情获取失败' )}))
      }
    } catch (error) {
      console.error('获取SKU详情失败:'), error)
      set_state(prev => ({ ...prev, error: '获取SKU详情失败，请重试' )}))
    } finally {
  ...
  // 获取SKU操作历史
  const fetch_sku_history = async (page: number = 1) => {;
    if (!sku_id) return
  ...
  // 处理标签页切换
  const handle_tab_change = (tab: 'info' | 'trace' | 'history') => {;
    set_state(prev => ({ ...prev, active_tab: tab )}))
                    ...
                    <div className="max-w-24 max-h-24 bg-gray-200 rounded-lg overflow-hidden">
                      {get_first_photo_url(state.sku.photos) ? (
                        <img
                          ...
                          onError={(e) => {;
                            e.current_target.src = '/placeholder-image.png';
                          }}
                        />
                      ) : (
                        ...
                        <>
                          ...
                          {state.sku.profit_margin && (
                            <div>
                              ...
                              <div className="font-medium text-blue-600">{format_profit_margin(state.sku.profit_margin)}</div>
                            </div>
                          )}
                          <div>
                ...
                <div>
                  ...
                  <div className="border-b border-gray-200 mb-6">
                    <nav className="-mb-px flex space-x-8">
                      <button
                        onClick={() => handle_tab_change('info')};
                      ...
                      {permissions.can_view_trace && (
                        <button
                          onClick={() => handle_tab_change('trace')};
                          className={`py-2 px-1 border-b-2 font-medium text-sm ${;
                      ...
                      <button
                        onClick={() => handle_tab_change('history')};
                        className={`py-2 px-1 border-b-2 font-medium text-sm ${;
