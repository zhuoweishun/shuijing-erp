...
export default function FinishedProductGrid({search_term,
  ...
  const { user } = use_auth()
  const [products, setProducts] = use_state<FinishedProduct[]>([])
  const [loading, setLoading] = use_state(false)
  const [page, setPage] = use_state(1)
  const [hasMore, setHasMore] = use_state(true)
  const [selectedProduct, setSelectedProduct] = use_state<FinishedProduct | null>(null)
  ...
  const fetch_finished_products = async(page_num: number = 1), append: boolean = false) => {try {;
      ...
      if (response.success && response.data) {
        ...
        const finished_products = response_data.products || [];
        ...
        // set_total(response_data.pagination?.total || finished_products.length) // total变量已移除
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {user?.role === 'BOSS' && selectedProduct.price_per_unit && (
                    <div>
                      ...
                      <div className="text-xl font-bold text-gray-900">¥{selectedProduct.price_per_unit.to_fixed(2)}</div>
                    </div>
                  )}
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      {user?.role === 'BOSS' && selectedProduct.price_per_unit && (
                        <div>
                          ...
                          <span className="ml-1">¥{selectedProduct.price_per_unit.to_fixed(2)}</span>
                        </div>
                      )}
                    </div>
                  </div>
