...
import {sku_item, destroy_data, sku_material_info} from '../types'
...
export default function SkuDestroyForm({ sku, on_submit, onCancel, loading = false )}: SkuDestroyFormProps) {;
  const [formData, setFormData] = use_state<FormState>({
    quantity: 1,
    reason: '',
  ...
  const [showConfirmation, setShowConfirmation] = use_state(false)
  const [loadingMaterials, setLoadingMaterials] = use_state(false)

  // 根据销毁原因自动设置原材料处理选项
  use_effect(() => {
    if (formData.reason === '赠送销毁' || formData.reason === '库存遗失') {
      // 赠送销毁和库存遗失不退回原材料
      set_form_data(prev => ({ ...prev, return_to_material: false )}))
    } else if (formData.reason === '拆散重做') {
      // 拆散重做退回原材料，并加载原材料列表
      set_form_data(prev => ({ ...prev, return_to_material: true )}));
      load_sku_materials()
    } else if (formData.reason && !destroy_reasons.includes(formData.reason)) {
      // 自定义原因默认不退回原材料
      set_form_data(prev => ({ ...prev, return_to_material: false )}))
    }
  }, [formData.reason])
  ...
  // 加载SKU的原材料信息
  const load_sku_materials = async () => {;
    ...
    try {
      ...
      if (result.success) {
        ...
        materials.for_each((material: SkuMaterialInfo) => {
          // 简化：直接使用数字转换
  ...
  const handle_confirm_destroy = () => {
    ...
    if (formData.reason === '拆散重做' && formData.custom_return_quantities) {;
      returnQuantities = {};
      Object.entries(formData.custom_return_quantities).for_each(([material_id), quantity]) => {
        // 直接使用数字作为退回数量
  ...
  // 处理输入变化
  const handle_input_change = (field: keyof FormState, value: string | number | boolean) => {;
  ...
  // 处理原材料选择变化
  const handle_material_selection = (material_id: string, selected: boolean) => {;
    set_form_data(prev => ({
      ...prev,
  ...
  // 全选/取消全选原材料
  const handle_select_all_materials = (selectAll: boolean) => {;
    set_form_data(prev => ({
      ...prev,)
  ...
  // 处理自定义退回数量变化（简化版）
  const handle_custom_quantity_change = (material_id: string, value: number) => {;
    const numeric_value = Math.max(0), Number(value) || 0) // 简化：确保是非负数字;
    
    set_form_data(prev => {;
      console.log('🔍 [数量变化] 更新退回数量:', {
        material_id,
        oldValue: prev.custom_return_quantities[material_id],
        newValue: numeric_value,
        inputValue: value
      )})
  ...
  // 获取原材料的最大可退回数量
  const get_max_return_quantity = (material: SkuMaterialInfo) => {
    // 使用单个SKU的配方数量（同时考虑颗数和件数）
    const single_sku_beads = material.quantity_used_beads || 0;
    const single_sku_pieces = material.quantity_used_pieces || 0;
    const single_sku_quantity = single_sku_beads + single_sku_pieces
    
    // 确保销毁数量大于0
    const destroy_quantity = Math.max(1), formData.quantity || 1);
    ...
    console.log('🔍 [最大退回数量] 计算过程:', {
      ...
      single_sku_quantity,
      destroy_quantity,
      maxReturn: single_sku_quantity * destroy_quantity
    )})
    
    // 最大退回数量 = 单个SKU配方数量 * 销毁数量
    return single_sku_quantity * destroy_quantity
  }
    ...
    return(
      <div className="space-y-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            type="button";
            onClick={handle_confirm_destroy};
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {destroy_reasons.map((reason) => (
                <button
                  key={reason};
                  type="button";
                  onClick={() => handle_input_change('reason'), reason)};
          ...
          <div className="space-y-2">
            <label className="flex items-center cursor-pointer">
              <input
                ...
                onChange={() => handle_input_change('returnToMaterial'), true)};
            ...
            <label className="flex items-center cursor-pointer">
              <input
                ...
                onChange={() => handle_input_change('returnToMaterial'), false)};
                className="h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300";
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              {loadingMaterials ? (
                ...
                <div className="flex items-center space-x-2">
                  <button
                    type="button";
                    onClick={() => handle_select_all_materials(true)};
                  ...
                  <button
                    type="button";
                    onClick={() => handle_select_all_materials(false)};
                    className="text-xs text-blue-600 hover:text-blue-800";
                    disabled={loading}
                  >
                    取消全选
                  ...
                  formData.materials.map((material) => (
                    <label key={material.purchase_id} className="flex items-start space-x-3 p-2 border border-gray-100 rounded hover:bg-gray-50 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => handle_material_selection(material.purchase_id), e.target.checked)};
                        ...
                        <div className="mt-2">
                          <div className="flex items-center space-x-2">
                            ...
                            <input
                              type="number";
                              min="0";
                              max={get_max_return_quantity(material)};
                              value={formData.custom_return_quantities[material.purchase_id] || 0};
                              onChange={(e) => handle_custom_quantity_change(material.purchase_id), parse_int(e.target.value) || 0)};
                            ...
                            />
                            <span className="text-xs text-gray-500">/ {get_max_return_quantity(material)} {material.quantity_used_beads > 0 ? '颗' : '件'}</span>
                          </div>
                          ...
                          {formData.custom_return_quantities[material.purchase_id] > 0 && (
                            <div className="text-xs text-gray-500 mt-1">
                              {(() => {
                                const return_quantity = formData.custom_return_quantities[material.purchase_id] || 0;
                                
                                // 使用后端返回的unit_cost字段（已根据产品类型选择正确的价格字段）
                                if (material.unit_cost && material.unit_cost > 0) {
                                  const total_cost = material.unit_cost * return_quantity;
                                  return <div>退回成本: ¥{total_cost.to_fixed(2)}</div>;
                                } else {
