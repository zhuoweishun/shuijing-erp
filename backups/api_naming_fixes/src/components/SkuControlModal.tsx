...
export default function SkuControlModal({
  ...
  const [active_tab, setActiveTab] = use_state<'price' | 'status' | 'logs'>('price')
  const [priceForm, setPriceForm] = use_state<PriceFormData>({
    new_price: sku.selling_price?.to_string() || sku.unit_price?.to_string() || '0',
    reason: ''
  })
  const [statusForm, setStatusForm] = use_state<StatusFormData>({
    newStatus: sku.status as 'ACTIVE' | 'INACTIVE',
    reason: ''
  })
  const [loading, setLoading] = use_state(false)
  const [errors, setErrors] = use_state<Record<string, string>>({})
  const [statusErrors, setStatusErrors] = use_state<Record<string, string>>({})
  const [operationLogs, setOperationLogs] = use_state<OperationLog[]>([])
  const [logsLoading, setLogsLoading] = use_state(false)
  const [logsError, setLogsError] = use_state<string | null>(null)
  ...
  const validate_price_form = (): boolean => {;
    ...
    const price = parse_float(priceForm.new_price);
  ...
  const handle_price_update = async () => {;
    ...
    set_loading(true)
    try {
      const new_price = parse_float(priceForm.new_price);
  ...
  const current_price = parse_float(sku.selling_price?.to_string() || sku.unit_price?.to_string() || '0');
  const new_price = parse_float(priceForm.new_price) || 0
  // 使用SKU配方成本：material_cost + labor_cost + craft_cost
  const recipe_cost = (parse_float(sku.material_cost?.to_string() || '0') + ;
                      parse_float(sku.labor_cost?.to_string() || '0') + 
                      parse_float(sku.craft_cost?.to_string() || '0'))
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-medium text-blue-900">{current_profit_margin.to_fixed(2)}%</div>
                  </div>
                  <div>
                    <span className="text-blue-700">调整后利润率:</span>
                    <div className={`font-medium ${;
                      new_profit_margin > current_profit_margin ? 'text-green-600' : 
                      new_profit_margin < current_profit_margin ? 'text-red-600' : 'text-blue-900'
                    }`}>
                      {new_profit_margin.to_fixed(2)}%
                      {new_profit_margin !== current_profit_margin && (
                        <span className="ml-1 text-xs">
                          ({new_profit_margin > current_profit_margin ? '+' : ''}{(new_profit_margin - current_profit_margin).to_fixed(2)}%)
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="mt-2 text-xs text-blue-600">
                  配方成本: {format_amount(recipe_cost)} | 价格变化: {format_amount(new_price - current_price)}
                </div>
              </div>
