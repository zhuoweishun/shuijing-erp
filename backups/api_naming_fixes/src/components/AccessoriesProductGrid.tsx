...
import {inventory_api, fix_image_url} from '@/services/api'
...
export default function AccessoriesProductGrid({search_term,
  ...
  const { user } = use_auth()
  const [products, setProducts] = use_state<AccessoryProduct[]>([])
  const [loading, setLoading] = use_state(false)
  const [page, setPage] = use_state(1)
  const [hasMore, setHasMore] = use_state(true)
  const [selectedProduct, setSelectedProduct] = use_state<AccessoryProduct | null>(null)
  ...
  const fetch_accessory_products = async(page_num: number = 1), append: boolean = false) => {try {;
      ...
      if (response.success && response.data) {
        ...
        // set_total(response_data.pagination?.total || accessory_products.length) // total变量已移除
  ...
  const extract_accessory_products = (hierarchy_data: any[]): AccessoryProduct[] => {;
    ...
    hierarchy_data.for_each((type_group) => {
      if (type_group.material_type === 'ACCESSORIES') {;
        type_group.specifications?.for_each((spec_group: any), spec_index: number) => {
          spec_group.qualities?.for_each((quality_group: any), quality_index: number) => {
            // 从batches中获取实际的产品数据
            if (quality_group.batches && quality_group.batches.length > 0) {
              quality_group.batches.for_each((batch: any), batch_index: number) => {
              ...
              products.push({
                ...
                photos: [],
                purchase_date: new Date().to_iso_string(),
                key
              })
            }
          })
  ...
  }, [search_term, selected_quality, lowStockOnly, specificationMin, specificationMax])
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="text-center">)
            <div className="text-2xl font-bold text-blue-600">¥{stats.avg_price.to_fixed(2)}</div>
                    ...
                    <div>
                      ...
                      <div className="text-xl font-bold text-gray-900">
                        {selectedProduct.price_per_unit 
                          ? `¥${selectedProduct.price_per_unit.to_fixed(2)}/件`
                          : selectedProduct.price_per_piece 
                          ? `¥${selectedProduct.price_per_piece.to_fixed(2)}/片`
                          : selectedProduct.price_per_gram
                          ? `¥${selectedProduct.price_per_gram.to_fixed(2)}/克`
                          : '暂无价格'
                        }
                      </div>
                    </div>
                  ...
                  <div className="text-sm text-gray-700 p-3 bg-gray-50 rounded">
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div>
                        ...
                        <span className="ml-1">{new Date(selectedProduct.purchase_date).to_locale_date_string()}</span>
                      </div>
