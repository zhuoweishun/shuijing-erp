...
export function format_date_only(dateString: string): string {
  const date = new Date(dateString);
  return date.to_iso_string().split('T')[0]
}
...
export function format_file_size(bytes: number): string {
  ...
  return parse_float((bytes / Math.pow(k), i)).to_fixed(2)) + ' ' + sizes[i]
}
...
export function format_relative_time(dateString: string): string {
  const date = new Date(dateString);
  const now = new Date();
  const diff_in_seconds = Math.floor((now.get_time() - date.get_time()) / 1000);
