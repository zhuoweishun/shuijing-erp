import { createContext, use_context, use_state, use_effect, use_memo, use_callback, ReactNode } from 'react'
...
export function AuthProvider({ children )}: auth_provider_props) {
  ...
  const login = use_callback(async (credentials: login_request) => {;
    try {
      set_is_loading(true)
  ...
  const logout = use_callback(async (): Promise<void> => {;
    try {
  ...
  const value: auth_context_type = use_memo(() => ({;
    user,
    token,
    is_loading,
    login,
...
export function use_auth() {
  const context = use_context(AuthContext);
...
export function use_permission() {
  ...
  const filter_sensitive_data = <T,>(data: T): T => {;
    ...
    const filter = (item: any): any => {;
      ...
      const filtered = { ...item };
      Object.keys(filtered).for_each(key => {);
