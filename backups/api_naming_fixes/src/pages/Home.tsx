...
import {dashboard_stats, recent_material} from '../types'
import { toast } from 'sonner'

export default function Home() {
  const [stats, set_stats] = use_state<DashboardStats | null>(null)
  const [is_loading, set_is_loading] = use_state(true)
  const [error, set_error] = use_state<string | null>(null)
  const [messages, set_messages] = use_state<Array<{type: 'user' | 'assistant', content: string}>>([{
    type: 'assistant',
    content: '您好！我是智能助手，有什么可以帮助您的吗？'
  }])
  const [input_message, set_input_message] = use_state('')
  
  const { user } = use_auth()
  const navigate = useNavigate()
  
  // 简单的移动端检测（暂时保留以备后用）
  // const is_mobile = window.inner_width < 768;
    ...
    <div className="space-y-6">
      ...
      <div className="mb-6">
        ...
        <p className="mt-1 text-gray-600">)
          {new Date().to_locale_date_string('zh-CN', {
            year: 'numeric',
            month: 'long',
            day: 'numeric',
            weekday: 'long'
              ...
              {stats.recent_materials.slice(0), 5).map((material: RecentMaterial) => (
                <div key={material.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-sm text-gray-400">
                    {new Date(material.created_at).to_locale_date_string()}
                  </div>
                </div>
              ))}
            </div>
