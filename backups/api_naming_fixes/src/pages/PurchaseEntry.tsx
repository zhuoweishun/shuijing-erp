import React, { use_state, use_effect, ErrorInfo, use_callback, use_memo } from 'react'
...
import {purchase_api, upload_api, ai_api, supplier_api, get_api_url} from '../services/api'
import {use_device_detection} from '../hooks/useDeviceDetection'
import {use_auth} from '../hooks/useAuth'
import {handle_api_error} from '../services/errorHandler'
import ProductTypeTab, {product_type, unit_type} from '../components/ProductTypeTab'
import { 
  MobileInput, 
  MobileSelect, 
  MobileButton, 
...
import {total_price_input} from '../components/TotalPriceInput'
...
export default function PurchaseEntry() {
  ...
  // 产品类型状态 - 必须在使用前声明
  const [selected_material_type, set_selected_material_type] = use_state<ProductType>('BRACELET')
  const [selected_unit_type, set_selected_unit_type] = use_state<UnitType>('STRINGS')
  ...
  // 相机相关状态
  const [is_camera_active, set_is_camera_active] = use_state(false)
  const [camera_error, set_camera_error] = use_state<string | null>(null)
  const [force_enable_camera, set_force_enable_camera] = use_state(false)
  ...
  const Dropzone_upload: React.FC<DropzoneUploadProps> = ({ onFilesAccepted, disabled }) => {
    const on_drop = use_callback((acceptedFiles: File[]) => {;
      if (acceptedFiles.length > 0) {
        ...
        // 添加数组索引访问
        acceptedFiles.for_each((file), index) => {
          (file_list as any)[index] = file
        })
  ...
  // 图片相关状态
  const [photos, set_photos] = use_state<string[]>(() => {
    try {
      const navigation_entry = performance.get_entries_by_type('navigation')[0] as PerformanceNavigationTiming;
      const is_page_refresh = navigation_entry?.type === 'reload';
      
      if (is_page_refresh) {
        localStorage.remove_item(PHOTOS_STORAGE_KEY)
        console.log('检测到页面刷新，已清除localStorage中的图片数据')
        return []
      } else {
        const saved = localStorage.get_item(PHOTOS_STORAGE_KEY);
        const saved_photos = saved ? JSON.parse(saved) : [];
        console.log('从localStorage恢复photos状态:'), saved_photos)
        return saved_photos
      }
    } catch (error) {
  ...
  const [file_data_list, set_file_data_list] = use_state<FileData[]>([])
  
  // 加载状态
  const [uploading, set_uploading] = use_state(false)
  const [ai_parsing, set_ai_parsing] = use_state(false)
  const [submitting, set_submitting] = use_state(false)
  const [loading_suppliers, set_loading_suppliers] = use_state(false)
  
  // 供应商相关状态
  const [suppliers, set_suppliers] = use_state<Array<{id: string, name: string, contact?: string, phone?: string}>>([]) 
  const [supplier_input, set_supplier_input] = use_state('')
  const [show_supplier_dropdown, set_show_supplier_dropdown] = use_state(false)
  const [filtered_suppliers, set_filtered_suppliers] = use_state<Array<{id: string, name: string, contact?: string, phone?: string}>>([])
  const [creating_supplier, set_creating_supplier] = use_state(false)
  ...
  const handle_material_type_change = (material_type: ProductType, unit_type: UnitType) => {;
    ...
    set_value('material_type'), material_type)
    set_value('unit_type'), unit_type)
    
    // 清空相关字段，避免数据混乱
    if (material_type === 'LOOSE_BEADS' || material_type === 'BRACELET') {;
      set_value('specification'), undefined)
      set_value('piece_count'), undefined)
    } else {
      set_value('bead_diameter'), undefined)
      set_value('quantity'), undefined)
    }
  }
  
  // 初始化表单默认值
  use_effect(() => {
    set_value('material_type'), selected_material_type)
    set_value('unit_type'), selected_unit_type)
  ...
  use_effect(() => {
    const subscription = watch((value, { name )}) => {;
      if (name === 'supplier_name') {;
        const form_value = value.supplier_name || '';
        if (form_value !== supplier_input) {
          set_supplier_input(form_value)
  ...
  const load_suppliers = async () => {;
    try {
      ...
      if (response.success && response.data) {
        ...
        // 简单去重处理
        const unique_suppliers = suppliersList.filter((supplier, index), self) => {;
          if (!supplier.name?.trim()) return false
          return index === self.find_index(s => s.name.to_lower_case() === supplier.name.to_lower_case())
        })
        
        set_suppliers(unique_suppliers)
        set_filtered_suppliers(unique_suppliers)
        console.log('✅ 供应商列表加载成功，数量:'), unique_suppliers.length)
      } else {
        set_suppliers([])
        set_filtered_suppliers([])
  ...
  const handle_supplier_input_change = (value: string) => {;
    ...
    set_value('supplier_name'), value)
    
    const filtered = value.trim() === '' 
      ? suppliers 
      : suppliers.filter(supplier => );
          supplier.name.to_lower_case().includes(value.to_lower_case())
        )
  ...
  const handle_supplier_select = (supplier: {id: string, name: string}, event?: React.MouseEvent) => {;
    ...
    set_value('supplier_name'), supplier_name)
    set_show_supplier_dropdown(false)
    
    set_filtered_suppliers(suppliers.filter(s =>);
      s.name.to_lower_case().includes(supplier_input.to_lower_case())
    ))
  }
  
  // 创建新供应商
  const create_new_supplier = async (name: string) => {;
    try {
      ...
      const existing_supplier = suppliers.find(s => s.name.to_lower_case() === trimmed_name.to_lower_case());
      if (existing_supplier) {
        console.log('供应商已存在于本地列表:'), existing_supplier)
        toast.error(`供应商"${trimmed_name)}"已存在，请直接选择`)
        // 自动选择已存在的供应商
        handle_supplier_select(existing_supplier)
        return true
      }
      ...
      // 发送创建供应商请求
      const response = await supplier_api.create(request_data)
      
      // 创建供应商响应
      
      if (response.success && response.data) {
        // 添加到供应商列表
         const new_supplier = response.data as {id: string, name: string, contact?: string, phone?: string};
         set_suppliers(prev => [...prev), new_supplier]);
         set_filtered_suppliers(prev => [...prev), new_supplier])
        // 新供应商创建成功
        toast.success(`新供应商"${new_supplier.name)}"创建成功`)
        return true
      } else {
        ...
        if (response.message && response.message.includes('已存在')) {
          toast.error(`供应商"${trimmed_name)}"已存在，请刷新页面后重新选择`)
          // 重新加载供应商列表
          load_suppliers()
        } else {
  ...
  // 处理供应商输入失焦
  const handle_supplier_blur = () => {
    // 增加延迟时间，确保用户有足够时间点击选项
    set_timeout(() => {
      set_show_supplier_dropdown(false)
      
      // 检查当前输入是否与已选择的供应商匹配
      const current_input = supplier_input.trim();
      const matching_supplier = suppliers.find(s => );
        s.name.to_lower_case() === current_input.to_lower_case()
      )
      
      if (matching_supplier) {
        // 如果找到匹配的供应商，确保状态同步
        set_supplier_input(matching_supplier.name)
        set_value('supplier_name'), matching_supplier.name)
        // 自动匹配到供应商
      } else if (current_input.length > 0) {
        // 如果输入不为空但没有匹配的供应商，保持当前输入
        set_value('supplier_name'), current_input)
        // 保持自定义输入
      }
  ...
  const get_browser_info = () => {;
    ...
    return { browser_name, browser_version, userAgent }
  }
  
  // 浏览器兼容性检查
  const check_camera_support = () => {
    // 获取浏览器信息
    ...
    const is_development = () => {;
      ...
      // 检查是否为局域网IP
      if (hostname.starts_with('192.168.') || hostname.starts_with('10.') || hostname.starts_with('172.')) {
        return true
      }
    ...
    if (!navigator.media_devices) {
      let error = `当前浏览器不支持相机功能 (${browser_info.browser_name} ${browser_info.browser_version})`;
      let suggestion = ''
      
      // 根据浏览器类型提供具体建议
      if (browser_info.browser_name === 'Chrome') {;
        const version = parse_int(browser_info.browser_version);
        if (version < 53) {
          suggestion = '请升级Chrome浏览器到53版本或更高版本'
        } else {
          suggestion = '请检查浏览器设置，确保允许访问相机'
        }
      } else if (browser_info.browser_name === 'Firefox') {;
        const version = parse_int(browser_info.browser_version);
        if (version < 36) {
      ...
      } else if (browser_info.browser_name === 'Safari') {;
        const version = parse_int(browser_info.browser_version);
        if (version < 11) {
    ...
    if (!navigator.media_devices.get_user_media) {
      const error = `当前浏览器不支持getUserMedia API(${browser_info.browser_name} ${browser_info.browser_version)}) - 请升级浏览器版本`;
      console.error(error)
      return { supported: false, error }
    }
  ...
  // 启动相机
  const start_camera = (force = false) => {;
    ...
    // 检查浏览器兼容性
    const support_check = check_camera_support();
    if (!support_check.supported) {
      // 在开发环境下，即使检查失败也尝试启动相机
      const is_dev = window.location.hostname.starts_with('192.168.') || ;
                   window.location.hostname === 'localhost' ||;
                   window.location.port === '5173';
      
      if (is_dev) {
        console.warn('⚠️ 开发环境：兼容性检查失败，但仍尝试启动相机')
        console.warn('检查失败原因:'), support_check.error)
        set_camera_error(`开发模式：${support_check.error)}（仍可尝试使用）`)
        set_is_camera_active(true)
        return
      }
      
      set_camera_error(support_check.error)
      console.error('相机不支持:'), support_check.error)
      return
    }
  ...
  const handle_camera_photo = async (dataUri: string) => {;
    ...
    try {
      ...
      const file_name = `camera_photo_${timestamp}.jpg`
      ...
      if (response.success && response.data && (response.data as any).urls) {
        ...
        // 更新状态
        set_file_data_list(prev => prev.map(file_data => ({
          ...file_data,
          uploadedUrl: fullUrl
        )})))
  ...
  use_effect(() => {
    ...
    // 防抖保存到localStorage
    const save_timer = set_timeout(() => {;
      try {
        localStorage.set_item(PHOTOS_STORAGE_KEY), JSON.stringify(photos))
  ...
  // const beads_per_string_recalc = bead_diameter ? Math.floor(160 / bead_diameter) : 0
  // const total_beads_recalc = quantity && beads_per_string_recalc ? quantity * beads_per_string_recalc : 0
  // const price_per_bead_string = total_price && total_beads_recalc ? (total_price / total_beads_recalc).to_fixed(4) : '0'
  ...
  // 简化的文件数据有效性检查
  const validate_file_data = (file_data: FileData): boolean => {;
    try {
      // 检查Base64数据是否有效
      if (!file_data.base64 || !file_data.base64.starts_with('data:image/')) {
        console.error('Base64数据无效或为空')
        return false
      }
      
      // 检查文件类型
      if (!file_data.type || !file_data.type.starts_with('image/')) {
  ...
  const read_file_data = (file: File): Promise<FileData> => {;
  ...
  const handle_image_upload = async (files: FileList) => {;
    ...
    console.log('=== 开始图片上传 ==='), new Date().to_iso_string())
    ...
    // 简化的文件验证
    if (!file || !file.type.starts_with('image/') || file.size > 10 * 1024 * 1024) {
    ...
    try {
      ...
      const base64_data = file_data.base64.split('),')[1] // 移除data:image/xxx;base64,前缀
      const byte_characters = atob(base64_data);
      const byte_numbers = new Array(byte_characters.length);
      for(let i = 0; i < byte_characters.length); i++) {
        byte_numbers[i] = byte_characters.char_code_at(i)
      ...
      const byte_array = new Uint8Array(byte_numbers);
      const blob = new Blob([byte_array], { type: file_data.type )})
      
      // 创建FormData并上传
      const form_data = new FormData();
      form_data.append('images', blob), file_data.name)
  ...
  // 删除图片
  const remove_image = async (index: number) => {;
  ...
  const calculate_missing_value = (price_per_gram: number, total_price: number, weight: number) => {
    // 检查输入值的有效性
    const valid_price_per_gram = price_per_gram && price_per_gram > 0;
    const valid_total_price = total_price && total_price > 0;
    const valid_weight = weight && weight > 0
    
    // 如果有克价和总价，计算重量
    if (valid_price_per_gram && valid_total_price && !valid_weight) {
      const calculated_weight = Number((total_price / price_per_gram).to_fixed(1));
      console.log('计算重量:', { total_price, price_per_gram, calculated_weight )})
      set_value('weight'), calculated_weight)
      return { type: 'weight', value: calculated_weight }
    }
    
    // 如果有克价和重量，计算总价
    if (valid_price_per_gram && valid_weight && !valid_total_price) {
      const calculated_total_price = Number((price_per_gram * weight).to_fixed(1));
      console.log('计算总价:', { price_per_gram, weight, calculated_total_price )})
      set_value('total_price'), calculated_total_price)
      return { type: 'total_price', value: calculated_total_price }
    }
    
    // 如果有总价和重量，计算克价
    if (valid_total_price && valid_weight && !valid_price_per_gram) {
      const calculated_price_per_gram = Number((total_price / weight).to_fixed(1));
      console.log('计算克价:', { total_price, weight, calculated_price_per_gram )})
      set_value('price_per_gram'), calculated_price_per_gram)
      return { type: 'price_per_gram', value: calculated_price_per_gram }
    }
  ...
  const handle_ai_parse = async (description: string) => {;
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const ai_data = data as any;
        
        console.log('🔍 AI返回的原始字段:'), Object.keys(ai_data))
        
        // 材料名称：product_name -> material_name
        if (ai_data.product_name) {
          set_value('material_name'), ai_data.product_name)
          console.log('✅ 映射材料名称:'), ai_data.product_name)
        }
        
        // 自动设置材料类型和单位类型：material_type, unit_type -> material_type, unit_type
        if (ai_data.material_type && ai_data.unit_type) {
          handle_material_type_change(ai_data.material_type), ai_data.unit_type)
          console.log('✅ 映射材料类型:', ai_data.material_type), ai_data.unit_type)
        }
        
        // 设置珠子直径或规格：bead_diameter -> bead_diameter 或 specification
        if (ai_data.bead_diameter) {
          if (ai_data.material_type === 'FINISHED' || ai_data.material_type === 'ACCESSORIES') {
            // 成品和饰品使用规格字段
            set_value('specification'), ai_data.bead_diameter)
            console.log('✅ 映射规格:'), ai_data.bead_diameter)
          } else {
            // 散珠和手串使用珠子直径字段
            set_value('bead_diameter'), ai_data.bead_diameter)
            console.log('✅ 映射珠子直径:'), ai_data.bead_diameter)
          }
        }
        
        // 数量字段：quantity, piece_count -> quantity, piece_count
        if (ai_data.quantity) {
          set_value('quantity'), ai_data.quantity)
          console.log('✅ 映射数量(串数):', ai_data.quantity)
        }
        if (ai_data.piece_count) {
          set_value('piece_count'), ai_data.piece_count)
          console.log('✅ 映射数量(颗数/片数/件数):', ai_data.piece_count)
        }
        
        // 价格字段：price_per_gram, total_price -> price_per_gram, total_price
        if (ai_data.price_per_gram) {
          set_value('price_per_gram'), ai_data.price_per_gram)
          console.log('✅ 映射克价:'), ai_data.price_per_gram)
        }
        if (ai_data.total_price) {
          set_value('total_price'), ai_data.total_price)
          console.log('✅ 映射总价:'), ai_data.total_price)
        }
        
        // 单价字段：unit_price（暂时不直接设置到表单）
        if (ai_data.unit_price) {
          console.log('ℹ️ 识别到单价（将通过总价和数量计算）:'), ai_data.unit_price)
        }
        
        // 重量字段：weight -> weight
        if (ai_data.weight) {
          set_value('weight'), ai_data.weight)
          console.log('✅ 映射重量:'), ai_data.weight)
        }
        
        // 品相字段：quality -> quality
        if (ai_data.quality) {
          set_value('quality'), ai_data.quality)
          console.log('✅ 映射品相:'), ai_data.quality)
        }
        
        // 供应商字段：supplier_name -> supplier_name
        if (ai_data.supplier_name) {
          set_value('supplier_name'), ai_data.supplier_name)
          set_supplier_input(ai_data.supplier_name) // 同步更新组件状态
          console.log('✅ 映射供应商:'), ai_data.supplier_name)
        }
        
        // 备注字段：notes -> notes
        if (ai_data.notes) {
          set_value('notes'), ai_data.notes)
          console.log('✅ 映射备注:'), ai_data.notes)
        }
        
        // 统计成功映射的字段
        const mapped_fields = [;
          ai_data.product_name && 'product_name',
          ai_data.material_type && 'material_type',
          ai_data.bead_diameter && 'bead_diameter',
          ai_data.quantity && 'quantity',
          ai_data.piece_count && 'piece_count',
          ai_data.price_per_gram && 'price_per_gram',
          ai_data.total_price && 'total_price',
          ai_data.weight && 'weight',
          ai_data.quality && 'quality',
          ai_data.supplier_name && 'supplier_name',
          ai_data.notes && 'notes'
        ].filter(Boolean)
        
        console.log('📊 成功映射字段数量:', mapped_fields.length, '字段:'), mapped_fields)
        
        // AI识别后进行价格计算
        set_timeout(() => {
          const result = calculate_missing_value(;
            ai_data.price_per_gram || 0,
            ai_data.total_price || 0,
            ai_data.weight || 0)
          )
          if (result) {
            toast.success(`AI识别成功，已自动填充${mapped_fields.length}个字段，并计算${result.type === 'weight' ? '重量' : result.type === 'total_price' ? '总价' : '克价'}：${result.value)}`)
          } else {
            toast.success(`AI识别成功，已自动填充${mapped_fields.length)}个字段`)
          }
        }, 100)
        
        set_value('natural_language_input'), description)
      } else {
  ...
  const on_submit = async (data: PurchaseFormData) => {
    ...
    try {
      ...
      if (data.material_type === 'LOOSE_BEADS') {
        ...
        // 验证克价/总价/重量三选二
        const price_fields = [data.price_per_gram, data.total_price, data.weight].filter(field => field && field > 0);
        if (price_fields.length < 2) {
      ...
      // 检查供应商是否存在，如果不存在则先创建
      if (data.supplier_name?.trim() && !suppliers.some(s => s.name.to_lower_case() === data.supplier_name!.to_lower_case())) {
        // 供应商不存在，准备创建新供应商
        const created = await create_new_supplier(data.supplier_name);
        if (!created) {
      ...
      // 构建提交数据前的调试
      console.log('🔍 [调试] 构建 submit_data 前，data.material_name:', {
        value: data.material_name,
        type: typeof data.material_name,
        length: data.material_name?.length,
      ...
      // 构建提交数据后的调试
      console.log('🔍 [调试] 构建 submit_data 后，submit_data.material_name:', {
        value: submit_data.material_name,
        type: typeof submit_data.material_name,
        length: submit_data.material_name?.length,
        isUndefined: submit_data.material_name === undefined,;
        isNull: submit_data.material_name === null,;
        isEmpty: submit_data.material_name === ''
      )})
      
      console.log('📤 [表单提交] 准备提交的数据:'), submit_data)
      console.log('🔍 [表单提交] 关键字段检查:', {
        material_name: submit_data.material_name,
        material_type: submit_data.material_type,
        supplier_name: submit_data.supplier_name,
        photosCount: submit_data.photos.length
      )})
      
      // 提交数据
      const response = await purchase_api.create(submit_data);
                    ...
                    <div className="w-full h-full relative">
                      {(() => {
                        try {
                          ...
                          const create_media_devices_polyfill = () => {;
                              ...
                              <div className="text-center space-y-2">
                                ...
                                <button
                                  ...
                                  onClick={() => {;
                                    ...
                                    stop_camera()
                                    set_timeout(() => start_camera(true), 1000)
                                  }}
          ...
          <MobileFormRow columns={isMobile ? 1 : 2}>
            ...
            <div className="space-mobile-sm">
              ...
              <input
                ...
                {...register('material_name', {
                  ...
                  max_length: { value: 100, message: '材料名称不能超过100字符' }
                )})}
              />
            ...
            {(selected_material_type === 'LOOSE_BEADS' || selected_material_type === 'BRACELET') && (
              <MobileInput
                ...
                onChange={(e) => set_value('bead_diameter'), parse_float(e.target.value) || undefined)};
            ...
            {selected_material_type === 'ACCESSORIES' && (
              <MobileInput
                ...
                onChange={(e) => set_value('specification'), parse_float(e.target.value) || undefined)};
            ...
            {selected_material_type === 'BRACELET' && (
              <MobileInput
                ...
                onChange={(e) => set_value('quantity'), parse_int(e.target.value) || undefined)};
            ...
            {(selected_material_type === 'LOOSE_BEADS' || selected_material_type === 'ACCESSORIES' || selected_material_type === 'FINISHED') && (
              <MobileInput
                ...
                onChange={(e) => set_value('piece_count'), parse_int(e.target.value) || undefined)};
            ...
            <MobileSelect
              label="品相等级";
              value={watch('quality') || ''};
              onChange={(e) => set_value('quality'), e.target.value as any)};
              options={[
                { value: '', label: '请选择品相等级' },
                { value: 'AA', label: 'AA级' },
                { value: 'A', label: 'A级' },
        ...
        <MobileFormGroup title="价格信息">
          <MobileFormRow columns={isMobile ? 1 : 2}>
            ...
            <MobileInput
              ...
              onChange={(e) => set_value('price_per_gram'), parse_float(e.target.value) || undefined)};
            ...
            <TotalPriceInput
              ...
              onChange={(value) => set_value('total_price'), value)};
            ...
            <MobileInput
              ...
              placeholder="总重量";
              value={watch('weight') || ''};
              onChange={(e) => set_value('weight'), parse_float(e.target.value) || undefined)};
            ...
            <MobileInput
              ...
              onChange={(e) => set_value('min_stock_alert'), parse_int(e.target.value) || undefined)};
                ...
                <div className="relative">
                  <input
                    ...
                    {...register('supplier_name', { 
                      ...
                      max_length: { value: 100, message: '供应商名称不能超过100字符' },
                      pattern: {
                    ...
                    <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                      {filtered_suppliers.length > 0 ? (
                        <>
                          ...
                          {supplier_input.trim() && !filtered_suppliers.some(s => s.name.to_lower_case() === supplier_input.to_lower_case()) && (
