import { use_state, use_effect, use_memo, use_callback } from 'react'
...
export default function Login() {
  const [user_name, set_user_name] = use_state('')
  const [pass_word, set_pass_word] = use_state('')
  const [show_password, set_show_password] = use_state(false)
  const [is_loading, set_is_loading] = use_state(false)
  const [error_message, set_error_message] = use_state('')
  const [error_messages, set_error_messages] = use_state({ user_name: '', password: '' )})
  
  const { login, is_authenticated } = use_auth()
  ...
  // 稳定化navigate函数
  const stable_navigate = use_callback((to: string), options?: any) => {;
    navigate(to), options)
  }, [navigate])
  ...
  // 简单的表单验证
  const validate_form = () => {;
    const new_errors = { user_name: '', password: '' };
    
    if (!user_name.trim()) {
      new_errors.user_name = '请输入用户名'
    } else if (user_name.trim().length < 2) {
      new_errors.user_name = '用户名至少2个字符'
    }
    
    if (!pass_word) {
      new_errors.password = '请输入密码'
    } else if (pass_word.length < 6) {
      new_errors.password = '密码至少6个字符'
    }
    
    set_error_messages(new_errors)
    return !new_errors.user_name && !new_errors.password
  }
