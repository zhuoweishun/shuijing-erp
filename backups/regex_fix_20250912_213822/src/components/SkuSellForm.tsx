...
import { SkuItem, SellData, Customer } from '../types'
...
export default function SkuSellForm({ sku, on_submit, onCancel, loading = false, sale_source = 'SKU_PAGE', customer }: SkuSellFormProps) {
  ...
  const [form_data, setFormData] = useState<FormState>(getInitialFormData())
  ...
  const validateForm = (): boolean => {
    ...
    // 验证客户姓名（必填）
    if (!form_data.customer_name || form_data.customer_name.trim().length < 2) {
      newErrors.customer_name = '客户姓名必填，至少需要2个字符'
    }

    // 验证客户手机号（必填）
    if (!form_data.customer_phone || form_data.customer_phone.trim().length === 0) {
      newErrors.customer_phone = '客户手机号必填'
    } else if (!/^1[3-9]\d{9}$/.test(form_data.customer_phone.trim())) {
      newErrors.customer_phone = '请输入正确的手机号格式'
    }

    // 验证实际销售总价（必填）
    if (!form_data.actual_total_price || form_data.actual_total_price <= 0) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const sellData: SellData = {
      ...
      customer_name: form_data.customer_name.trim(),
      customer_phone: form_data.customer_phone.trim(),
      customer_address: form_data.customer_address.trim() || undefined,
      sale_channel: form_data.sale_channel.trim() || undefined,
      sale_source: sale_source, // 添加销售来源标记
      notes: form_data.notes.trim() || undefined,
      actual_total_price: form_data.actual_total_price
    }
  ...
  const handleInputChange = (field: keyof FormState, value: string | number) => {
    setFormData(prev => {
      ...
      // 当数量变化时，自动更新实际销售总价
      if (field === 'quantity') {
        newData.actual_total_price = Number(sku.unit_price || sku.selling_price || 0) * Number(value)
      }
  ...
  const calculateSuggestedPrice = () => {
    return Number(sku.unit_price || sku.selling_price || 0) * Number(form_data.quantity)
  }

  // 计算优惠金额
  const calculateDiscount = () => {
    const suggested = calculateSuggestedPrice()
    const actual = Number(form_data.actual_total_price)
      ...
      <form on_submit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              value={form_data.customer_address}
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              min="0.01"
              step="0.01"
              value={form_data.actual_total_price}
              onChange={(e) = /> handleInputChange('actual_total_price', parseFloat(e.target.value) || 0)}
              className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent ${
          ...
          <div className="mt-1 text-xs text-gray-500">
            <p>建议价格：¥{calculateSuggestedPrice().toFixed(2)}</p>
            {calculateDiscount() > 0 && (
              <p className="text-orange-600">优惠金额：¥{calculateDiscount().toFixed(2)}</p>
            )}
            {calculateDiscount() < 0 && (
              <p className="text-green-600">溢价金额：¥{Math.abs(calculateDiscount()).toFixed(2)}</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-lg font-medium text-green-600">
              ¥{Number(form_data.actual_total_price || 0).toFixed(2)}
            </span>
          </div>
          <div className="flex items-center justify-between mt-1">
            <span className="text-xs text-gray-500">数量：{form_data.quantity} 件</span>
            <span className="text-xs text-gray-500">平均单价：¥{(Number(form_data.actual_total_price || 0) / Number(form_data.quantity || 1)).toFixed(2)}/件</span>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3 pt-4">
          ...
          <button
            ...
            disabled={loading || form_data.quantity <= 0 || form_data.quantity > sku.available_quantity || !form_data.customer_name.trim() || !form_data.customer_phone.trim() || form_data.actual_total_price <= 0}
          >
            {loading ? (
