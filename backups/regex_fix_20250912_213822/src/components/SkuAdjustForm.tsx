...
export default function SkuAdjustForm({ sku, on_submit, onCancel, loading = false }: SkuAdjustFormProps) {
  ...
  const validateForm = (): boolean => {
    ...
    } else if (form_data.type === 'decrease' && form_data.quantity > sku.available_quantity) {
    ...
    // 验证成本调整
    if (form_data.cost_adjustment < 0) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const adjustData: AdjustData = {
      ...
      cost_adjustment: form_data.cost_adjustment
    }
  ...
  const calculateNewQuantity = () => {
    if (form_data.type === 'increase') {
      return sku.available_quantity + form_data.quantity
    } else {
      return sku.available_quantity - form_data.quantity
    }
  }
  ...
  const calculateNewTotalCost = () => {
    ...
    if (form_data.type === 'increase') {
      return currentTotalCost + (form_data.cost_adjustment * form_data.quantity)
    } else {
      const removedCost = unitCost * form_data.quantity
  ...
  const calculateNewUnitCost = () => {
    const new_quantity = calculateNewQuantity()
    ...
    <div className="space-y-6">
      ...
      <form on_submit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            <input
              type="number"
              min="1"
              max={form_data.type === 'decrease' ? sku.available_quantity : undefined}
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {adjustReasons[form_data.type].map((reason) => (
                <button
                  key={reason}
                  type="button"
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                min="0"
                step="0.01"
                value={form_data.cost_adjustment}
        ...
        <div className={`rounded-lg p-4 ${
          form_data.type === 'increase' ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'
        }`}>
          <h3 className={`text-sm font-medium mb-3 ${
            form_data.type === 'increase' ? 'text-green-800' : 'text-red-800'
          }`}>
            调整预览
          </h3>
          <div className="space-y-2 text-sm">
            ...
            <div className="flex items-center justify-between">
              <span className="text-gray-600">
                {form_data.type === 'increase' ? '增加数量：' : '减少数量：'}
              </span>
              <span className={`font-medium ${
                form_data.type === 'increase' ? 'text-green-600' : 'text-red-600'
              }`}>
                {form_data.type === 'increase' ? '+' : '-'}{form_data.quantity} 件
              </span>
            </div>
        ...
        <div className="flex items-center space-x-3 pt-4">
          ...
          <button
            ...
            disabled={loading || form_data.quantity <= 0 || !form_data.reason.trim() || (form_data.type === 'decrease' && form_data.quantity > sku.available_quantity)}
          >
            {loading ? (
              ...
              `确认${form_data.type === 'increase' ? '增加' : '减少'}库存`
            )}
          </button>
        </div>
      </form>
