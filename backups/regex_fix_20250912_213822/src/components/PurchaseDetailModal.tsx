...
import { purchase_api, supplier_api, fixImageUrl } from '../services/api'
...
export default function PurchaseDetailModal({ 
  ...
  const fetchPurchaseDetail = async () => {if (!purchase_id) return
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // 调试日志：检查editLogs数据
        console.log('📝 修改日志数据:', {
          hasEditLogs: !!purchaseData.editLogs,
        editLogsCount: purchaseData.editLogs?.length || 0,
        editLogs: purchaseData.editLogs
        })
  ...
  const updateEditData = (field: string, value: any) => {
    ...
    setEditData(prev => {
      ...
      if (field === 'total_beads') {
        console.log('🔍 [totalBeads更新] 保存后的totalBeads值:', newData.total_beads, '类型:', typeof newData.total_beads)
      }
  ...
  const calculateAllSuggestions = (currentEditData: any, originalData: any) => {// 获取产品类型
    ...
    // 3. 总颗数计算和验证 - 仅适用于手串类型
    if (product_type === 'BRACELET') {
      if (quantity > 0 && beads_per_string > 0 && !totalBeadsChanged && !quantity_changed) {
        ...
        if (Math.abs(total_beads - calculatedTotalBeads) > 0.1) {
          newSuggestions.beads_count_warning = {
            ...
            calculatedValue: calculatedTotalBeads
          }
        }
      }
    }
  ...
  // 保存编辑
  const handleSave = async () => {if (!purchase || !canEdit) return
    ...
    try {
      ...
      if (editData.total_beads !== undefined) {
        ...
        console.log('🔍 [totalBeads调试] editData.total_beads:', editData.total_beads, '类型:', typeof editData.total_beads, '转换后:', editValue)
        console.log('🔍 [totalBeads调试] purchase.total_beads:', purchase.total_beads, '类型:', typeof purchase.total_beads, '转换后:', originalValue)
        console.log('🔍 [totalBeads调试] 数值比较结果:', editValue !== originalValue)
        
        if (editValue !== originalValue) {
          update_data.total_beads = editValue
          console.log('🔍 [totalBeads调试] 已添加到updateData:', editValue)
        } else {
      ...
      if (editData.supplier_name !== undefined && editData.supplier_name !== (purchase.supplier?.name || '')) {
        update_data.supplier_name = editData.supplier_name
      }
      if (editData.notes !== undefined && editData.notes !== purchase.notes) {
        update_data.notes = editData.notes
      }
      ...
      console.log('🔍 [前端调试] 准备更新的数据:', update_data)
      console.log('🔍 [前端调试] updateData字段数量:', Object.keys(update_data).length)
      console.log('🔍 [前端调试] updateData详细内容:', JSON.stringify(update_data, null, 2))
      console.log('🔍 [前端调试] 即将调用API:', `purchaseApi.update(${purchase.id}, update_data)`)
      
      // 调用后端API保存数据
      console.log('🚀 [API调用] 开始调用purchaseApi.update')
      const response = await purchase_api.update(purchase.id, update_data)
  ...
  // 删除采购记录
  const handleDelete = async () => {if (!purchase || !canEdit) return
                                ...
                                <p className="font-semibold text-orange-900 text-xs">
                                  {user?.role === 'EMPLOYEE' ? '-' : (
                                    (() => {
                                      ...
                                      const currentTotalBeads = is_edit_mode && editData.total_beads !== undefined ? editData.total_beads : purchase.total_beads
                                      ...
                                      if (currentTotalPrice > 0 && totalBeadsForCalculation && totalBeadsForCalculation > 0) {
                                        return format_price(currentTotalPrice / totalBeadsForCalculation)
                                      }
                                      return '-'
                                    })()
                                  )}
                              ...
                              <div className="text-center p-1.5 bg-blue-50 rounded-md border border-blue-100">
                                ...
                                <p className="font-semibold text-blue-900 text-xs">
                                  {user?.role === 'EMPLOYEE' ? '-' : (
                                    (() => {
                                      const currentTotalPrice = isEditMode && editData.total_price !== undefined ? editData.total_price : purchase.total_price || 0
                                      const currentPieceCount = is_edit_mode && editData.piece_count !== undefined ? editData.piece_count : purchase.piece_count || 0
                                      
                                      if (currentTotalPrice > 0 && currentPieceCount > 0) {
                                        return format_price(currentTotalPrice / currentPieceCount)
                                      }
                                      return '-'
                                    })()
                                  )}
                    ...
                    <div className="space-y-2">
                      ...
                      {purchase.editLogs && purchase.editLogs.length > 0 ? (
                        <div className="space-y-2">
                          ...
                          {(() => {
                            // 按用户和时间分组合并日志
                            const groupedLogs = purchase.editLogs.reduce((groups: {[key: string]: any[]}, log: any) => {
                              // 使用用户ID和精确到分钟的时间作为分组键
                              const timeKey = new Date(log.created_at).to_i_s_o_string().slice(0, 16)
                            ...
                            return sortedGroups.map(([groupKey, logs]) => {
                              ...
                              const editTime = new Date(timeKey).to_locale_string('zh-CN', {
                                year: 'numeric',
                                month: '2-digit',
                                day: '2-digit',
                                hour: '2-digit',
