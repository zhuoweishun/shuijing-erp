...
import { Customer, SkuItem, SellData } from '../types'
...
export default function ReverseSaleModal({ customer, is_open, on_close, on_success }: ReverseSaleModalProps) {
  ...
  const fetchAvailableSkus = async () => {
    try {
      ...
      const response = await customer_api.get_available_skus(params)
  ...
  }, [is_open, state.pagination.page, state.search_term])
  ...
  // 处理销售提交
  const handleSaleSubmit = async (sellData: SellData) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        {/* 背景遮罩 */}
        <div className="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" onClick={on_close} />
