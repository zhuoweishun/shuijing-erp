...
const InventoryPieChart: React.FC = () => {const [selected_type, setSelectedType] = useState<ProductType>('LOOSE_BEADS')
  const [data, set_data] = useState<ProductDistributionData | null>(null)
  ...
  const fetchProductDistribution = async (material_type: ProductType) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // 检查数据是否有效
        if (!backendData || !Array.isArray(backendData.items) || backendData.items.length === 0) {
        ...
        set_data(compatibleData as ProductDistributionData)
