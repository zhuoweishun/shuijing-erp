...
const Inventory_consumption_chart: React.FC = () => {
  ...
  const [data, set_data] = useState<ConsumptionData | null>(null)
  const [loading, set_loading] = useState(false)
  const [viewMode, set_view_mode] = useState<'chart' | 'table'>('chart')
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          <p className="text-sm text-gray-600">
            总消耗: {data.consumed.toLocaleString()} {data.unit_type}
          </p>
          <p className="text-sm text-gray-600">
            消耗次数: {data.count} 次
          </p>
                    ...
                    <div key={item.purchase_id} className="card-mobile">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-mobile-caption">
                        <div>
                          ...
                          <span className="ml-1 font-medium">{item.totalConsumed.toLocaleString()} {item.unit_type}</span>
                        </div>
                        <div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {data?.topConsumedProducts?.map((item, index) => (
                      <tr key={item.purchase_id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        ...
                        <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                          {item.totalConsumed.toLocaleString()} {item.unit_type}
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
