...
const Customer_create_modal: React.FC<CustomerCreateModalProps> = ({
  ...
  const [form_data, setFormData] = useState<CustomerFormData>({
    name: '',
    phone: '',
    address: '',
    wechat: '',
  ...
  const validateForm = (): boolean => {
    ...
    } else if (form_data.name.trim().length > 20) {
    ...
    if (!form_data.phone.trim()) {
      ...
      if (!phoneRegex.test(form_data.phone.trim())) {
    ...
    } else if (form_data.address.trim().length > 100) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await customer_api.create({
        ...
        wechat: form_data.wechat?.trim() || undefined,
        birthday: form_data.birthday?.trim() || undefined,
