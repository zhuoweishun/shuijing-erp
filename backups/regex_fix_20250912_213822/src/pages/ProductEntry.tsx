...
export default function ProductEntry() {
  ...
  const addMaterial = (material: AvailableMaterial) => {
    if (form_data.mode === 'DIRECT_TRANSFORM') {
      ...
      // 组合制作模式：原有逻辑
      const isAlreadySelected = form_data.selected_materials.some(
  ...
  const calculateMaxProductionQuantityForMaterials = (materials: typeof form_data.selected_materials): number => {
  ...
  const calculateMaxProductionQuantity = (): number => {
    return calculateMaxProductionQuantityForMaterials(form_data.selected_materials)
  }
    ...
    set_batch_form_data(prev => ({
      selected_materials: prev.selected_materials.map(material => {
        if (material.purchase_id === productId) {
          ...
          // 重新计算成本和利润率
          const total_cost = updatedProductInfo.material_cost + updatedProductInfo.labor_cost + updatedProductInfo.craft_cost
  ...
  const handleSubmit = async () => {
    try {
      ...
      // 验证制作数量是否超过库存限制
      const maxQuantity = calculateMaxProductionQuantity()
      if (form_data.production_quantity > maxQuantity) {
            ...
            <div className="mt-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
              ...
              <div className="space-y-2">
                {form_data.selected_materials.map((item: any) => {
                  ...
                  const itemCost = (material.unit_cost || 0) * used_quantity
                  ...
                  {(form_data.labor_cost > 0 || form_data.craft_cost > 0 || form_data.selling_price > 0) && (
                    <div className="border-t border-blue-300 pt-2 mt-2">
                      ...
                      <div className="flex justify-between text-xs text-blue-700">
                        <span>人工成本:</span>
                        <span>¥{((form_data.labor_cost || 0) * form_data.production_quantity).toFixed(2)}</span>
                      </div>
                      
                      {/* 工艺成本 */}
                      <div className="flex justify-between text-xs text-blue-700">
                        <span>工艺成本:</span>
                        <span>¥{((form_data.craft_cost || 0) * form_data.production_quantity).toFixed(2)}</span>
                      </div>
                      ...
                      {form_data.selling_price > 0 && (() => {
                        ...
                        const craft_cost = cost_calculation?.craftCost || 
                          ((form_data.craft_cost || 0) * form_data.production_quantity)
                        const total_cost = materialCost + laborCost + craftCost
                        const totalSellingPrice = form_data.selling_price * form_data.production_quantity
                        ...
                        return (
                          <>
                            <div className="flex justify-between text-xs text-blue-700 mt-1">
                              <span>销售价格:</span>
                              <span>¥{(form_data.selling_price * form_data.production_quantity).toFixed(2)}</span>
                            </div>
               ...
               <div className="bg-gray-50 p-3 rounded-lg">
                 ...
                 <div className="space-y-1 text-sm">
                   ...
                   <div className="flex justify-between">
                     ...
                     <span>¥{(cost_calculation?.labor_cost || form_data.labor_cost || 0).toFixed(2)}</span>
                   </div>
                   
                   {/* 工艺成本 - 始终显示 */}
                   <div className="flex justify-between">
                     <span className="text-gray-600">工艺成本：</span>
                     <span>¥{(cost_calculation?.craft_cost || form_data.craft_cost || 0).toFixed(2)}</span>
                   </div>
        ...
        <div className="flex items-center justify-between">
          {(() => {
            ...
            const handleStepClick = (stepKey: string, stepIndex: number) => {
              const currentIndex = steps.find_index(s => s.key === current_step)
              // 只允许点击当前步骤或之前的步骤
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {user?.role === 'BOSS' && selected_material_detail.unit_cost && (
                  <div>
                    ...
                    <div className="text-xl font-bold text-gray-900">¥{selected_material_detail.unit_cost.toFixed(2)}</div>
                  </div>
                )}
              </div>
                    ...
                    <div>
                     ...
                     {user?.role === 'BOSS' && selected_material_detail.unit_cost && (
                       <div>
                         ...
                         <span className="ml-1">¥{selected_material_detail.unit_cost.toFixed(2)}</span>
                       </div>
                     )}
                  </div>
                </div>
