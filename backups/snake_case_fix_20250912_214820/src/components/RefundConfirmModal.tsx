...
export default function RefundConfirmModal({
  ...
  const validateForm = () => {
    ...
    const maxQuantity = getMaxRefundQuantity()
    if (refundData.quantity > maxQuantity) {
      newErrors.quantity = `退货数量不能超过${maxQuantity}`
    }
  ...
  // 处理输入变化
  const handleInputChange = (field: keyof RefundData, value: string | number) => {
  ...
  const maxQuantity = getMaxRefundQuantity()
  // const reasonLabel = REFUND_REASONS.find(r => r.value === refundData.reason)?.label || ''
