...
router.get('/:id', authenticate_token, async (req, res) => {
  try {
    ...
    const material = await prisma.material.find_unique({
      where: { id },
      include: {
        purchase: {
          select: {
...
router.post('/', authenticate_token, async (req, res) => {
  try {
    ...
    // 检查是否已经转换过
    const existingMaterial = await prisma.material.find_first({
...
router.put('/:id', authenticate_token, async (req, res) => {
  try {
    ...
    // 检查原材料是否存在
    const existingMaterial = await prisma.material.find_unique({
      where: { id }
    });
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        semiFinishedCount,
        finishedCount,
        total_value: total_value.Sum.total_cost || 0
      }
    });
  } catch (error) {
