...
function calculateCustomerType(customer: any): string {
  ...
  if (customer.last_purchase_date) {
    const days_since_last_purchase = Math.floor(
      (Date.now() - new Date(customer.last_purchase_date).get_time()) / (1000 * 60 * 60 * 24)
    )
    if (daysSinceLastPurchase >= 366) return 'LOST'
    if (daysSinceLastPurchase >= 181) return 'SILENT'
    if (daysSinceLastPurchase >= 91) return 'COOLING'
    if (daysSinceLastPurchase >= 31) return 'DECLINING'
  }
  ...
  try {
    ...
    switch (time_period) {
      case 'week':
        dateFilter = {
          created_at: {
            gte: new Date(now.get_time() - 7 * 24 * 60 * 60 * 1000)
          }
        }
        break
      case 'month':
      ...
      case 'half_year':
        dateFilter = {
          created_at: {
            gte: new Date(now.get_time() - 180 * 24 * 60 * 60 * 1000)
          }
        }
        break
      case 'year':
        dateFilter = {
          created_at: {
            gte: new Date(now.get_time() - 365 * 24 * 60 * 60 * 1000)
          }
        }
        break
      case 'all':
   ...
   // 总客户数（根据时间筛选）
    const [total_customers, currentPurchases, totalRevenue, new_customers, repeat_customers, vip_customers, active_customers, total_refunds, active_purchases] = await Promise.all([
      ...
      prisma.customer_purchase.find_many({
        where: {
          ...purchaseDateFilter,
    ...
    const inactive_customers = total_customers - activeCustomers
    const average_order_value = currentPurchases > 0 ? Number(totalRevenue.Sum.total_price || 0) / currentPurchases : 0
    const repeat_purchase_rate = total_customers > 0 ? (repeatCustomers / total_customers) * 100 : 0
    
    // 退货率计算逻辑：总订单数就是所有购买记录数（包括正常和已退货的）
    // 退货率 = 已退货记录数 / 总订单数 * 100%
    const refund_rate = currentPurchases > 0 ? (totalRefunds / currentPurchases) * 100 : 0
    ...
    const totalSalesAmount = totalRevenue.Sum.total_price || 0
...
router.get('/', authenticate_token, asyncHandler(async (req, res) => {
  ...
  let customersWithMapping = customers.map((customer, index) => {
    ...
    const totalActiveOrders = customer.purchases ? 
      customer.purchases.filter(purchase => purchase.status === 'ACTIVE').length : 0
    
    // 计算退货相关统计
    const totalRefundedOrders = customer.purchases ? 
      customer.purchases.filter(purchase => purchase.status === 'REFUNDED').length : 0
  ...
  if (customer_code_search) {
    const codeSearch = String(customer_code_search).to_lower_case()
    customersWithMapping = customersWithMapping.filter(customer => 
      customer.customer_code.to_lower_case().includes(codeSearch)
    )
  }
  ...
  // 获取城市统计数据（如果请求）
  let city_stats = null
  if (get_city_stats === 'true' || get_city_stats === true) {
    // 获取所有客户的城市信息
    const allCustomers = await prisma.customer.find_many({
      select: {
        address: true
      }
    })
...
router.get('/:id', authenticate_token, asyncHandler(async (req, res) => {
  ...
  const customer = await prisma.customer.find_unique({
    where: { id },
    include: {
      purchases: {
...
router.post('/', authenticate_token, asyncHandler(async (req, res) => {
  ...
  // 检查手机号是否已存在
  const existingCustomer = await prisma.customer.find_unique({
...
router.put('/:id', authenticate_token, asyncHandler(async (req, res) => {
  ...
  if (validatedData.phone !== existingCustomer.phone) {
    const phoneExists = await prisma.customer.find_unique({
      where: { phone: validatedData.phone }
    })
...
router.post('/:customer_id/purchases/:purchase_id/refund', authenticate_token, asyncHandler(async (req, res) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    // 1. 检查客户是否存在
    const customer = await tx.customer.find_unique({
    ...
    // 2. 检查购买记录是否存在
    const purchase = await tx.customer_purchase.find_unique({
      where: { 
        id: purchase_id,
