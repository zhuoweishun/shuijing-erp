...
router.get('/debug', authenticate_token, asyncHandler(async (req, res) => {
  try {
    ...
    // 查询前5条采购记录
    const samplePurchases = await prisma.purchase.find_many({
      take: 5,
      select: {
        id: true,
        product_name: true,
...
router.get('/hierarchical', authenticate_token, asyncHandler(async (req, res) => {// 验证查询参数
  ...
  try {// 查询所有库存数据
    ...
    allInventory.forEach((item: any) => {
      ...
      // 应用筛选条件
      if (search && !item.product_name.to_lower_case().includes(search.to_lower_case())) return
...
router.get('/product-distribution', authenticate_token, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const pieChartData = topProducts.map(item => ({
      ...
      percentage: ((item.total_quantity / total_quantity) * 100).to_fixed(1)
    }))
    ...
    if (othersQuantity > 0) {
      pieChartData.push({
        ...
        percentage: ((othersQuantity / total_quantity) * 100).to_fixed(1)
      })
    }
...
router.get('/consumption-analysis', authenticate_token, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    switch (time_range) {
      case '7d':
        const sevenDaysAgo = new Date(now.get_time() - 7 * 24 * 60 * 60 * 1000)
        timeCondition = `AND mu.created_at >= '${sevenDaysAgo.toISOString()}'`
        break
      case '30d':
        const thirtyDaysAgo = new Date(now.get_time() - 30 * 24 * 60 * 60 * 1000)
        timeCondition = `AND mu.created_at >= '${thirtyDaysAgo.toISOString()}'`
        break
      case '90d':
        const ninetyDaysAgo = new Date(now.get_time() - 90 * 24 * 60 * 60 * 1000)
        timeCondition = `AND mu.created_at >= '${ninetyDaysAgo.toISOString()}'`
        break
      case '6m':
        const sixMonthsAgo = new Date(now.get_time() - 6 * 30 * 24 * 60 * 60 * 1000)
        timeCondition = `AND mu.created_at >= '${sixMonthsAgo.toISOString()}'`
        break
      case '1y':
        const oneYearAgo = new Date(now.get_time() - 365 * 24 * 60 * 60 * 1000)
  ...
  try {
    ...
    if (price_type === 'unit_price') {
      ...
      const priceRanges = rangeData.map(item => ({
        ...
        percentage: totalCount > 0 ? (Number(item.count) / total_count * 100).to_fixed(1) : '0'
      }))
