...
const calculateRemainingQuantity = async (purchase: any, tx: any) => {
  ...
  const totalUsage = await tx.materialUsage.aggregate({
...
router.get('/:id', authenticate_token, asyncHandler(async (req, res) => {
  ...
  if (userRole !== 'BOSS') {
    ...
    delete (filteredSku as any).totalValue
    delete (filteredSku as any).totalCost
...
router.post('/:id/sell', authenticate_token, asyncHandler(async (req, res) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    // 1. 查找或创建客户
    let customer = await tx.customer.find_unique({
...
router.get('/:id/history', authenticate_token, asyncHandler(async (req, res) => {
  ...
  const [logs, total] = await Promise.all([
    prisma.sku_inventory_log.find_many({
      where: { sku_id: id },
      skip,
      take: Number(limit),
...
router.get('/stats/overview', authenticate_token, asyncHandler(async (req, res) => {
  ...
  // 获取低库存SKU（可售数量 <= 1）
  const lowStockSkus = await prisma.product_sku.find_many({
    where: {
      status: 'ACTIVE',
      available_quantity: {
        lte: 1
...
router.get('/:id/trace', authenticate_token, asyncHandler(async (req, res) => {
  ...
  for (const product of sku.products) {
    // 只处理第一次制作时的原材料使用记录
    for (const materialUsage of product.material_usages) {
      ...
      const materialBeads = materialUsage.quantity_used || 0
      const materialPieces = materialUsage.quantity_used || 0
...
router.get('/:id/materials', authenticate_token, asyncHandler(async (req, res) => {
  ...
  const firstMaterialUsage = await prisma.materialUsage.find_first({
    where: { 
  ...
  for (const product of sku.products) {
    for (const materialUsage of product.material_usages) {
      ...
      if (purchase && !processedPurchaseIds.has(purchase.id)) {
        ...
        const actualBeads = materialUsage.quantity_used || 0
        ...
        if (purchase) {
          ...
          materials.push({
            ...
            total_cost: materialUsage.total_cost ? parseFloat(materialUsage.total_cost.toString()) : 0
          })
         }
      }
    }
...
router.post('/:id/destroy', authenticate_token, asyncHandler(async (req, res) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (validatedData.return_to_material) {
      ...
      for (const product of sku.products) {
        for (const materialUsage of product.material_usages) {
          material_usages.push(materialUsage)
        }
      }
      ...
      if (validatedData.reason === '拆散重做' && validatedData.selected_materials && validatedData.selected_materials.length > 0) {
        ...
        for (const purchase_id of validatedData.selected_materials!) {
          const materialUsage = material_usages.find(mu => mu.purchase_id === purchase_id)
          if (materialUsage) {
            // 计算单个SKU的配方数量（从MaterialUsage记录中获取）
            const singleSkuBeads = materialUsage.quantity_used || 0
            const singleSkuPieces = materialUsage.quantity_used || 0
        ...
        for (const purchase_id of allPurchaseIds) {
          const materialUsage = material_usages.find(mu => mu.purchase_id === purchase_id)
          if (materialUsage) {
            const returnBeads = (materialUsage.quantity_used || 0) * validatedData.quantity
            const returnPieces = (materialUsage.quantity_used || 0) * validatedData.quantity
...
router.get('/:id/restock-info', authenticate_token, asyncHandler(async (req, res) => {
  ...
  for (const product of sku.products) {for (const materialUsage of product.material_usages) {
      const purchase = materialUsage.purchase
      ...
      if (purchase && !processedPurchaseIds.has(purchase.id)) {
        ...
        const currentMaterialUsage = await prisma.materialUsage.find_first({
          where: { 
            purchase_id: purchase.id,
            product: { sku_id: sku.id
            }
...
router.post('/:id/restock', authenticate_token, asyncHandler(async (req, res) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const firstMaterialUsage = await tx.materialUsage.find_first({
      where: { 
        product: { sku_id: sku.id
        }
      },
    ...
    for (const material of required_materials) {
      ...
      // 根据产品类型创建原材料使用记录
       const purchase = await tx.purchase.find_unique({ where: { id: material.purchase_id } })
  ...
  try {
    ...
    if (!finalSkuCode) {
      ...
      while (!isUnique && attempts < 10) {
        ...
        const existing = await prisma.product_sku.find_unique({
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const material of validatedData.materials) {
        const materialRecord = await tx.material.find_unique({
    ...
    // 获取完整的SKU信息返回
    const skuWithDetails = await prisma.product_sku.find_unique({
      where: { id: result.id },
      include: {
        creator: {
          select: {
