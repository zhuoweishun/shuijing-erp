...
const createUserSchema = z.object({
  user_name: z.string().min(3, '用户名至少3个字符').max(20, '用户名最多20个字符'),
...
const updateUserSchema = z.object({
  user_name: z.string().min(3, '用户名至少3个字符').max(20, '用户名最多20个字符').optional(),
  email: z.string().email('邮箱格式不正确').optional(),
...
// 获取用户列表（仅老板权限）
router.get('/', authenticate_token, require_role(['BOSS']), asyncHandler(async (req, res) => {
  try {
    const { page = 1, limit = 10, role, active } = req.query
    
    const page_num = Math.max(1, parseInt(page as string))
    const limitNum = Math.min(100, Math.max(1, parseInt(limit as string)))
    const skip = (page_num - 1) * limitNum
...
// 获取用户资料
router.get('/profile', authenticate_token, asyncHandler(async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(404).json(
        ErrorResponses.record_not_found('用户不存在')
      )
    }
...
// 更新用户资料
router.put('/profile', authenticate_token, asyncHandler(async (req, res) => {
  try {
    const user_id = req.user?.id
    if (!user_id) {
      return res.status(401).json({
...
// 创建用户（仅老板权限）
router.post('/', authenticate_token, require_role(['BOSS']), asyncHandler(async (req, res) => {
  try {
    const validatedData = createUserSchema.parse(req.body)

    // 检查用户名是否已存在
    const existingUser = await prisma.user.find_unique({
      where: { user_name: validatedData.user_name }
    })
    ...
    if (validatedData.email) {
      const existingEmail = await prisma.user.find_unique({
...
// 更新用户（仅老板权限）
router.put('/:id', authenticate_token, require_role(['BOSS']), asyncHandler(async (req, res) => {
  try {
    const { id } = req.params
    ...
    if (validatedData.user_name) {
      const usernameExists = await prisma.user.find_first({
        where: {
          user_name: validatedData.user_name,
          id: { not: id }
        }
    ...
    // 检查邮箱是否已被其他用户使用
    if (validatedData.email) {
      const emailExists = await prisma.user.find_first({
        where: {
          email: validatedData.email,
          id: { not: id }
        }
    ...
    const updatedUser = await prisma.user.update({
      where: { id },
      data: {
        ...(validatedData.user_name && { user_name: validatedData.user_name }),
        ...(validatedData.email !== undefined && { email: validatedData.email }),
        ...(validatedData.name && { name: validatedData.name }),
        ...(validatedData.phone !== undefined && { phone: validatedData.phone }),
...
// 删除用户（仅老板权限）
router.delete('/:id', authenticate_token, require_role(['BOSS']), asyncHandler(async (req, res) => {
  try {
    const { id } = req.params
    ...
    if (purchase_count > 0 || productCount > 0) {
      return res.status(400).json({
        success: false,
