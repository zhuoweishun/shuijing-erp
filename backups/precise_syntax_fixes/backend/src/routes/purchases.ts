  ...
  if (sort_by && sort_order) {
    ...
    if (field && (sort_order === 'asc' || sort_order === 'desc')) {
      if (field === 'supplier.name') {
        ...
        const rawQuery = `
           SELECT p.*, s.name as supplier_name, u.id as user_id, u.name as userName, u.user_name as user_username
           FROM purchases p
        ...
        const rawPurchases = await prisma.$query_raw_unsafe(
          rawQuery,
          Number(limit),
...
router.post('/', authenticate_token, asyncHandler(async (req, res) => {
  ...
  while (!isUnique && attempts < 10) {
    ...
    const existing = await prisma.purchase.find_unique({
  ...
  if (validatedData.material_type === 'LOOSE_BEADS') {
    ...
    if (validatedData.piece_count && validatedData.total_price) {
      ...
      unit_price = pricePerPiece // 对于饰品配件，单价就是每片价格
    }
  } else if (validatedData.material_type === 'FINISHED') {
    // 成品：按件计算
    if (validatedData.piece_count && validatedData.total_price) {
      pricePerPiece = validatedData.total_price / validatedData.piece_count
      unit_price = pricePerPiece // 对于成品，单价就是每件价格
    }
  }
  ...
  if (validatedData.supplier_name) {
    // 查找现有供应商
    let supplier = await prisma.supplier.find_first({
      where: {
...
router.put('/:id', authenticate_token, asyncHandler(async (req, res) => {
  ...
  // 检查采购记录是否存在
  const existingPurchase = await prisma.purchase.find_unique({
    where: { id },
    include: {
      supplier: true,
      user: {
  ...
  if (fieldChanges.length > 0) {
    ...
    const timeStr = currentTime.to_locale_string('zh-CN', {
      year: 'numeric',
      month: '2-digit',
      day: '2-digit',
      hour: '2-digit',
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.edit_log.create({
        data: {
          ...
          user_id: req.user!.id,
          action: 'DELETE',
          details: `老板 ${userName} 在 ${timeStr} 删除了采购记录：${existingPurchase.product_name}（采购编号：${existingPurchase.purchase_code}）。该操作同时清理了相关库存数据和财务记录。`,
          changedFields: [{
            field: 'deleted',
            displayName: '删除操作',
            oldValue: '存在',
      ...
      // 删除相关的财务支出记录
      await tx.financial_record.delete_many({
        where: {
          source_type: 'PURCHASE',
          source_id: id,
          type: 'EXPENSE'
...
router.get('/export/excel', authenticate_token, asyncHandler(async (req, res) => {
  ...
  if (start_date || end_date) {
    ...
    if (end_date) {
      ...
      endDateObj.set_hours(23, 59, 59, 999)
  ...
  try {
    ...
    const worksheet = workbook.add_worksheet('采购记录')
    ...
    // 设置标题行样式
    const headerRow = worksheet.get_row(1)
    ...
    // 设置响应头
    const fileName = `采购记录_${new Date().toISOString().slice(0, 10)}.xlsx`
...
router.post('/:id/convert-to-material', authenticate_token, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    while (!isUnique && attempts < 10) {
      ...
      const randomNum = Math.floor(Math.random() * 1000000).toString().padStart(6, '0')
      materialCode = `MAT${date_str}${randomNum}`
      
      const existing = await prisma.material.find_unique({
    ...
    switch (purchase.material_type) {
      ...
      case 'BRACELET':
        ...
        unit = '条'
        unitCost = purchase.unit_price ? parseFloat(purchase.unit_price.toString()) : 0
        break
      case 'ACCESSORIES':
        total_quantity = purchase.piece_count || 0
        unit = '片'
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.material_usage.create({
        data: {
          ...
          quantity_used: total_quantity, unit_cost,
          total_cost,
          action: 'CREATE',
    ...
    // 获取完整的原材料信息返回
    const materialWithDetails = await prisma.material.find_unique({
      where: { id: result.id },
      include: {
        purchase: {
          select: {
