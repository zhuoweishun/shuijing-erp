...
router.get('/', authenticate_token, asyncHandler(async (req, res) => {
  ...
  const suppliers = await prisma.supplier.find_many({
    where,
    orderBy: {
      name: 'asc'
    },
...
router.get('/debug/count', authenticate_token, asyncHandler(async (req, res) => {
  ...
  // 获取所有活跃供应商的详细信息
  const allActiveSuppliers = await prisma.supplier.find_many({
...
router.post('/', authenticate_token, asyncHandler(async (req, res) => {
  ...
  // 数据一致性检查：确保供应商名称唯一性
  const existingSupplier = await prisma.supplier.find_first({
    where: {
      name: validatedData.name,
      is_active: true
    }
  ...
  const normalizedName = validatedData.name.to_lower_case().trim()
  const similarSuppliers = await prisma.supplier.find_many({
    where: {
      is_active: true
    }
  })
  ...
  const conflictingSupplier = similarSuppliers.find(s => 
    s.name.to_lower_case().trim() === normalizedName && s.name !== validatedData.name
  )
