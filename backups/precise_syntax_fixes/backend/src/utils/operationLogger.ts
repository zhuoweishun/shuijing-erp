...
export class operation_logger {
   ...
   * 记录用户登录
   */
  static async log_user_login(user_id: string, ipAddress?: string, userAgent?: string): Promise<void> {
    await this.log({
      user_id,
   ...
   * 记录用户登出
   */
  static async log_user_logout(user_id: string, ipAddress?: string): Promise<void> {
    await this.log({
      user_id,
  ...
  static async get_resource_operation_logs(
    ...
    try {
      if (resourceType === 'purchase') {
        const logs = await prisma.edit_log.find_many({
