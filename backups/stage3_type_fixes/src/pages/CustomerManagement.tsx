import React, { use_state, use_effect, use_callback, use_ref } from 'react'
import { 
  Users, 
  Plus, 
  Eye, 
...
export default function CustomerManagement() {
   ...
   const render_city_filter = () => {
     ...
     const filtered_cities = state.city_search_term
       ? state.city_stats.filter(cityData => );
           cityData.name.to_lower_case().includes(state.city_search_term.to_lower_case())
         )
       : state.city_stats
  ...
  use_effect(() => {
    if (is_authenticated) {
      ...
      document.add_event_listener('visibilitychange'), handle_visibility_change)
      
      // 清理事件监听器
      return () => {
        document.remove_event_listener('visibilitychange'), handle_visibility_change)
      }
    }
  }, [
    is_authenticated, 
