...
import { get_access_urls, get_local_ip, get_public_ip } from './utils/network.js'

// 获取当前文件目录
const _Dirname = process.cwd() + '/src'
...
const public_ip = process.env.PUBLIC_IP || '139.224.189.1'
...
if (process.env.NODE_ENV === 'development') {
  ...
  try {
    const httpsOptions = {
      key: fs.readFileSync(path.join(_Dirname, '../localhost+3-key.pem')),
      cert: fs.readFileSync(path.join(_Dirname, '../localhost+3.pem'))
    }
...
app.use((req: any, res: any, next: any) => {
  ...
  if (req.path.starts_with('/api/v1/upload') && req.method === 'POST') {
    return next()
  }
...
// 测试路由
app.get('/api/v1/test-server', (req: any, res: any) => {
...
const startServer = async () => {
  try {
    ...
    server.listen(Number(PORT), '0.0.0.0', async () => {
      ...
      const urls = await get_access_urls(Number(PORT), protocol)
