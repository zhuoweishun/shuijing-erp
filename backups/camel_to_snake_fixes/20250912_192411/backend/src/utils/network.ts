...
// 验证IP地址格式
export const is_validIP = (ip: string): boolean => {
  const ipRegex = /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/
  return ipRegex.test(ip)
}

// 获取所有可用的访问地址
export const get_access_urls = async (port: number = 3001, protocol: string = 'http'): Promise<{
  local: string
  network: string
  public: string
  domain?: string
...
// 获取推荐的端口
export const get_available_port = async (startPort: number = 3001): Promise<number> => {
