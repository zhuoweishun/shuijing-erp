...
import { chatWithAssistant, get_business_insights } from '../services/ai.js'
import { z } from 'zod'
...
// 智能助理对话
router.post('/chat', authenticate_token, asyncHandler(async (req, res) => {
  try {
...
// 获取业务洞察（仅老板权限）
router.post('/insights', authenticate_token, require_role(['BOSS']), asyncHandler(async (req, res) => {
  try {
    const validatedData = insightsRequestSchema.parse(req.body)
    const { query, timeRange, includeFinancial } = validatedData

    const result = await get_business_insights(query, {
      timeRange,
      includeFinancial,
