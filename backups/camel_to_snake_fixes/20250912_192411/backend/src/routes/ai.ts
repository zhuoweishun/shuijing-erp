...
import { parseCrystalPurchaseDescription, checkAIHealth, get_ai_config } from '../services/ai.js'
import { z } from 'zod'
...
// 解析水晶采购描述
router.post('/parse-crystal-purchase', authenticate_token, asyncHandler(async (req, res) => {
  // 验证请求数据
...
// 解析采购描述（保持向后兼容）
router.post('/parse-description', authenticate_token, asyncHandler(async (req, res) => {
  // 重定向到新的水晶采购解析接口
  const validatedData = parseDescriptionSchema.parse(req.body)
  const result = await parseCrystalPurchaseDescription(validatedData.description)
