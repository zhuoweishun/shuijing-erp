...
const calculateRemainingQuantity = async (purchase: any, tx: any) => {
  ...
  switch (purchase.material_type) {
    case 'LOOSE_BEADS':
      original_quantity = purchase.piece_count || 0
      remaining_quantity = original_quantity - netUsedBeads
      break
    case 'BRACELET':
      original_quantity = purchase.quantity || 0
      remaining_quantity = original_quantity - netUsedBeads
      break
    case 'ACCESSORIES':
    case 'FINISHED':
      original_quantity = purchase.piece_count || purchase.total_beads || 0
      // FINISHED类型需要同时计算两个字段，因为退回记录可能存储在quantityUsed中
      const netUsedTotal = netUsedBeads + netUsedPieces
      remaining_quantity = original_quantity - netUsedTotal
      break
    default:
      // 对于其他类型，使用总颗数或片数，优先使用totalBeads
      original_quantity = purchase.total_beads || purchase.piece_count || purchase.quantity || 0
      // 计算净使用量（正数表示消耗，负数表示退回）
      const netUsed = netUsedBeads + netUsedPieces
      remaining_quantity = original_quantity - netUsed
  }
  
  return Math.max(0, remaining_quantity)
}
...
router.post('/:id/sell', authenticate_token, asyncHandler(async (req, res) => {
  ...
  const validatedData = sellSchema.parse({
    ...
    customer_address: customer_address ? String(customer_address) : undefined,
...
router.get('/:id/trace', authenticate_token, asyncHandler(async (req, res) => {
  ...
  // 按原材料名称排序
  recipeData.sort((a, b) => a.material_name.localeCompare(b.material_name))
  ...
  for (const product of sku.products) {
    for (const material_usage of product.material_usages) {
      ...
      if (purchase && !processedPurchaseIds.has(purchase.id)) {
        ...
        const actualPieces = material_usage.quantity_used || 0
        ...
        if (purchase) {
          switch (purchase.material_type) {
            ...
            case 'FINISHED':
              ...
              break
            default:
              unit_cost = purchase.unit_price ? parseFloat(purchase.unit_price.toString()) : 
          ...
          materials.push({
            ...
            unit_cost: parseFloat(purchase.unit_price?.toString() || '0'), // 使用Purchase表中的正确价格字段
...
router.post('/:id/destroy', authenticate_token, asyncHandler(async (req, res) => {
  ...
  console.log('🔍 [销毁API] 接收到的原始数据:', {
    ...
    return_to_material: typeof return_to_material,
  ...
  try {
    validatedData = destroySchema.parse({
      ...
      return_to_material: Boolean(return_to_material),
      selected_materials: selected_materials || [],
  ...
  const result = await prisma.$transaction(async (tx) => {
          ...
          if (material_usage) {
            ...
            if (returnBeads > 0 || returnPieces > 0) {
               ...
               if (purchase_id) {
                 await tx.material_usage.create({
                   data: {
                     material_id: purchase_id || '', // 使用purchase_id作为material_id，确保非null
                     purchase_id: purchase_id,
                     sku_id: id,
                     quantity_used: -(returnBeads || returnPieces), // 负数表示退回
                     unit_cost: 0,
    ...
    return { sku: updated_sku, returned_materials: returned_materials }
  })
...
router.get('/:id/restock-info', authenticate_token, asyncHandler(async (req, res) => {
  ...
  for (const product of sku.products) {for (const material_usage of product.material_usages) {
      ...
      if (purchase && !processedPurchaseIds.has(purchase.id)) {
        ...
        const isSufficient = isAvailable && remaining_quantity >= quantityNeeded
  ...
  // 检查是否所有原材料都充足
  const can_restock = required_materials.length > 0 && required_materials.every(material => material.isSufficient)
  const insufficient_materials = required_materials
...
router.post('/:id/restock', authenticate_token, asyncHandler(async (req, res) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    // 4. 计算总成本
    const material_cost = required_materials.reduce((sum, material) => {
...
router.put('/:id/control', authenticate_token, asyncHandler(async (req, res) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (validatedData.action === 'price') {
      ...
      log_message = `调整售价：¥${oldPrice.toFixed(2)} → ¥${newPrice.toFixed(2)}，原因：${validatedData.reason}`
      ...
      log_message = `状态变更：${statusText[oldStatus]} → ${statusText[newStatus]}，原因：${validatedData.reason}`
