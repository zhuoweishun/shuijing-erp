...
import { z } from 'zod'
import { quality_schema, material_type_schema } from '../utils/validation.js'
...
const filterInventoryData = (inventory: any[], user_role: string) => {
...
router.get('/hierarchical', authenticate_token, asyncHandler(async (req, res) => {// 验证查询参数
  ...
  try {// 查询所有库存数据
    ...
    allInventory.forEach((item: any) => {
      ...
      if (low_stock_only && Number(item.is_low_stock) !== 1) return
      ...
      level3.remaining_quantity += remaining_quantity
      ...
      // 向上累加统计
      level2.total_quantity += remaining_quantity
      level2.has_low_stock = level2.has_low_stock || is_low_stock
      
      level1.total_quantity += remaining_quantity
...
router.get('/grouped', authenticate_token, asyncHandler(async (req, res) => {// 验证查询参数
  ...
  try {// 查询分组库存数据 - 使用两步查询避免JSON_ARRAYAGG兼容性问题
    ...
    const processedGroups = await Promise.all(
      groupedInventory.map(async (group) => {// 查询该产品的所有采购记录（用于合并相同规格品相的变体）
        ...
        purchases.forEach((purchase: any) => {
          ...
          const original_beads = Number(purchase.originalBeads)
          const remaining_beads = Number(purchase.remainingBeads)
          
          // 累加库存数量
          variant.remaining_beads += remaining_beads
          variant.totalOriginalBeads += original_beads
          ...
          variant.batches.push({
            ...
            originalBeads: original_beads,
            used_beads: Number(purchase.used_beads),
            remaining_beads: remaining_beads,
    ...
    console.log('📤 [分组库存查询] 即将发送的响应数据:', {
      ...
      dataKeys: Object.keys(response_data.data),
      groupsLength: response_data.data.groups.length,
      paginationInfo: response_data.data.pagination,
...
router.get('/finished-products-cards', authenticate_token, asyncHandler(async (req, res) => {// 验证查询参数
  ...
  try {
    ...
    const finishedProductsQuery = `
      ...
      FROM purchases p
      LEFT JOIN (
        SELECT purchase_id, SUM(quantity_used) as usedQuantity
        FROM material_usage
        GROUP BY purchase_id
      ) mu ON p.id = mu.purchase_id
      LEFT JOIN suppliers s ON p.supplier_id = s.id
...
router.get('/statistics', authenticate_token, asyncHandler(async (req, res) => {
  ...
  try {// 修复的统计查询，避免嵌套聚合函数
    ...
    console.log('📊 [库存统计] 响应数据:', response_data)

    // 使用convertToApiFormat确保字段格式符合API规范
    const convertedData = convertToApiFormat(response_data)
...
router.get('/product-distribution', authenticate_token, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    console.log('📊 [产品分布] 响应数据:', response_data)
...
router.get('/consumption-analysis', authenticate_token, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const consumptionData = await prisma.$queryRawUnsafe(consumptionQuery, parseInt(limit as string)) as any[]
    ...
    console.log('📊 [库存消耗分析] 响应数据:', response_data)

    // 权限过滤（雇员不能查看供应商和价格信息）
    if (req.user!.role === 'EMPLOYEE') {
      response_data.topConsumedProducts = response_data.topConsumedProducts.map(item => ({
        ...item,
        supplier_name: null
      }))
    }
...
router.get('/price-distribution', authenticate_token, asyncHandler(async (req, res) => {
  ...
  try {
    // 构建产品类型筛选条件
    let material_type_condition = ''
    if (material_type && material_type !== 'ALL') {
      material_type_condition = `AND p.material_type = '${material_type}'`
    }
    ...
    if (price_type === 'unit_price') {
      ...
      console.log('📊 [单价区间分布] 响应数据:', response_data)
    ...
    const priceData = await prisma.$queryRawUnsafe(priceQuery, parseInt(limit as string)) as any[]
    ...
    console.log('📊 [产品价格分布] 响应数据:', response_data)

    // 权限过滤（雇员不能查看供应商和价格信息）
    if (req.user!.role === 'EMPLOYEE') {
      response_data.topPriceProducts = response_data.topPriceProducts.map(item => ({
        ...item,
        supplier_name: null,
        unit_price: null,
        total_price: null,
      ...
      response_data.avg_price = 0
      response_data.maxPrice = 0
      response_data.minPrice = 0
    }
...
router.get('/material-distribution', authenticate_token, asyncHandler(async (req, res) => {
  try {
    ...
    const distributionQuery = `
      ...
      FROM purchases p
      LEFT JOIN (
        SELECT purchase_id, SUM(quantity_used) as total_used
        FROM material_usage
        GROUP BY purchase_id
    ...
    const result = await prisma.$queryRawUnsafe(distributionQuery, ...queryParams) as any[]
