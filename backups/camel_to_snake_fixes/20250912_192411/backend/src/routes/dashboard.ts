...
router.get('/', authenticate_token, asyncHandler(async (req, res) => {
  try {
    ...
    // 5. 查询最近的采购记录（最近5条）
    const recentPurchasesData = await prisma.purchase.findMany({
      where: {
        status: 'ACTIVE'
      },
      orderBy: {
    ...
    // 6. 查询最近的原材料记录（最近5条）
    const recentMaterialsData = await prisma.material.findMany({
      where: {
        status: 'ACTIVE'
      },
      orderBy: {
    ...
    // 7. 查询供应商统计数据
    const supplierStatsData = await prisma.supplier.findMany({
      where: {
        is_active: true
      },
      select: {
