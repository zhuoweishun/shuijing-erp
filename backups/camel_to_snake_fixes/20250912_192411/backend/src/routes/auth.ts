...
router.post('/login', asyncHandler(async (req, res) => {
  ...
  if (!user_name || !password) {
    return res.status(400).json(
      ErrorResponses.bad_request('用户名和密码不能为空')
    )
  }
  ...
  try {
    ...
    // 记录登录日志
    logger.info(`用户登录成功: ${user.user_name}`, {
      user_id: user.id,
      ip: req.ip,
    ...
    // 记录操作日志
    await operation_logger.log_user_login(
      user.id,
      req.ip,
      req.get('User-Agent')
    )
...
router.post('/register', asyncHandler(async (req, res) => {
  // TODO: 实现用户注册逻辑
  res.json(
    ErrorResponses.bad_request('注册功能正在开发中...')
  )
}))
...
router.get('/verify', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // 检查decoded中是否包含userId
    if (!decoded.user_id) {
      return res.status(401).json(
        ErrorResponses.unauthorized('Token格式无效')
      )
    }
    ...
    res.status(401).json(
      ErrorResponses.invalid_token('Token无效或已过期')
    )
    return
  }
}))
...
router.post('/refresh', asyncHandler(async (req, res) => {
  // TODO: 实现token刷新逻辑
  res.json(
    ErrorResponses.bad_request('Token刷新功能正在开发中...')
  )
}))

// 用户登出
router.post('/logout', authenticate_token, asyncHandler(async (req, res) => {
  // 记录登出操作日志
  await operation_logger.log_user_logout(
    req.user!.id,
    req.ip
  )
