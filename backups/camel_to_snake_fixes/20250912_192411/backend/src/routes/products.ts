...
router.get('/materials', authenticate_token, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    let material_types_array: string[] = []
    if (material_types) {  // 前端传入的参数名保持兼容
      if (typeof material_types === 'string') {
        // 如果是逗号分隔的字符串，分割成数组
        material_types_array = material_types.split(',').map(type => type.trim()).filter(Boolean)
      } else if (Array.isArray(material_types)) {
        material_types_array = material_types as string[]
      }
    }
    
    if (material_types_array.length > 0) {
      const placeholders = material_types_array.map(() => '?').join(',')
      whereClause += ` AND p.material_type IN (${placeholders})`  // 数据库字段保持不变
      params.push(...material_types_array)
    }
...
router.post('/', authenticate_token, asyncHandler(async (req, res) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    // 查找或创建SKU
    // 计算SKU规格（从原材料推导）
    let sku_specification = null;
    if (material_usages_for_sku.length > 0) {
      const first_material = material_usages_for_sku[0].purchase;
      if (first_material.material_type === 'LOOSE_BEADS' || first_material.material_type === 'BRACELET') {
        // 散珠和手串优先使用bead_diameter
        if (first_material.bead_diameter) {
          sku_specification = `${first_material.bead_diameter}mm`;
        } else if (first_material.specification) {
          sku_specification = `${first_material.specification}mm`;
        }
      } else if (first_material.material_type === 'ACCESSORIES' || first_material.material_type === 'FINISHED') {
        // 饰品配件和成品优先使用specification
        if (first_material.specification) {
          sku_specification = `${first_material.specification}mm`;
        } else if (first_material.bead_diameter) {
          sku_specification = `${first_material.bead_diameter}mm`;
        }
      }
    }

    const sku_result = await find_or_create_sku({
      material_usages: material_usages_for_sku,
      ...
      additional_data: {
        ...
        specification: sku_specification,
        material_cost: totalMaterialCost,
            labor_cost: Number(labor_cost),
            craft_cost: Number(craft_cost),
        total_cost: total_cost,
    ...
    const sku = (sku_result as any).sku
     const is_new_sku = (sku_result as any).is_new_sku
    ...
    for (const material of materials) {
      const used_beads = material.quantity_used_beads || 0
      const used_pieces = material.quantity_used_pieces || 0
      
      if (used_beads > 0 || used_pieces > 0) {
        await tx.material_usage.create({
          data: {
            material_id: material.purchase_id,
            product_id: product.id,
            quantity_used: used_beads || used_pieces,
            action: 'USE'
          }
        })
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    try {
      ...
      const result = await prisma.$transaction(async (tx) => {
        // 验证原材料存在性和库存
        const purchase = await tx.purchase.findUnique({
          where: { id: product_data.material_id },
          include: { material_usages: true }
        })
        ...
        const labor_cost = product_data.labor_cost || 0;
        const craft_cost = product_data.craft_cost || 0;
        const total_cost = Number(material_cost) + Number(labor_cost) + Number(craft_cost);
        
        // 处理图片继承逻辑（直接转化模式）
        let product_images = null;
        if (product_data.photos && product_data.photos.length > 0) {
          // 如果前端传递了图片，使用前端图片
          product_images = JSON.stringify(product_data.photos);
          ...
          if (photosArray.length > 0) {
            product_images = JSON.stringify(photosArray);
          }
        }
        ...
        if (purchase.material_type === 'FINISHED') {
          ...
          if (purchase.specification) {
            sku_specification = `${purchase.specification}mm`;
          } else if (purchase.bead_diameter) {
            sku_specification = `${purchase.bead_diameter}mm`;
          }
        } else if (purchase.material_type === 'FINISHED') {
        ...
        const sku_result = await find_or_create_sku({
          material_usages: material_usages_for_sku,
          product_name: product_data.product_name,  // 成品名称
          selling_price: Number(product_data.selling_price),
          user_id: req.user!.id,
          tx: tx,
          additional_data: {
            ...
            total_cost: total_cost,
            profit_margin: Number(product_data.selling_price) > 0 
              ? ((Number(product_data.selling_price) - total_cost) / Number(product_data.selling_price) * 100)
              : 0
          }
        })
        ...
        const finished_product = await tx.product.create({
          data: {
            product_code: null, // 不再使用单独的成品编号
            name: product_data.product_name,
            description: product_data.description || null,
            unit_price: Number(product_data.selling_price),
            total_value: total_cost,
            unit: '件',
            quantity: 1,
            images: product_images,
            notes: product_data.description || null,
            user_id: req.user!.id,
        ...
        await tx.material_usage.create({
          data: {
            ...
            product_id: finished_product.id,  // 关联到成品
            quantity_used: 1,
            action: 'USE'
          }
        })
        ...
        await create_sku_inventory_log({ sku_id: sku.id,
          ...
          reference_id: finished_product.id,
          notes: `直接转化模式创建成品: ${product_data.product_name}`,
          user_id: req.user!.id,
          tx: tx
        })
        
        return {
          finished_product_id: finished_product.id,  // 成品ID
          sku_code: sku.sku_code,
          sku_id: sku.id,
          is_new_sku: is_new_sku,
          finished_product_name: product_data.product_name,  // 成品名称
          material_cost: Number(material_cost),
          total_cost: total_cost,
          selling_price: Number(product_data.selling_price),
          profit_margin: Number(product_data.selling_price) > 0 
            ? ((Number(product_data.selling_price) - total_cost) / Number(product_data.selling_price) * 100).toFixed(1)
            : '0.0',
          status: 'AVAILABLE',
