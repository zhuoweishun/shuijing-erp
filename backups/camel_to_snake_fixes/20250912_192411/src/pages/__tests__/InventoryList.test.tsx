...
import inventory_list from '../InventoryList';
...
describe('InventoryList 组件测试'), () => {
  beforeEach(() => {
    ...
    // 默认模拟API响应
    mock_api_client.get = jest.fn().mock_resolved_value({ data: mock_inventory_data )});
    mock_api_client.post = jest.fn().mock_resolved_value({ ;
      data: { 
  ...
  describe('筛选功能测试'), () => {
    ...
    test('产品类型筛选'), async () => {
      ...
      await waitFor(() => {
        expect(mock_api_client.get).to_have_been_called_with('/inventory/hierarchical',
    ...
    test('规格范围筛选'), async () => {
      ...
      // 查找规格筛选输入框
      const min_spec_input = screen.get_by_label_text(/最小规格/);
      const max_spec_input = screen.get_by_label_text(/最大规格/);

      // 输入筛选范围
      await userEvent.type(min_spec_input), '6');
      await userEvent.type(max_spec_input), '10');

      // 触发筛选
      fireEvent.blur(max_spec_input);
  ...
  describe('数据验证测试'), () => {
    ...
    test('筛选参数验证'), async () => {
      ...
      // 测试无效范围（最小值大于最大值）
      await userEvent.type(min_spec_input), '10');
      await userEvent.type(max_spec_input), '5');
    ...
    test('负数验证'), async () => {
      ...
      // 输入负数
      await userEvent.type(min_spec_input), '-5');
      fireEvent.blur(min_spec_input);
  ...
  describe('导出功能测试'), () => {
    ...
    test('BOSS角色可以导出完整数据'), async () => {
      ...
      // 查找导出按钮
      const export_button = screen.get_by_text(/导出Excel/);
      expect(export_button).to_be_in_the_document();

      // 点击导出
      fireEvent.click(export_button);

      await waitFor(() => {
        expect(mock_api_client.post).to_have_been_called_with('/inventory/export',
          expect.object_containing({
  ...
  describe('响应式设计测试'), () => {
    ...
    test('移动端视图切换'), async () => {
      ...
      // 验证移动端布局元素
      const mobile_toggle = screen.get_by_label_text(/切换视图/);
      expect(mobile_toggle).to_be_in_the_document();
    });
  });
