...
export default function SkuControlModal({
  ...
  const current_price = parse_float(sku.selling_price?.toString() || sku.unit_price?.toString() || '0');
  const new_price = parse_float(priceForm.new_price) || 0
  // 使用SKU配方成本：material_cost + labor_cost + craft_cost
  const recipe_cost = (parse_float(sku.material_cost?.toString() || '0') + ;
                      parse_float(sku.labor_cost?.toString() || '0') + 
                      parse_float(sku.craft_cost?.toString() || '0'))
