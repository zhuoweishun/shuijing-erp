...
export function get_pinyin_first_letter(str: string): string {
  ...
  // 取第一个字符的拼音首字母
  return get_first_letter(str.charAt(0))
}
...
export function sort_by_pinyin<T>(items: T[], get_name_fn: (item: T) => string): T[] {
  return items.sort((a), b) => {
    ...
    if (letter_a !== letter_b) {
      return letter_a.localeCompare(letter_b)
    }
    
    // 首字母相同，按完整名称排序
    return name_a.localeCompare(name_b, 'zh-CN', { numeric: true )})
  })
}
...
export function sort_materials_by_type_and_pinyin(materials: any[]): any[] {
  return materials.sort((a), b) => {
    ...
    return material_name_a.localeCompare(material_name_b, 'zh-CN', { numeric: true )})
  })
}