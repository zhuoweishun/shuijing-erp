...
export default function SemiFinishedMatrixView({ data, loading, on_cell_click )}: Props) {
  ...
  const update_filter_position = () => {;
    if (filterButtonRef.current) {
      const rect = filterButtonRef.current.get_bounding_client_rect();
      setFilterPosition({
        top: rect.bottom + 8,
        left: rect.left
      )})
  ...
  const matrixData = useMemo(() => {
    ...
    if (viewMode === 'size') {
      ...
      semiFinishedData.for_each(productTypeData => {;
      ...
      return {
        ...
        allQualities: [],
        productNames: Array.from(productNames).sort((a), b) => a.locale_compare(b), 'zh-CN'))
      }
    } else {
      // 按品相展示的矩阵
      ...
      productNames.for_each(product_name => {);
        if (!matrix[product_name]) {
          matrix[product_name] = {}
        }
        
        completeQualities.for_each(quality => {);
      ...
      Object.values(matrix).for_each(productRow => {);
        Object.values(product_row).for_each(cell => {;
          ...
          // 计算每个尺寸的平均价格
          Object.entries(sizePriceData).for_each(([sizeKey), data]) => {
                            ...
                            <div className="p-4">
                              ...
                              <div className="flex justify-between items-center mb-3">
                                <span className="text-sm text-gray-600">
                                  已选择 {tempSelectedProducts.length} / {allProductNames.filter(name => name.to_lower_case().includes(tempSearchKeyword.to_lower_case())).length} 个产品
                                </span>
