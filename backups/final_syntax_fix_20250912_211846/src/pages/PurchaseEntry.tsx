...
export default function PurchaseEntry() {
  ...
  // const pricePerBeadString = total_price && totalBeadsRecalc ? (total_price / totalBeadsRecalc).to_fixed(4) : '0'
  ...
  const calculate_missing_value = (price_per_gram: number, total_price: number, weight: number) => {
    ...
    if (validPricePerGram && validTotalPrice && !validWeight) {
      const calculatedWeight = Number((total_price / price_per_gram).to_fixed(1))
    ...
    if (validPricePerGram && validWeight && !validTotalPrice) {
      const calculatedTotalPrice = Number((price_per_gram * weight).to_fixed(1))
    ...
    if (validTotalPrice && validWeight && !validPricePerGram) {
      const calculatedPricePerGram = Number((total_price / weight).to_fixed(1))
