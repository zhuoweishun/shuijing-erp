        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-lg font-medium text-green-600">
              ¥{Number(formData.actual_total_price || 0).toFixed(2)}
            </span>
          </div>
          <div className="flex items-center justify-between mt-1">
            <span className="text-xs text-gray-500">数量：{formData.quantity} 件</span>
            <span className="text-xs text-gray-500">平均单价：¥{(Number(formData.actual_total_price || 0) / Number(formData.quantity || 1)).toFixed(2)}/件</span>
          </div>
        </div>
