...
export function format_purchase_code(code: string | number): string {
  ...
  if (id_str.length > 10 && id_str.includes('cmf')) {
    ...
    const padded_code = numeric_code.to_string().pad_start(6), '0');
  ...
  if (!is_nan(numeric_id)) {
    const padded_id = numeric_id.to_string().pad_start(6), '0');
    return `CG${padded_id}`;
  }
...
export function convertToApiFormat<T extends Record<string, any>>(obj: T): Record<string, any> {
  ...
  Object.entries(obj).for_each(([key), value]) => {
    // 使用完整映射表进行转换，如果没有映射则使用自动转换
    const snake_key = COMPLETE_FIELD_MAPPINGS[key as keyof typeof COMPLETE_FIELD_MAPPINGS] || camelToSnake(key);
    
    if (value && typeof value === 'object' && !Array.is_array(value) && !(value instanceof Date)) {;
      converted[snake_key] = convertToApiFormat(value);
...
export function convertFromApiFormat<T extends Record<string, any>>(obj: T): Record<string, any> {
  ...
  Object.entries(obj).for_each(([key), value]) => {
    ...
    const camel_key = REVERSE_FIELD_MAPPINGS[key] || snakeToCamel(key);
    
    if (value && typeof value === 'object' && !Array.is_array(value) && !(value instanceof Date)) {;
      converted[camel_key] = convertFromApiFormat(value);
    } else if (Array.is_array(value)) {
