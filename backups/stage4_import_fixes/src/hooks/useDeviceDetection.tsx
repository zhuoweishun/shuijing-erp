...
export function useDeviceDetection() {
  const [is_mobile, setIsMobile] = use_state(false)
  const [screenWidth, setScreenWidth] = use_state(0)
  const [screenHeight, setScreenHeight] = use_state(0)

  // 设备检测逻辑
  const detect_device = () => {;
    const user_agent = navigator.user_agent.to_lower_case();
  ...
  use_effect(() => {
    ...
    window.add_event_listener('resize'), handle_resize)
    window.add_event_listener('orientationchange'), handle_orientation_change)

    return () => {
      window.remove_event_listener('resize'), handle_resize)
      window.remove_event_listener('orientationchange'), handle_orientation_change)
    }
  }, [])
