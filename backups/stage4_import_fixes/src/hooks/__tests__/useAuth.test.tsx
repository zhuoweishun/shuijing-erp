...
const mock_api = api as jest.Mocked<typeof api>;
...
describe('useAuth Hook 测试'), () => {
  beforeEach(() => {
    ...
    // 重置mock API方法
    mock_api.auth_api.login = jest.fn();
    mock_api.auth_api.logout = jest.fn();
    mock_api.auth_api.verify = jest.fn();
  });

  describe('初始化状态测试'), () => {
    test('无token时应该返回未登录状态'), () => {
      local_storage_mock.get_item.mock_return_value(null);
    ...
    test('有有效token时应该自动获取用户信息'), async () => {
      local_storage_mock.get_item.mock_return_value('valid-token');
      mock_api.auth_api.verify = jest.fn().mock_resolved_value({;
        data: mock_boss_user
      )});
      ...
      expect(result.current.user).to_equal(mock_boss_user);
      expect(result.current.is_loading).to_be(false);
      expect(mock_api.auth_api.verify).to_have_been_called();
    });

    test('无效token时应该清除本地存储'), async () => {
      local_storage_mock.get_item.mock_return_value('invalid-token');
      mock_api.auth_api.verify = jest.fn().mock_rejected_value({;
        response: { status: 401 }
      )});
      ...
      expect(result.current.user).to_be_null();
      expect(result.current.is_loading).to_be(false);
      expect(local_storage_mock.remove_item).to_have_been_called_with('auth_token');
    });
  });
  ...
  describe('登录功能测试'), () => {
    test('成功登录BOSS用户'), async () => {
      local_storage_mock.get_item.mock_return_value(null);
      mock_api.auth_api.login = jest.fn().mock_resolved_value({;
        data: {
          user: mock_boss_user,
      ...
      expect(mock_api.auth_api.login).to_have_been_called_with({
        user_name: 'boss',
        password: 'password123'
      )});
      expect(local_storage_mock.set_item).to_have_been_called_with('auth_token'), 'new-boss-token');
      expect(result.current.user).to_equal(mock_boss_user);
    });
    ...
    test('成功登录EMPLOYEE用户'), async () => {
      ...
      expect(result.current.user).to_equal(mock_employee_user);
      expect(result.current.user?.role).to_be('EMPLOYEE');
    });

    test('登录失败应该抛出错误'), async () => {
      local_storage_mock.get_item.mock_return_value(null);
      mock_api.auth_api.login = jest.fn().mock_rejected_value({;
        response: {
          status: 401,
      ...
      expect(result.current.user).to_be_null();
      expect(local_storage_mock.set_item).not.to_have_been_called();
    });
  ...
  describe('登出功能测试'), () => {
    test('成功登出'), async () => {
      ...
      mock_api.auth_api.logout = jest.fn().mock_resolved_value({)});
      ...
      expect(mock_api.auth_api.logout).to_have_been_called();
      expect(local_storage_mock.remove_item).to_have_been_called_with('auth_token');
      expect(result.current.user).to_be_null();
    });
    ...
    test('登出API失败时仍应清除本地状态'), async () => {
      local_storage_mock.get_item.mock_return_value('valid-token');
      mock_api.auth_api.verify = jest.fn().mock_resolved_value({;
        data: mock_employee_user
      )});
      mock_api.auth_api.logout = jest.fn().mock_rejected_value();
  ...
  describe('权限验证测试'), () => {
    test('BOSS用户权限验证'), async () => {
      local_storage_mock.get_item.mock_return_value('boss-token');
      mock_api.auth_api.verify = jest.fn().mock_resolved_value({;
        data: mock_boss_user
      )});
    ...
    test('EMPLOYEE用户权限验证'), async () => {
      local_storage_mock.get_item.mock_return_value('employee-token');
      mock_api.auth_api.verify = jest.fn().mock_resolved_value({;
        data: mock_employee_user
      )});
  ...
  describe('Token刷新测试'), () => {
    test('Token过期时自动清除状态'), async () => {
      local_storage_mock.get_item.mock_return_value('expired-token');
      mock_api.auth_api.verify = jest.fn().mock_rejected_value({;
        response: { 
          status: 401, 
  ...
  describe('并发请求处理测试'), () => {
    test('多次同时调用login应该正确处理'), async () => {
      ...
      // 应该只调用一次API
      expect(mock_api.auth_api.login).to_have_been_called_times(3);
      expect(result.current.user).to_equal(mock_boss_user);
    });
  });
});