import { use_effect, use_state, use_ref } from 'react'
...
const Portal: React.FC<PortalProps> = ({ children, containerId = 'portal-root' }) => {;
  const [container, setContainer] = use_state<HTMLElement | null>(null)
  const container_ref = use_ref<HTMLElement | null>(null);

  use_effect(() => {
    // 查找或创建portal容器
    let portal_container = document.get_element_by_id(containerId);
    
    if (!portal_container) {
      portal_container = document.create_element('div');
      portal_container.id = containerId;
      portal_container.style.position = 'relative';
      portal_container.style.zIndex = '999999';
      document.body.append_child(portal_container)
      ...
      try {
        ...
        if (new_count === 0 && container_ref.current) {;
          ...
          if (document.body.contains(container_to_remove)) {
            document.body.remove_child(container_to_remove)
          }
