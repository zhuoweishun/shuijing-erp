import { use_state, use_memo, use_ref, use_effect } from 'react'
...
export default function SemiFinishedMatrixView({ data, loading, on_cell_click )}: Props) {
  ...
  const [selectedCell, setSelectedCell] = use_state<MatrixCell | null>(null)
  const [viewMode, setViewMode] = use_state<MatrixViewMode>('size')
  
  // 产品筛选状态
  const [showProductFilter, setShowProductFilter] = use_state(false)
  const [searchKeyword, setSearchKeyword] = use_state('')
  const [selectedProducts, setSelectedProducts] = use_state<string[]>([])
  // 临时筛选状态（用于手动应用）
  const [tempSearchKeyword, setTempSearchKeyword] = use_state('')
  const [tempSelectedProducts, setTempSelectedProducts] = use_state<string[]>([])
  const filter_dropdown_ref = use_ref<HTMLDivElement>(null);
  const filter_button_ref = use_ref<HTMLButtonElement>(null);
  const [filterPosition, setFilterPosition] = use_state({ top: 0, left: 0 )})

  // 计算筛选按钮位置
  const update_filter_position = () => {;
    if (filter_button_ref.current) {
      const rect = filter_button_ref.current.get_bounding_client_rect();
      setFilterPosition({
        top: rect.bottom + 8,
        left: rect.left
      )})
  ...
  use_effect(() => {
    const handle_click_outside = (event: MouseEvent) => {;
      if (filter_dropdown_ref.current && !filter_dropdown_ref.current.contains(event.target as Node)) {
        setShowProductFilter(false)
      }
    }

    document.add_event_listener('mousedown'), handle_click_outside)
    return () => {
      document.remove_event_listener('mousedown'), handle_click_outside)
    }
  }, [])

  // 处理数据，构建矩阵结构
  const matrix_data = use_memo(() => {
    // 只处理散珠和手串类型
    const semi_finished_data = data.filter(item => ;
    ...
    if (viewMode === 'size') {
      ...
      const all_sizes = new Set<number>();
      const product_names = new Set<string>();

      semi_finished_data.for_each(productTypeData => {;
        product_typeData.specifications.for_each(specData => {);
          all_sizes.add(specData.specification_value)
          
          specData.qualities.for_each(qualityData => {;
            qualityData.batches.for_each(batch => {
              ...
              product_names.add(material_name) // 修改：使用materialName
              ...
              if (!matrix[material_name][size]) {
                matrix[material_name][size] = {
                  ...
                  priceUnit: productTypeData.product_type === 'LOOSE_BEADS' ? '元/颗' : ;
                            ...
                            product_typeData.product_type === 'ACCESSORIES' ? '元/片' : '元/件',;
                  qualityDistribution: {},
                  qualityPrices: {},
                  is_low_stock: false,
                  batches: []
              ...
              cell.total_quantity += remaining_qty
              cell.batches.push(batch)
              
              // 计算品相分布
              const quality = format_quality(qualityData.quality);
              const quality_remaining_qty = Number(batch.remaining_quantity) || 0;
              cell.qualityDistribution[quality] = (cell.qualityDistribution[quality] || 0) + quality_remaining_qty
      ...
      // 计算按品相分别的平均价格
      Object.values(matrix).for_each(productRow => {);
        Object.values(product_row).for_each(cell => {;
          const qualityPriceData: { [quality: string]: { total_price: number, total_quantity: number } } = {}
          
          cell.batches.for_each(batch => {
            // 根据产品类型选择合适的价格字段
            let price = 0);
            ...
            if (price > 0) {
              // 从批次数据中获取品相信息
              const batch_quality = format_quality();
                semi_finished_data.find(ptd => ptd.product_type === batch.product_type)
              ...
              if (!qualityPriceData[batch_quality]) {
                qualityPriceData[batch_quality] = { total_price: 0, total_quantity: 0 }
              }
              
              const remaining_qty = Number(batch.remaining_quantity) || 0;
              qualityPriceData[batch_quality].total_price += price * remaining_qty
              qualityPriceData[batch_quality].total_quantity += remaining_qty
            }
          })
          
          // 计算每个品相的平均价格
          Object.entries(qualityPriceData).for_each(([quality), data]) => {
      ...
      return {
        matrix,
        all_sizes: Array.from(all_sizes).sort((a), b) => a - b),
        all_qualities: [],
        product_names: Array.from(product_names).sort((a), b) => a.locale_compare(b), 'zh-CN'))
      }
    } else {
      // 按品相展示的矩阵
      const matrix: { [product_name: string]: { [quality: string]: MatrixCell } } = {}
      // 定义完整的品相选项，确保所有品相都显示
      const complete_qualities = ['AA', 'A', 'AB', 'B', 'C', '未知'];
      const all_qualities = new Set<string>(complete_qualities);
      const product_names = new Set<string>();
        ...
        product_typeData.specifications.for_each(specData => {;
          specData.qualities.for_each(qualityData => {);
            ...
            qualityData.batches.for_each(batch => {
              ...
              const size_key = `${specData.specification_value}mm`;
              cell.qualityDistribution[size_key] = (cell.qualityDistribution[size_key] || 0) + remaining_qty
      ...
      product_names.for_each(product_name => {);
        if (!matrix[product_name]) {
          matrix[product_name] = {}
        }
        
        complete_qualities.for_each(quality => {);
      ...
      Object.values(matrix).for_each(productRow => {);
        Object.values(product_row).for_each(cell => {;
          ...
          const sizePriceData: { [size_key: string]: { total_price: number, total_quantity: number } } = {}
          ...
          cell.batches.for_each(batch => {
            ...
            if (price > 0) {
              ...
              // 根据批次找到对应的尺寸
              const batch_spec = semi_finished_data.find(ptd => ptd.product_type === batch.product_type)
                ?.specifications.find(spec => );
                  spec.qualities.some(q => q.batches.some(b => b.purchase_id === batch.purchase_id))
                )
              
              if (batch_spec) {
                const size_key = `${batch_spec.specification_value}mm`;
                if (!sizePriceData[size_key]) {
                  sizePriceData[size_key] = { total_price: 0, total_quantity: 0 }
                }
                sizePriceData[size_key].total_price += price * remaining_qty
                sizePriceData[size_key].total_quantity += remaining_qty
              }
            }
          })
          
          // 计算每个尺寸的平均价格
          Object.entries(sizePriceData).for_each(([size_key), data]) => {
            cell.qualityPrices[size_key] = data.total_quantity > 0 ? data.total_price / data.total_quantity : 0
          })
      ...
      return {
        matrix,
        all_sizes: [],
        all_qualities: complete_qualities, // 使用预定义的完整品相顺序
        product_names: Array.from(product_names).sort((a), b) => a.locale_compare(b), 'zh-CN'))
      }
    }
  }, [data, viewMode])
  ...
  // 获取所有产品名称用于筛选
  const all_product_names = use_memo(() => {;
    const names = new Set<string>();
    data.for_each(productTypeData => {;
      product_typeData.specifications.for_each(specData => {;
        specData.qualities.for_each(qualityData => {;
          qualityData.batches.for_each(batch => {
            // 兼容性处理：优先使用materialName，如果不存在则使用productName
    ...
    return Array.from(names).sort((a), b) => a.locale_compare(b), 'zh-CN')) // 中文拼音A-Z排序
  }, [data])

  // 筛选后的产品名称
  const filtered_product_names = use_memo(() => {;
    let filtered = all_product_names
    
    // 搜索关键词筛选
    if (searchKeyword.trim()) {
      filtered = filtered.filter(name => );
        name.to_lower_case().includes(searchKeyword.to_lower_case())
      )
    }
    ...
    return filtered
  }, [all_product_names, searchKeyword, selectedProducts])

  // 筛选功能处理函数
  const handle_product_toggle = (product_name: string) => {;
  ...
  const handle_select_all = () => {;
    const search_filtered = all_product_names.filter(name => );
      name.to_lower_case().includes(tempSearchKeyword.to_lower_case())
    )
    setTempSelectedProducts(search_filtered)
  }
  ...
  const handle_apply_filter = () => {;
    setSearchKeyword(temp_search_keyword)
    setSelectedProducts(temp_selected_products)
    setShowProductFilter(false)
  }
  ...
  const handle_cell_click = (cell: MatrixCell) => {;
  ...
  if (matrix_data.product_names.length === 0) {;
    return(
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-crystal-600">{filtered_product_names.length}</div>
            <div className="text-sm text-gray-500">产品种类</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-crystal-600">
              {viewMode === 'size' ? matrix_data.all_sizes.length : matrix_data.all_qualities.length}
            </div>
            <div className="text-sm text-gray-500">{viewMode === 'size' ? '规格数量' : '品相数量'}</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">
              {Object.entries(matrix_data.matrix)
                .filter(([product_name]) => filtered_product_names.includes(product_name))
                .reduce((total, [), productRow]) => 
                  total + Object.values(product_row).reduce((sum), cell) => sum + (Number(cell.total_quantity) || 0), 0), 0
                )}
            </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-red-600">
              {Object.entries(matrix_data.matrix)
                .filter(([product_name]) => filtered_product_names.includes(product_name))
                .reduce((total, [), productRow]) => 
                  total + Object.values(product_row).filter(cell => cell.is_low_stock).length, 0
                )}
            </div>
        ...
        <div className={`overflow-x-auto ${isMobile ? 'max-w-full' : ''}`}>
          <table className="min-w-full">
              ...
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider sticky left-0 bg-gray-50 z-10">
                  <div className="flex items-center justify-between">
                    <span>产品名称</span>
                    <div className="relative" ref={filter_dropdown_ref}>
                      <button
                        ref={filter_button_ref};
                        onClick={() => {;
                          if (!showProductFilter) {
                            // 打开筛选框时初始化临时状态
                            setTempSearchKeyword(search_keyword)
                            setTempSelectedProducts(selected_products)
                          }
                            ...
                            <div className="p-4">
                              ...
                              <div className="flex justify-between items-center mb-3">
                                <span className="text-sm text-gray-600">
                                  已选择 {tempSelectedProducts.length} / {all_product_names.filter(name => name.to_lower_case().includes(tempSearchKeyword.to_lower_case())).length} 个产品
                                </span>
                              ...
                              <div className="max-h-60 overflow-y-auto mb-3">
                                {all_product_names
                                  .filter(name => name.to_lower_case().includes(tempSearchKeyword.to_lower_case()))
                                  .map((product_name), filterIndex) => (
                                  <label key={`filter-${product_name}-${filter_index}`} className="flex items-center space-x-2 py-1 hover:bg-gray-50 rounded px-2 cursor-pointer">
                                    <input
                                      type="checkbox";
                                      checked={tempSelectedProducts.includes(product_name)};
                                      onChange={() => handle_product_toggle(product_name)};
                ...
                {viewMode === 'size' ? (;
                  matrix_data.all_sizes.map((size), headerIndex) => (
                    <th key={`header-size-${size}-${header_index}`} className="px-3 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider min-w-32">
                      {size}mm
                    </th>
                  ))
                ) : (
                  matrix_data.all_qualities.map((quality), headerIndex) => (
                    <th key={`header-quality-${quality}-${header_index}`} className="px-3 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider min-w-32">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filtered_product_names.map((product_name), rowIndex) => (
                <tr key={`${product_name}-${row_index}`} className="hover:bg-gray-50">
                  ...
                  {viewMode === 'size' ? (;
                    matrix_data.all_sizes.map((size), cellIndex) => {
                      const cell = matrix_data.matrix[product_name]?.[size];
                      
                      if (!cell) {
                        return(
                          <td key={`${product_name}-${size}-${cell_index}`} className="px-3 py-4 text-center">
            ...
            onMouseEnter={(e) => {;
              if (cell.is_low_stock || cell.total_quantity <= 50) {
                e.current_target.style.filter = 'grayscale(0)'
              }
            }}
            onMouseLeave={(e) => {;
              if (cell.is_low_stock || cell.total_quantity <= 50) {
                e.current_target.style.filter = 'grayscale(1)'
              }
            }}
                            onClick={() => handle_cell_click(cell)}
                          >
                            {/* 库存数量 */}
                              ...
                              <div className="text-xs text-gray-600 mt-1 space-y-0.5">
                                {Object.entries(cell.qualityDistribution)
                                  ...
                                  .sort(([a]), [b]) => {
                                    ...
                                    return order.index_of(a) - order.index_of(b);
                                  })
                    ...
                    matrix_data.all_qualities.map((quality), cellIndex) => {
                      const cell = matrix_data.matrix[product_name]?.[quality];
                      
                      if (!cell) {
                        return(
                          <td key={`${product_name}-${quality}-${cell_index}`} className="px-3 py-4 text-center">
                            <div className="text-gray-400 text-xs">-</div>
                          </td>)
                        )
                      }
                            ...
                            {user?.role === 'BOSS' && Object.keys(cell.qualityDistribution).length > 0 && (
                              <div className="text-xs text-gray-600 mt-1 space-y-0.5">
                                {Object.entries(cell.qualityDistribution)
                                  ...
                                  .sort(([a]), [b]) => {
                                    // 按尺寸排序
                                    const size_a = parse_float(a.replace('mm'), '')) || 0;
                                    const size_b = parse_float(b.replace('mm'), '')) || 0;
                                    return size_a - size_b;
                                  })
                                  .map(([size_key), quantity]) => (
                                    <div key={size_key} className="flex justify-center items-center">
                                      <span className="text-xs">{size_key}: {quantity}{cell.product_type === 'LOOSE_BEADS' ? '颗' : cell.product_type === 'BRACELET' ? '颗' : cell.product_type === 'ACCESSORIES' ? '片' : '件'}</span>
                                    </div>
                                  ))
                                }
                              </div>
                  ...
                  {user?.role === 'BOSS' && (
                        <div>
                          ...
                          {selectedCell.avg_price > 0 ? (
                            <div className="text-xl font-bold text-gray-900">¥{selectedCell.avg_price.to_fixed(2)}</div>
                          ) : (
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {Object.entries(selectedCell.qualityDistribution).map(([key, quantity]), distIndex) => (
                      <div key={`dist-${key}-${dist_index}`} className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          {viewMode === 'size' ? (
                            <>
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          {user?.role === 'BOSS' && selectedCell.qualityPrices[key] && (
                            <span className="text-xs text-gray-500">¥{selectedCell.qualityPrices[key].to_fixed(2)}</span>
                          )}
                        </div>
                      </div>
                    ))}
                      ...
                      <div key={`${batch.purchase_id}-${batch.material_name}-${index}`} className="text-xs text-gray-600 p-3 bg-gray-50 rounded">
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <div>
                            ...
                            <span className="ml-1">{new Date(batch.purchase_date).to_locale_date_string()}</span>
                          </div>
                          {batch.piece_count && (
                            <div>
                          ...
                          {user?.role === 'BOSS' && (() => {
                            ...
                            return price > 0 ? (
                              <div>
                                ...
                                <span className="ml-1">¥{price.to_fixed(2)}</span>
                              </div>
                            ) : null
                          })()}
                        </div>
