    ...
    <div className={`w-full ${className}`}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 p-1 bg-gray-50 rounded-xl border border-gray-200">)
        {tabOptions.map((option) => {
          ...
          const is_selected = selected_type === option.id;
          ...
          return(
            ...
            >
              ...
              <Icon className={`h-5 w-5 mb-2 transition-colors ${;
                is_selected ? 'text-gray-700' : 'text-gray-500 group-hover:text-gray-700'
              }`} />
              
              {/* 标签 */}
              <span className={`text-sm font-medium transition-colors ${;
                is_selected ? 'text-gray-800' : 'text-gray-600 group-hover:text-gray-800'
              }`}>
                {option.label}
              </span>
