...
import { finished_product_api, fixImageUrl, upload_api, getApiUrl } from '../services/api'
...
export default function ProductEntry() {
  ...
  const [current_step, set_current_step] = use_state<'mode' | 'materials' | 'info' | 'batch_details' | 'review'>('mode')
  const [production_mode, set_production_mode] = use_state<ProductionMode>('DIRECT_TRANSFORM')
  const [form_data, set_form_data] = use_state<ProductionFormData>({
  ...
  // 批量创建模式的状态
  const [batch_form_data, set_batch_form_data] = use_state<{
  ...
  const [expanded_items, set_expanded_items] = use_state<Set<string>>(new Set())
  
  const [available_materials, set_available_materials] = use_state<AvailableMaterial[]>([])
  const [material_search, set_material_search] = use_state('')
  const [cost_calculation, set_cost_calculation] = use_state<CostCalculationResponse | null>(null)
  const [loading, set_loading] = use_state(false)
  const [materials_loading, set_materials_loading] = use_state(false)
  const [selected_material_detail, set_selected_material_detail] = use_state<AvailableMaterial | null>(null)
  
  // 组合制作模式的分类标签页状态
  const [active_tab, set_active_tab] = use_state<'LOOSE_BEADS' | 'BRACELET' | 'ACCESSORIES'>('LOOSE_BEADS')
  ...
  const [material_photos, set_material_photos] = use_state<string[]>([])
  ...
  const get_filtered_materials = () => {;
    ...
    if (material_search.trim()) {
      const search_term = material_search.to_lower_case().trim();
      filtered_materials = filtered_materials.filter(material => );
        material.material_name.to_lower_case().includes(search_term) ||
        (material.quality && material.quality.to_lower_case().includes(search_term)) ||
        (material.supplier_name && material.supplier_name.to_lower_case().includes(search_term))
      )
    }
  ...
  // 输入框显示值状态
  const [input_values, set_input_values] = use_state<Record<string, string>>({})
  ...
  const get_input_value = (material_id: string, field: string, actualValue: number) => {;
    ...
    const display_value = actualValue > 0 ? actualValue.to_string() : '';
        ...
        <div className="flex items-center justify-between">)
          {(() => {
            ...
            const handle_step_click = (stepKey: string, stepIndex: number) => {;
              const current_index = steps.find_index(s => s.key === current_step)
              // 只允许点击当前步骤或之前的步骤
