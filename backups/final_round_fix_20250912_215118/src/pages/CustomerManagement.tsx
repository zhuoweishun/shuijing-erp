...
export default function CustomerManagement() {
  ...
  }, [state.column_filters.visible, state.column_filters.column, update_filter_position])
       ...
       <div className="grid grid-cols-4 gap-2">
         {Object.entries(categoryGroups).map(([categoryName, types]) => (
           <div key={category.name} className="space-y-1.5">
             <h4 className="text-xs font-medium text-gray-700 text-center border-b border-gray-200 pb-0.5 mb-1">{category.name}</h4>
  ...
  const fetchCustomers = async (customState?: Partial<CustomerManagementState>) => {
    try {
      // 使用传入的状态或当前状态
      const currentState = customState ? { ...state, ...custom_status } : state
                        ...
                        <td className="px-6 py-4 whitespace-nowrap" style={{ width: '90px' }}>
                          <div className="flex flex-wrap gap-1">
                            {customer_labels.map((label_key) => {
                              const labelConfig = CUSTOMER_TYPE_LABELS[label_key]
