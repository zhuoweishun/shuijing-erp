...
interface PurchaseFormData {
  ...
  piece_count_value?: number // 散珠颗数/饰品片数/成品件数
...
export default function PurchaseEntry() {
  ...
  const piece_count_value = watch('piece_count_value')
  ...
  const unit_price = useMemo(() => {
    ...
    switch (selected_material_type) {
      case 'LOOSE_BEADS':
        return piece_count_value ? total_price / piece_count_value : 0 // 每颗价格
      case 'BRACELET':
        return quantity ? total_price / quantity : 0 // 每条价格
      case 'ACCESSORIES':
        return piece_count_value ? total_price / piece_count_value : 0 // 每片价格
      case 'FINISHED':
        return piece_count_value ? total_price / piece_count_value : 0 // 每件价格
      default:
        return 0
    }
  }, [total_price, selected_material_type, quantity, piece_count_value])
  
  // 计算每颗珠子价格（仅用于散珠和手串）
  const price_per_bead = useMemo(() => {
    if (!total_price) return 0
    
    if (selected_material_type === 'LOOSE_BEADS' && piece_count_value) {
      return total_price / piece_count_value
    } else if (selected_material_type === 'BRACELET' && total_beads) {
      return total_price / total_beads
    }
    return 0
  }, [total_price, selected_material_type, piece_count_value, total_beads])
  ...
  const handle_material_type_change = (material_type: ProductType, unit_type: UnitType) => {
    ...
    if (material_type === 'LOOSE_BEADS' || material_type === 'BRACELET') {
      ...
      setValue('piece_count_value', undefined)
    } else {
  ...
  const get_browser_info = () => {
    ...
    return { browserName, browserVersion, user_agent }
  }
  ...
  const checkCameraSupport = () => {
    ...
    console.log('=== 相机兼容性检查 ===', {
      ...
      mediaDevicesExists: !!(navigator && navigator.mediaDevices),
      getUserMediaExists: !!(navigator && navigator.mediaDevices && navigator.mediaDevices.get_user_media),
      getSupportedConstraintsExists: !!(navigator && navigator.mediaDevices && navigator.mediaDevices.get_supported_constraints)
    })
    ...
    // 检查getUserMedia是否存在
    if (!navigator.mediaDevices.get_user_media) {
    ...
    if (!navigator.mediaDevices.get_supported_constraints) {
  ...
  const handle_ai_parse = async (description: string) => {
    ...
    set_ai_parsing(true)
    try {
      const response = await ai_api.parseCrystalPurchase(description)
      if (response.success && response.data) {
        ...
        // 数量字段：quantity, piece_count_value -> quantity, piece_count_value
        if (aiData.quantity) {
          setValue('quantity', aiData.quantity)
          console.log('✅ 映射数量(串数):', aiData.quantity)
        }
        if (aiData.piece_count_value) {
          setValue('piece_count_value', aiData.piece_count_value)
          console.log('✅ 映射数量(颗数/片数/件数):', aiData.piece_count_value)
        }
        ...
        const mappedFields = [
          ...
          aiData.piece_count_value && 'piece_count_value',
  ...
  const on_submit = async (data: PurchaseFormData) => {
    ...
    try {
      ...
      const submitData = {
        ...
        piece_count_value: data.piece_count_value ? Number(data.piece_count_value) : undefined,
                          ...
                          const createMediaDevicesPolyfill = () => {
                            ...
                            if (!navigator.mediaDevices) {
                              ...
                              if (!navigator.mediaDevices.get_supported_constraints) {
                                ...
                                navigator.mediaDevices.get_supported_constraints = () => {
                                  return {
                                    width: true,
                                    height: true,
                                    aspectRatio: true,
                            ...
                            console.log('=== Polyfill创建完成 ===', {
                              ...
                              hasGetUserMedia: !!(navigator.mediaDevices && navigator.mediaDevices.get_user_media),
                              hasGetSupportedConstraints: !!(navigator.mediaDevices && navigator.mediaDevices.get_supported_constraints)
                            })
                          }
          ...
          <MobileFormRow columns={isMobile ? 1 : 2}>
            ...
            {(selected_material_type === 'LOOSE_BEADS' || selected_material_type === 'ACCESSORIES' || selected_material_type === 'FINISHED') && (
              <MobileInput
                ...
                value={watch('piece_count_value') || ''}
                onChange={(e) => setValue('piece_count_value', parseInt(e.target.value) || undefined)}
                error={errors.piece_count_value?.message}
                inputMode="numeric"
              />
            )}
