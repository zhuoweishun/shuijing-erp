...
export default function SkuAdjustForm({ sku, on_submit, onCancel, loading = false }: SkuAdjustFormProps) {
  ...
  const validateForm = (): boolean => {
    ...
    // 验证成本调整
    if (formData.costAdjustment < 0) {
      newErrors.costAdjustment = '成本调整不能为负数'
    }
  ...
  const calculateNewTotalCost = () => {
    ...
    if (formData.type === 'increase') {
      return currentTotalCost + (formData.costAdjustment * formData.quantity)
    } else {
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                min="0"
                step="0.01"
                value={formData.costAdjustment}
                onChange={(e) => handleInputChange('costAdjustment', parseFloat(e.target.value) || 0)}
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                  errors.costAdjustment ? 'border-red-300' : 'border-gray-300'
                }`}
                placeholder="0.00"
                disabled={loading}
              />
