...
export default function PurchaseDetailModal({ 
  ...
  const fetchPurchaseDetail = async () => {if (!purchase_id) return
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        editLogsCount: purchaseData.edit_logs?.length || 0,
        editLogs: purchaseData.edit_logs
        })
    ...
    if (purchase && isEditMode) {
      ...
      if (purchase.material_type === 'BRACELET') {
        ...
        const editDataObj: any = {
          ...baseData,
          piece_count_value: purchase.piece_count_value || undefined
        }
  ...
  const calculateAllSuggestions = (currentEditData: any, originalData: any) => {// 获取产品类型
    ...
    if (product_type === 'BRACELET') {
      ...
      quantity = typeof currentEditData.piece_count_value === 'string' ? parseFloat(currentEditData.piece_count_value) : (currentEditData.piece_count_value || 0)
      original_quantity = originalData?.piece_count_value || 0
    }
    ...
    const quantityChanged = Math.abs(quantity - original_quantity) > 0.1
  ...
  const handleSave = async () => {if (!purchase || !canEdit) return
    ...
    try {
      ...
      if (editData.piece_count_value !== undefined && editData.piece_count_value !== purchase.piece_count_value) {
        updateData.piece_count_value = editData.piece_count_value
      }
      ...
      console.log('🔍 [前端调试] purchase原始数据:', {
        ...
        piece_count_value: purchase.piece_count_value,
      ...
      console.log('🔍 [前端调试] 即将调用API:', `purchase_api.update(${purchase.id}, updateData)`)
      
      // 调用后端API保存数据
      console.log('🚀 [API调用] 开始调用purchaseApi.update')
      const response = await purchase_api.update(purchase.id, updateData)
      console.log('📥 [API响应] purchase_api.update响应:', response)
  ...
  const handleDelete = async () => {if (!purchase || !canEdit) return
    ...
    try {
      ...
      const response = await purchase_api.delete(purchase.id)
  ...
  const renderEditField = (field: string, Label: string, value: any, type: 'text' | 'number' | 'select' = 'text', options?: string[]) => {
    if (!isEditMode) {
      ...
      if (field === 'piece_count_value') {
        const piece_countValue = typeof value === 'object' && value !== null ? (value.piece_count_value || value.value || '') : value
        return piece_countValue ? `${piece_count_value}` : '-'
      }
    ...
    // 判断是否为整数字段
    const isIntegerField = ['quantity', 'beads_per_string', 'total_beads', 'piece_count_value', 'min_stock_alert'].includes(field)
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          {(purchase.material_type === 'LOOSE_BEADS' || purchase.material_type === 'ACCESSORIES' || purchase.material_type === 'FINISHED') && (
                            <div className="flex justify-between items-center">
                              ...
                              <div className="font-medium text-gray-900">
                                {renderEditField('piece_count_value', '数量', purchase.piece_count_value, 'number')}
                              </div>
                            </div>
                          )}
                                ...
                                <div className="font-semibold text-purple-900 text-xs">
                                  {isEditMode ? (
                                    ...
                                    (() => {
                                      ...
                                      } else if (current_quantity > 0 && currentBeadsPerString > 0) {
                                        return current_quantity * currentBeadsPerString
                                      }
                                      return '-'
                                    })()
                                  )}
                                ...
                                <p className="font-semibold text-orange-900 text-xs">
                                  {user?.role === 'EMPLOYEE' ? '-' : (
                                    (() => {
                                      ...
                                      if (!totalBeadsForCalculation && current_quantity > 0 && currentBeadsPerString > 0) {
                                        totalBeadsForCalculation = current_quantity * currentBeadsPerString
                                      }
                              ...
                              <div className="text-center p-1.5 bg-blue-50 rounded-md border border-blue-100">
                                ...
                                <p className="font-semibold text-blue-900 text-xs">
                                  {user?.role === 'EMPLOYEE' ? '-' : (
                                    (() => {
                                      const currentTotalPrice = isEditMode && editData.total_price !== undefined ? editData.total_price : purchase.total_price || 0
                                      const currentPieceCount = isEditMode && editData.piece_count_value !== undefined ? editData.piece_count_value : purchase.piece_count_value || 0
