...
// 获取用户列表（仅老板权限）
router.get('/', authenticateToken, requireRole('BOSS'), asyncHandler(async (_req, res) => {
  try {
...
// 获取用户资料
router.get('/profile', authenticateToken, asyncHandler(async (_req, res) => {
  try {
...
// 更新用户资料
router.put('/profile', authenticateToken, asyncHandler(async (_req, res) => {
  try {
    const userId = req.user?.id
    if (!userId) {
      return res.status(401).json({
...
// 创建用户（仅老板权限）
router.post('/', authenticateToken, requireRole('BOSS'), asyncHandler(async (_req, res) => {
  try {
...
// 更新用户（仅老板权限）
router.put('/:id', authenticateToken, requireRole('BOSS'), asyncHandler(async (_req, res) => {
  try {
    const { id } = req.params
...
// 删除用户（仅老板权限）
router.delete('/:id', authenticateToken, requireRole('BOSS'), asyncHandler(async (_req, res) => {
  try {
    const { id } = req.params
