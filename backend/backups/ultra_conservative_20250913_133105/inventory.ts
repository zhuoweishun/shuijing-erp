  ...
  try {
    ...
    const inventoryQuery = `
      SELECT 
        ...
        p.photos,
        CASE 
          WHEN p.product_type = 'LOOSE_BEADS' THEN COALESCE(p.pieceCount, 0)
          WHEN p.product_type = 'BRACELET' THEN COALESCE(p.totalBeads, p.pieceCount, 0)
          WHEN p.product_type = 'ACCESSORIES' THEN COALESCE(p.pieceCount, 0)
          WHEN p.product_type = 'FINISHED' THEN COALESCE(p.pieceCount, 0)
      ...
      WHERE 1=1
      ORDER BY p.product_type, p.productName, 
...
router.get('/grouped', authenticateToken, asyncHandler(async (req, res) => {
  ...
  if (low_stock_only === 'true') {
    whereClause += ' AND (CASE WHEN p.product_type = "LOOSE_BEADS" THEN COALESCE(p.pieceCount, 0) WHEN p.product_type = "BRACELET" THEN COALESCE(p.quantity, 0) WHEN p.product_type = "ACCESSORIES" THEN COALESCE(p.pieceCount, 0) WHEN p.product_type = "FINISHED" THEN COALESCE(p.pieceCount, 0) ELSE COALESCE(p.quantity, 0) END - COALESCE(mu.used_beads, 0)) <= p.minStockAlert'
  }
    ...
    const groupedQuery = `
      SELECT 
        ...
        SUM(
          CASE 
            WHEN p.product_type = 'LOOSE_BEADS' THEN COALESCE(p.pieceCount, 0)
            WHEN p.product_type = 'BRACELET' THEN COALESCE(p.quantity, 0)
            WHEN p.product_type = 'ACCESSORIES' THEN COALESCE(p.pieceCount, 0)
            WHEN p.product_type = 'FINISHED' THEN COALESCE(p.pieceCount, 0)
            ELSE COALESCE(p.quantity, 0)
...
router.get('/search', authenticateToken, asyncHandler(async (req, res) => {
  ...
  try {
    const searchQuery = `
      ...
      GROUP BY p.id, p.productName, p.beadDiameter, p.quality, p.minStockAlert, 
               p.totalBeads, p.price_per_bead, s.name, p.purchaseDate
...
router.get('/finished-products-cards', authenticateToken, asyncHandler(async (req, res) => {
  ...
  try {
    // 构建查询条件
    let whereConditions = ['p.product_type = "FINISHED"']
    let queryParams: any[] = []
      ...
      SELECT 
        ...
        CASE 
          WHEN ${req.user?.role === 'BOSS' ? 'TRUE' : 'FALSE'} THEN 
            CASE 
              WHEN p.product_type = 'FINISHED' AND p.pieceCount > 0 AND p.totalPrice IS NOT NULL 
  ...
  try {
    ...
    const basicStatsQuery = `
      SELECT 
        ...
        SUM(CASE 
          WHEN p.product_type = 'LOOSE_BEADS' THEN (p.pieceCount - COALESCE(mu.used_quantity, 0))
          WHEN p.product_type = 'BRACELET' THEN (p.totalBeads - COALESCE(mu.used_quantity, 0))
          WHEN p.product_type = 'ACCESSORIES' THEN (p.pieceCount - COALESCE(mu.used_quantity, 0))
          WHEN p.product_type = 'FINISHED' THEN (p.pieceCount - COALESCE(mu.used_quantity, 0))
          ELSE 0
...
router.get('/product-distribution', authenticateToken, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (product_type && product_type !== 'ALL') {
      whereClause = `WHERE p.product_type = '${product_type}'`
    }
    ...
    const distributionQuery = `
      ...
      ${whereClause}
      GROUP BY p.productName, p.product_type
  ...
  try {
    ...
    const consumptionQuery = `
      SELECT 
        ...
        SUM(
          CASE 
            WHEN p.product_type IN ('LOOSE_BEADS', 'BRACELET') THEN mu.quantity_used
            WHEN p.product_type IN ('ACCESSORIES', 'FINISHED') THEN mu.quantity_used
            ELSE 0
          END
        ) as total_consumed,
        ...
        CASE 
          WHEN p.product_type IN ('LOOSE_BEADS', 'BRACELET') THEN '颗'
          WHEN p.product_type IN ('ACCESSORIES', 'FINISHED') THEN '件'
          ELSE '个'
        END as unit_type
      ...
      WHERE 1=1 ${timeCondition}
        AND (
          (p.product_type IN ('LOOSE_BEADS', 'BRACELET') AND mu.quantity_used > 0) OR
          (p.product_type IN ('ACCESSORIES', 'FINISHED') AND mu.quantity_used > 0)
        )
      GROUP BY p.id, p.productName, p.product_type, p.beadDiameter, p.specification, p.quality, s.name
...
router.get('/price-distribution', authenticateToken, asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (product_type && product_type !== 'ALL') {
      productTypeCondition = `AND p.product_type = '${product_type}'`
    }
    ...
    if (price_type === 'unit_price') {
      ...
      const priceRangeQuery = `
        SELECT 
           ...
           END as price_range,
          COUNT(*) as count
        FROM (          SELECT             p.product_type as product_type,            CASE               WHEN p.product_type = 'LOOSE_BEADS' AND p.totalBeads > 0 THEN p.totalPrice / p.totalBeads              WHEN p.product_type = 'BRACELET' AND p.quantity > 0 THEN p.totalPrice / p.quantity              WHEN p.product_type = 'ACCESSORIES' AND p.pieceCount > 0 THEN p.totalPrice / p.pieceCount              WHEN p.product_type = 'FINISHED' AND p.pieceCount > 0 THEN p.totalPrice / p.pieceCount              ELSE NULL            END as calculated_price          FROM purchases p          WHERE p.status IN ('ACTIVE', 'PENDING')             AND p.totalPrice IS NOT NULL             AND p.totalPrice > 0            AND (              (p.product_type = 'LOOSE_BEADS' AND p.totalBeads IS NOT NULL AND p.totalBeads > 0) OR              (p.product_type = 'BRACELET' AND p.quantity IS NOT NULL AND p.quantity > 0) OR              (p.product_type = 'ACCESSORIES' AND p.pieceCount IS NOT NULL AND p.pieceCount > 0) OR              (p.product_type = 'FINISHED' AND p.pieceCount IS NOT NULL AND p.pieceCount > 0)            )            ${productTypeCondition}        ) as price_data
    ...
    // 查询价格分布数据
     const priceQuery = `
       ...
       WHERE p.status IN ('ACTIVE', 'PENDING') 
         ...
         AND p.totalPrice > 0
         AND (
           (p.product_type IN ('LOOSE_BEADS', 'BRACELET') AND (p.totalBeads IS NOT NULL AND p.totalBeads > 0 OR p.pieceCount IS NOT NULL AND p.pieceCount > 0)) OR
           (p.product_type = 'ACCESSORIES' AND p.pieceCount IS NOT NULL AND p.pieceCount > 0) OR
           (p.product_type = 'FINISHED' AND p.pieceCount IS NOT NULL AND p.pieceCount > 0)
         )
         ${productTypeCondition}
       GROUP BY p.id, p.productName, p.product_type, p.beadDiameter, p.specification, 
                p.quality, p.quantity, p.pieceCount, p.totalBeads, p.unit_price, 
                p.totalPrice, p.price_per_bead, p.price_per_piece, p.price_per_gram, p.weight, 
...
router.get('/:purchase_id', authenticateToken, asyncHandler(async (req, res) => {
  ...
  try {
    const detailQuery = `
      ...
      WHERE p.id = ?
      GROUP BY p.id, p.productName, p.beadDiameter, p.quality, p.minStockAlert, 
               p.totalBeads, p.price_per_bead, p.price_per_gram, p.totalPrice, p.weight,
...
router.get('/export/excel', authenticateToken, asyncHandler(async (req, res) => {
  try {
    ...
    exportQuery += `
      WHERE p.beadDiameter IS NOT NULL
      GROUP BY p.id, p.productName, p.beadDiameter, p.quality, p.minStockAlert, 
               p.totalBeads, p.price_per_gram, p.price_per_bead, p.purchaseDate`
