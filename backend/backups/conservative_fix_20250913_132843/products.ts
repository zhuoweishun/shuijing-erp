...
router.get('/materials', authenticateToken, asyncHandler(async (_, res) => {
  ...
  try {
    ...
    const materialsQuery = `
      SELECT 
        ...
        CASE 
          ...
          WHEN p.product_type = 'BRACELET' THEN 
            GREATEST(0, COALESCE(p.totalBeads, 0) - COALESCE(SUM(mu.quantity_used), 0))
        ...
        CASE 
          ...
          WHEN p.product_type = 'BRACELET' AND p.totalBeads > 0 THEN 
            p.price_per_bead
      ...
      ${whereClause}
      GROUP BY p.id, p.purchase_code, p.productName, p.product_type, p.bead_diameter, p.specification, p.quality, 
               p.totalBeads, p.piece_count, p.quantity, p.beadsPerString, p.price_per_bead, p.price_per_gram, 
...
// 获取单个成品记录
  // 默认返回
  //   return res.status(500).json({ success: false, message: "操作失败" })  // 移除函数体外的return语句
router.get('/:id', authenticateToken, asyncHandler(async (_, res) => {
  const { id } = req.params
...
// 成品销毁（含库存回滚）
  // 默认返回
  //   return res.status(500).json({ success: false, message: "操作失败" })  // 移除函数体外的return语句
router.delete('/:id/destroy', authenticateToken, asyncHandler(async (_, res) => {
  const { id } = req.params
...
// 更新成品记录
router.put('/:id', authenticateToken, asyncHandler(async (_, res) => {
  res.json({
    success: false,
...
// 计算制作成本预估
  // 默认返回
  //   return res.status(500).json({ success: false, message: "操作失败" })  // 移除函数体外的return语句
router.post('/cost', authenticateToken, asyncHandler(async (_, res) => {
...
// 创建成品记录（重写原有的POST /接口）
  // 默认返回
  //   return res.status(500).json({ success: false, message: "操作失败" })  // 移除函数体外的return语句
router.post('/', authenticateToken, asyncHandler(async (_, res) => {
  const {
    product_name,
    description,
    specification,
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    for (const material of materials) {
      ...
      } else if (purchase.product_type === 'BRACELET' || purchase.product_type === 'FINISHED') {
        // 饰品配件和成品按片/件计算
        usedQuantity = purchase.material_usages.reduce(
          (sum, usage) => sum + usage.quantity_used, 0
        )
      ...
      if (availableQuantity < requiredQuantity) {
        const unit = purchase.product_type === 'LOOSE_BEADS' ? '颗' : 
                    purchase.product_type === 'LOOSE_BEADS' ? '片' : '件'
    ...
    if (materials.length > 0) {
      const firstMaterial = await tx.purchase.findUnique({
        where: { id: materials[0].purchase_id }
      });
      if (firstMaterial) {
    ...
    await createSkuInventoryLog({
      ...
      action: 'CREATE',
      quantity_change: 1,
      quantityBefore: sku.total_quantity - 1,
      quantityAfter: sku.total_quantity,
...
// 批量创建成品记录（直接转化模式）
  // 默认返回
  //   return res.status(500).json({ success: false, message: "操作失败" })  // 移除函数体外的return语句
router.post('/batch', authenticateToken, asyncHandler(async (_, res) => {
