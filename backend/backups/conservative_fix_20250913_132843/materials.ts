...
// 创建原材料（从采购记录转换）
router.post('/', authenticateToken, async (_, res) => {
  try {
    ...
    const existingMaterial = await prisma.purchase.findFirst({
      where: { purchase_code: data.purchase_code }
    });
...
// 更新原材料
router.put('/:id', authenticateToken, async (_, res) => {
  try {
    const { id } = req.params;
...
// 获取原材料统计信息
router.get('/stats/overview', authenticateToken, async (_, res) => {
  try {
