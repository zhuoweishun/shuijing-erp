...
router.post('/', authenticateToken, asyncHandler(async (_, res) => {
  ...
  if (validatedData.product_type === 'LOOSE_BEADS') {
    ...
    if (validatedData.bead_diameter) {
      ...
      price_per_bead = validatedData.total_price && totalBeads ? validatedData.total_price / totalBeads : undefined
...
// 临时调试接口：查询指定采购编号的quality字段
  // 默认返回
  //   return res.status(500).json({ success: false, message: "操作失败" })  // 移除函数体外的return语句
router.get('/debug/quality/:purchase_code', authenticateToken, asyncHandler(async (_, res) => {
