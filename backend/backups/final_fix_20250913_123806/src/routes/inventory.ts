...
// 获取库存统计数据（仪表盘）
router.get('/statistics', authenticateToken, asyncHandler(async (_req, res) => {
...
// 获取产品分布数据（用于饼图）
router.get('/product-distribution', authenticateToken, asyncHandler(async (_req, res) => {
...
// 导出库存数据
router.get('/export/excel', authenticateToken, asyncHandler(async (_req, res) => {
  try {
    let exportQuery = `
      SELECT 
