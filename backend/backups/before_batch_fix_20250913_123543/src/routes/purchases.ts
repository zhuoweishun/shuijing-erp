...
import { ErrorResponses, createSuccessResponse } from '../utils/errorResponse'
  ...
  if (sortBy && sortOrder) {
    ...
    if (field && (sortOrder === 'asc' || sortOrder === 'desc')) {
      if (field === 'supplier.name') {
        ...
        purchases.slice(0, 15).forEach((p, index) => {
          const displayQuantity = p.product_type === 'BRACELET' ? p.quantity : p.piece_count
          const displayUnit = p.product_type === 'BRACELET' ? '条' : 
...
router.put('/:id', authenticateToken, asyncHandler(async (req, res) => {
  ...
  if (existingPurchase.material_usages && existingPurchase.material_usages.length > 0) {
    const usedByProducts = existingPurchase.material_usages.map(usage => usage.product.name).join('、')
    return res.status(400).json({
      ...
      data: {
        usedByProducts: existingPurchase.material_usages.map(usage => ({
          ...
          quantityUsed: usage.quantity_used_beads || usage.quantity_used_pieces
        }))
      }
    })
  }
  ...
  // 处理供应商
  let supplierId: string | undefined = existingPurchase.supplier_id
  ...
  if (dbData.beadDiameter && !dbData.beadsPerString) {
    updateData.beads_per_string = calculateBeadsPerString(dbData.beadDiameter)
  }
  ...
  const finalTotalPrice = dbData.totalPrice ?? existingPurchase.total_price
  ...
  // 根据产品类型计算相关的派生字段
  const finalPieceCount = dbData.pieceCount ?? existingPurchase.piece_count
  const productType = existingPurchase.product_type
  ...
  if (finalTotalPrice) {
    ...
    console.log('🔍 [派生字段计算] productType:', productType, 'pricePerBead:', updateData.price_per_bead, 'pricePerPiece:', updateData.price_per_piece, 'unitPrice:', updateData.unit_price)
  }
    ...
    return res.status(400).json({
      ...
      data: {
        usedByProducts: existingPurchase.material_usages.map(usage => ({
          productId: usage.product.id,
          productName: usage.product.name,
          quantityUsed: usage.quantity_used_beads
        }))
      }
    })
  }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.edit_log.create({
        data: {
          ...
          user_id: req.user.id,
          action: 'DELETE',
          details: `老板 ${userName} 在 ${timeStr} 删除了采购记录：${existingPurchase.product_name}（采购编号：${existingPurchase.purchase_code}）。该操作同时清理了相关库存数据。`,
          changed_fields: [{
            field: 'deleted',
            displayName: '删除操作',
            oldValue: '存在',
    ...
    res.json({
      ...
      data: {
        deletedPurchase: {
          ...
          purchaseCode: existingPurchase.purchase_code
        }
      }
    })
  } catch (error) {
