...
const createMaterialSchema = z.object({
  ...
  product_type: z.enum(['SEMI_FINISHED', 'FINISHED']),
...
const querySchema = z.object({
  ...
  product_type: z.enum(['SEMI_FINISHED', 'FINISHED']).optional(),
  status: z.enum(['ACTIVE', 'DEPLETED', 'INACTIVE']).optional(),
  ...
  try {
    ...
    const material = await prisma.purchase.create({
      data: {
        ...
        product_type: data.material_type,
...
// 更新原材料
router.put('/:id', authenticateToken, async (_req, res) => {
  try {
    const { id } = req.params;
...
// 删除原材料
router.delete('/:id', authenticateToken, async (_req, res) => {
  try {
    const { id } = req.params;
...
router.get('/stats/overview', authenticateToken, async (_req, res) => {
  try {
    const [total_count, active_count, depleted_count, semi_finished_count, finished_count] = await Promise.all([
      ...
      prisma.purchase.count({ where: { status: 'DEPLETED' } }),
      prisma.purchase.count({ where: { product_type: 'SEMI_FINISHED' } }),
