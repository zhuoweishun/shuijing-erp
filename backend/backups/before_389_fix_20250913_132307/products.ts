...
router.post('/', authenticateToken, asyncHandler(async (req, res) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    for (const material of materials) {
      ...
      if (purchase.product_type === 'LOOSE_BEADS' || purchase.product_type === 'LOOSE_BEADS') {
        // 散珠和手串按颗计算
      ...
      } else if (purchase.product_type === 'LOOSE_BEADS' || purchase.product_type === 'FINISHED') {
        // 饰品配件和成品按片/件计算
        usedQuantity = purchase.material_usages.reduce(
          (sum, usage) => sum + usage.quantity_used, 0
        )
      ...
      if (availableQuantity < requiredQuantity) {
        const unit = purchase.product_type === 'LOOSE_BEADS' || purchase.product_type === 'LOOSE_BEADS' ? '颗' : 
                    purchase.product_type === 'LOOSE_BEADS' ? '片' : '件'
        const available_quantity = purchase.available_quantity || 0;
