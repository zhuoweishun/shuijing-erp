...
async function fixNullStringPhotos() {
  ...
  try {
    console.log('=== 修复字符串null的SKU图片 ===');
    
    // 1. 获取所有photos为字符串'null'的SKU
    const [nullSkus] = await connection.execute(`
      SELECT id, sku_name, photos
      FROM product_skus 
      WHERE LENGTH(photos) = 4 AND photos = 'null'
    `);
    
    console.log(`找到 ${nullSkus.length} 个photos为字符串null的SKU`);
    ...
    const [afterFix] = await connection.execute(`
      SELECT 
        CASE 
          ...
          WHEN LENGTH(photos) = 4 AND photos = 'null' THEN '字符串null'
...
fixNullStringPhotos().catch(console.error);