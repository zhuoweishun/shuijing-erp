  ...
  try {
    ...
    const material = await prisma.material.create({
      data: {
        ...
        ...(data.material_type === 'ACCESSORIES' && data.specification ? { accessory_specification: data.specification } : {}),
        ...(data.material_type === 'FINISHED_MATERIAL' && data.specification ? { finished_material_specification: data.specification } : {}),
        original_quantity: data.total_quantity,
        remaining_quantity: data.remaining_quantity,
        inventory_unit: data.unit as any, // 临时类型断言，需要后续优化单位映射
