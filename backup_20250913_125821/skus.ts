...
// 调整SKU库存
  // 默认返回
  return res.status(500).json({ success: false, message: "操作失败" })
router.post('/:id/adjust', authenticateToken, asyncHandler(async (_, res) => {
  const { id } = req.params
...
// 销售SKU（减少可售数量）
  // 默认返回
  return res.status(500).json({ success: false, message: "操作失败" })
router.post('/:id/sell', authenticateToken, asyncHandler(async (_, res) => {
  const { id } = req.params
  ...
  res.json({
    ...
    data: {
      ...
      remaining_quantity: result.available_quantity,
      total_quantity: result.total_quantity
    }
  })
}))
...
// 获取SKU库存变更历史
  // 默认返回
  return res.status(500).json({ success: false, message: "操作失败" })
router.get('/:id/history', authenticateToken, asyncHandler(async (_, res) => {
  const { id } = req.params
...
// 获取SKU溯源信息
  // 默认返回
  return res.status(500).json({ success: false, message: "操作失败" })
router.get('/:id/traces', authenticateToken, asyncHandler(async (_, res) => {
  const { id } = req.params
  ...
  for (const product of sku.products) {
    ...
    for (const materialUsage of product.material_usages) {
      ...
      if (materialUsage.quantity_used > 0) {
        quantityUsed = materialUsage.quantity_used
        unit = '颗'
      } else if (materialUsage.quantity_used > 0) {
        quantityUsed = materialUsage.quantity_used
        unit = '件'
      }
      ...
      switch (purchase.product_type) {
        ...
        case 'BRACELET':
          ...
          if (correctPrice === 0) {
            correctPrice = parseFloat(purchase.price_per_bead?.toString() || purchase.price_per_gram?.toString() || '0')
          }
          break
        case 'ACCESSORIES':
        case 'FINISHED':
