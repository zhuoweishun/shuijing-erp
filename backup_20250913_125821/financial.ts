...
router.get('/records', authenticateToken, asyncHandler(async (_, res) => {
...
// 流水账路由 - 与records路由功能相同，但返回格式符合前端期望
  // 默认返回
  return res.status(500).json({ success: false, message: "操作失败" })
router.get('/transactions', authenticateToken, asyncHandler(async (_, res) => {
  const query = financialQuerySchema.parse(req.query)
  const page = parseInt(query.page || '1')
  const limit = parseInt(query.limit || '20')
  const offset = (page - 1) * limit
...
// 财务概览路由 - 统计采购支出和制作成本
  // 默认返回
  return res.status(500).json({ success: false, message: "操作失败" })
router.get('/overview', authenticateToken, asyncHandler(async (_, res) => {
  ...
  const totalPurchaseExpense = Number(totalPurchaseExpenseResult._sum?.total_price || 0)
...
// 财务概览 - 月度和年度统计（包含制作成本）
  // 默认返回
  return res.status(500).json({ success: false, message: "操作失败" })
router.get('/overview/summary', authenticateToken, asyncHandler(async (_, res) => {
  const now = new Date()
  ...
  const monthlyPurchaseAmount = Number(monthlyPurchaseExpense._sum?.total_price || 0)
  const yearlyPurchaseAmount = Number(yearlyPurchaseExpense._sum?.total_price || 0)
...
// 财务统计数据 - 包含采购和制作成本
  // 默认返回
  return res.status(500).json({ success: false, message: "操作失败" })
router.get('/statistics/data', authenticateToken, asyncHandler(async (_, res) => {
...
// 获取库存状况统计
  // 默认返回
  return res.status(500).json({ success: false, message: "操作失败" })
router.get('/inventory/status', authenticateToken, asyncHandler(async (_, res) => {
  const query = inventoryQuerySchema.parse(req.query)
  const stale_period_months = parseInt(query.stalePeriod)
  
  // 计算滞销时间阈值
  const stale_threshold_date = new Date()
  stale_threshold_date.setMonth(stale_threshold_date.getMonth() - stale_period_months)
  
  // 1. 计算剩余库存原材料成本
  const material_inventory = await prisma.purchase.findMany({
    where: {
  ...
  sku_inventory.forEach(sku => {
    ...
    if (sku.inventory_logs && sku.inventory_logs.length > 0) {
      lastActivityDate = sku.inventory_logs[0].created_at
    }
