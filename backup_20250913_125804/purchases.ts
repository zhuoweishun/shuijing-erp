...
router.get('/:id', authenticateToken, asyncHandler(async (_, res) => {
  ...
  const filteredPurchase = filterSensitiveFields(apiFormatPurchase, req.user?.role || "EMPLOYEE")
...
router.put('/:id', authenticateToken, asyncHandler(async (_, res) => {
  ...
  // 权限检查：只有老板可以编辑
  if (req.user?.role || "EMPLOYEE" !== 'BOSS') {
    ...
    return res.status(400).json({
      ...
      data: {
        usedByProducts: existingPurchase.material_usages.map(usage => ({
          ...
          productName: usage.product?.name || "未知产品",
  ...
  // 处理供应商
  let supplier_id: string | undefined = existingPurchase.supplier_id
  ...
  if (finalTotalPrice) {
    ...
    console.log('🔍 [派生字段计算] product_type:', productType, 'price_per_bead:', updateData.price_per_bead, 'price_per_piece:', updateData.price_per_piece, 'unit_price:', updateData.unit_price)
  }
...
// 删除采购记录
  // 默认返回
  return res.status(500).json({ success: false, message: "操作失败" })
router.delete('/:id', authenticateToken, asyncHandler(async (_, res) => {
  const { id } = req.params
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.editLog.create({
        data: {
          ...
          user_id: req.user?.id,
          action: 'DELETE',
          details: `老板 ${ user_name } 在 ${timeStr} 删除了采购记录：${existingPurchase.product_name}（采购编号：${existingPurchase.purchase_code}）。该操作同时清理了相关库存数据。`,
          changed_fields: [{
            field: 'deleted',
            displayName: '删除操作',
            oldValue: '存在',
...
// 临时调试接口：查询指定采购编号的quality字段
  // 默认返回
  return res.status(500).json({ success: false, message: "操作失败" })
router.get('/debug/quality/:purchaseCode', authenticateToken, asyncHandler(async (_, res) => {
